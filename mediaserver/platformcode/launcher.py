...
def run(item):
    ...
    #Action Search, para mostrar el teclado y lanzar la busqueda con el texto indicado.
      ...
      if item.action =="filterchannels":
        itemlist = channelselector.filterchannels(item.channel_type, "banner")
    ...
    if type(itemlist)==list:
      if  len(itemlist) ==0:
        itemlist = [Item(title="No hay elementos para mostrar", thumbnail=config.get_thumb("thumb_error.png"))]
...
def add_pelicula_to_library(item):
  ...
  videolibrarytools.add_pelicula_to_library(item)

def add_serie_to_library(item):
  channel = ImportarCanal(item)
  videolibrarytools.add_serie_to_library(item, channel)
...
def add_to_favorites(item):
    ...
    if title is not None:
        ...
        platformtools.dialog_ok("Alfa Mediaserver", config.get_localized_string(30102) +"\n"+ item.title +"\n"+ config.get_localized_string(30108))
    return
...
def add_to_library(item):
    ...
    platformtools.dialog_ok("Alfa Mediaserver", config.get_localized_string(30101) +"\n"+ item.title +"\n"+ config.get_localized_string(30135))
    return
