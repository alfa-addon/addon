...
class html(Controller):
    ...
    def __init__(self, handler=None, ID = None):
        ...
        if self.handler:
            ...
            self.send_message({"action": "connect", "data":{"version": "alfa %s" % versiontools.get_current_plugin_version_tag(), "date":versiontools.get_current_plugin_date()}})
...
class platform(Platformtools):
    ...
    def render_items(self, itemlist, parent_item):
        ...
        if parent_item.channel == "channelselector" and not parent_item.action == "filterchannels":
          # ALFA: Como ya no existen los elementos "banner" de la GUI se cambia a "movies" y se ve bastante mejor
          parent_item.viewmode = "movies"
        ...
        if not (parent_item.channel=="channelselector" and parent_item.action=="mainlist") and not itemlist[0].action=="go_back":
          if parent_item.viewmode in ["banner", "channel"]:
            itemlist.insert(0,Item(title="Atrás", action="go_back",thumbnail=config.get_thumb("banner_back.png")))
          else:
            itemlist.insert(0,Item(title="Atrás", action="go_back",thumbnail=config.get_thumb("thumb_back.png")))
        ...
        for item in itemlist:
            ...
            if not item.thumbnail and item.action == "search": item.thumbnail = config.get_thumb("thumb_search.png")
            if not item.thumbnail and item.folder and parent_item.channel != "channelselector": item.thumbnail = config.get_thumb("thumb_folder.png")
            if not item.thumbnail and not item.folder: item.thumbnail = config.get_thumb("thumb_nofolder.png")
            # Removed unused code to clean some images. Keeping fake URL to have a future reference
            #if "https://raw.githubusercontent.com/alfa/media/master" in item.thumbnail and not item.thumbnail.startswith("https://raw.githubusercontent.com/alfa/media/master/addon/thumb_"):
            #
            #  if parent_item.viewmode in ["banner", "channel"]:
            ...
            #elif parent_item.viewmode in ["banner", "channel"] and item.thumbnail.startswith("https://raw.githubusercontent.com/alfa/media/master/addon/thumb_"):
            #  item.thumbnail = ""
    ...
    def play_torrent(self,item):
        ...
        progreso = self.dialog_progress( "Alfa Mediaserver - Torrent" , "Iniciando...")
