...
finds = {'find':  dict([('find', [{'tag': ['div'], 'class': ['thumbs-holder']}]),
                       ('find_all', [{'tag': ['a'], 'class': ['js-open-popup']}])]),
         'categories': dict([('find', [{'tag': ['div'], 'class': ['thumbs-holder']}]),
...
def mainlist(item):
    ...
    itemlist.append(Item(channel=item.channel, title="Canal" , action="section", url=host + "sites/?sort_by=avg_videos_popularity&from=1", extra="Canal"))
...
def play(item):
    ...
    if soup.find('div', class_='info-block').find_all('a', href=re.compile("/(?:pornstars|models)/[A-z0-9-]+/")):
        pornstars = soup.find('div', class_='info-block').find_all('a', href=re.compile("/(?:pornstars|models)/[A-z0-9-]+/"))
