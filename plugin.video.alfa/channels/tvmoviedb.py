...
perfil = [['0xFFFFE6CC', '0xFFFFCE9C', '0xFF994D00', '0xFFFE2E2E', 'lime', '0xFFFFD700'],
...
def mainlist(item):
    ...
    itemlist.append(item.clone(title=config.get_localized_string(70023), action="imdb", extra="tv",
                               ...
                               thumbnail=images_predef + "0/TV%20Series.png"))
    itemlist.append(
        item.clone(title=config.get_localized_string(70415), action="trakt", text_color=color2, thumbnail="https://i.imgur.com/5sQjjuk.png"))
    #itemlist.append(
    ...
    itemlist.append(item.clone(title=config.get_localized_string(70036), action="search_",
                               url=urlparse.urljoin(host_imdb, "search/name/?start=1&name="), text_color=color4))
...
def imdb(item):
    ...
    if item.extra == "movie":
        itemlist.append(item.clone(title=config.get_localized_string(70030), action="listado_imdb",
                                   url=urlparse.urljoin(host_imdb, "showtimes/location?ref_=inth_ov_sh_sm")))
    ...
    if item.extra == "movie":
        itemlist.append(item.clone(title=config.get_localized_string(70034), action="listado_imdb", 
                                   url=urlparse.urljoin(host_imdb, "calendar/?ref_=rlm&region=%s&type=MOVIE") % country.upper()))
    
    itemlist.append(item.clone(title=config.get_localized_string(70033), action="listado_imdb",
                               url=urlparse.urljoin(host_imdb, "search/name/?gender=male,female&ref_=nv_cel_m_3&start=1"), 
                               text_color=color4))

    if config.get_platform() != "plex":
        title = item.contentType.replace("movie", config.get_localized_string(70283)).replace("tvshow", "serie")
        itemlist.append(item.clone(title="Buscar %s" % title, action="search_",
                                   url=urlparse.urljoin(host_imdb, "search/title/?%s&title=" % item.url), text_color=color4))
...
def filmaf(item):
    ...
    if item.extra == "movie":
        itemlist.append(item.clone(title=config.get_localized_string(70040), action="listado_fa", extra="top",
                                   url=urlparse.urljoin(host_filmaffinity, "%s/topgen.php?genres=&chv=0&orderby=avg&"
                                   "movietype=movie&country=&"% langf)))
        itemlist.append(item.clone(title=config.get_localized_string(70030), action="listado_fa", extra="cartelera", 
                                   url=urlparse.urljoin(host_filmaffinity, "%s/rdcat.php?id=new_th_%s" % (langf, langf))))
        itemlist.append(item.clone(title=config.get_localized_string(70032), action="indices_fa", extra="genres", 
                                   url=urlparse.urljoin(host_filmaffinity, "%s/topgen.php?chv=0&orderby=avg&"
                                   "movietype=movie&country=&"% langf),
                                   thumbnail="%s0/Genres.png" % images_predef))
    else:
        itemlist.append(item.clone(title=config.get_localized_string(70040), action="listado_fa", extra="top",
                                   url=urlparse.urljoin(host_filmaffinity, "%s/topgen.php?genres=&country=&orderby=avg&"
                                       "nodoc&movietype=serie" % langf)))
        itemlist.append(item.clone(title="Series de actualidad", action="listado_fa", extra="series_act",
                                   url=urlparse.urljoin(host_filmaffinity, "%s/topgen.php?genres=-DO&chv=1&orderby=avg&"
                                       "movietype=serie&country=&fromyear=%s&toyear=%s" % (langf, year-1, year))))

    itemlist.append(item.clone(title=config.get_localized_string(70042), action="indices_fa", extra='years', 
    ...
    if item.extra == "movie":
        itemlist.append(item.clone(title=config.get_localized_string(70043), action="listado_fa", extra="estrenos",
                                   url=urlparse.urljoin(host_filmaffinity, "%s/rdcat.php?id=upc_th_%s" % (langf, langf))))
        itemlist.append(item.clone(title=config.get_localized_string(70044), action="indices_fa", extra="sagas",
                                   url=urlparse.urljoin(host_filmaffinity, "%s/movie-groups-all.php" % langf)))
    itemlist.append(item.clone(title=config.get_localized_string(70045), action="indices_fa", extra="topics",
                               url=urlparse.urljoin(host_filmaffinity, '%s/topics.php' % langf), text_color=color3))
    if config.get_platform() != "plex":
        itemlist.append(item.clone(title=config.get_localized_string(70046), action="search_", text_color=color4, extra="search",
                                   url=urlparse.urljoin(host_filmaffinity, "%s/search.php?stype=title&from=0&orderby=relevance&stext=" % langf)))

        itemlist.append(item.clone(title=config.get_localized_string(70036), action="search_", text_color=color4, extra="search",
                                   url=urlparse.urljoin(host_filmaffinity, "%s/search.php?stype=cast&from=0&orderby=relevance&stext=" % langf)))
        itemlist.append(item.clone(title=config.get_localized_string(70047), action="search_", text_color=color4, extra="search",
                                   url=urlparse.urljoin(host_filmaffinity, "%s/search.php?stype=director&from=0&orderby=relevance&stext=" % langf)))
...
def mal(item):
    ...
    itemlist.append(
        item.clone(title=config.get_localized_string(70058), url=urlparse.urljoin(host_mal, "topanime.php?type=tv&limit=0"), action="top_mal",
                   contentType="tvshow", extra="tv"))
    itemlist.append(item.clone(title=config.get_localized_string(70059), url=urlparse.urljoin(host_mal, "topanime.php?type=movie&limit=0"),
                               action="top_mal",
                               contentType="movie", extra="movie"))
    itemlist.append(
        item.clone(title=config.get_localized_string(70061), url=urlparse.urljoin(host_mal, "topanime.php?type=ova&limit=0"), action="top_mal",
                   contentType="tvshow", extra="tv", tipo="ova"))
    itemlist.append(
        item.clone(title=config.get_localized_string(70028), url=urlparse.urljoin(host_mal, "topanime.php?type=bypopularity&limit=0"),
                   action="top_mal"))
    itemlist.append(item.clone(title=config.get_localized_string(70060), url=urlparse.urljoin(host_mal, "topanime.php?type=upcoming&limit=0"),
                               action="top_mal"))
    ...
    if config.get_platform() != "plex":
        itemlist.append(item.clone(title=config.get_localized_string(70064), url=urlparse.urljoin(host_mal, "anime.php?q="),
                                   action="search_"))
    ...
    if "collection" in item.search["url"]:
        ...
        try:
            ...
            for i in range(0, len(ob_tmdb.results)):
                new_item = item.clone(action="detalles", url='', infoLabels={'mediatype': item.contentType}, unify=unify_config)
...
def detalles(item):
    ...
    if not item.infoLabels["tmdb_id"]:
        ...
        data = httptools.downloadpage(urlparse.urljoin(host_imdb, "title/%s/" % item.infoLabels['imdb_id']), headers=headers, **kwargs).data

        pics = scrapertools.find_single_match(data, 'showAllVidsAndPics.*?href=".*?(tt\d+)')
        # Imágenes imdb
        if pics:
            images["imdb"] = {'url': urlparse.urljoin(host_imdb, '_json/title/%s/mediaviewer' % pics)}
    ...
    itemlist.append(item.clone(action="busqueda", title=config.get_localized_string(70069) % (title, item.contentTitle), unify=unify_config))
    if item.infoLabels['originaltitle'] and item.contentTitle != item.infoLabels['originaltitle']:
        itemlist.append(item.clone(action="busqueda", contentTitle=item.infoLabels['originaltitle'],
                                   title=config.get_localized_string(70070) % item.infoLabels['originaltitle'], unify=unify_config))
    ...
    try:
        if item.contentType == "movie" and item.infoLabels["year"] < 2014:
            url = urlparse.urljoin(host_theost, "search/custom/?key=%s&year=%s&country=0&genre=0" % (
...
def indices_tmdb(item):
    ...
    if config.get_localized_string(70032) in item.title:
        ...
        url = (urlparse.urljoin(host_tmdb, '3/genre/%s/list?api_key=a1ab8b8669da03637a4b98fa39c39228&language=%s'
               % (item.extra, langt)))
        try:
            lista = jsontools.load(httptools.downloadpage(url, cookies=False, **kwargs).data)["genres"]
...
def filtro(item):
    ...
    if "Personalizado" in item.title:
        ...
        url = (urlparse.urljoin(host_tmdb, '3/genre/%s/list?api_key=f7f51775877e0bb6703520952b3c7840&language=%s'
               % (item.extra, langt)))
        try:
            lista = jsontools.load(httptools.downloadpage(url, cookies=False, **kwargs).data)["genres"]
...
def listado_imdb(item):
    ...
    if 'search/name' in item.url:
        patron = '<td class="image">.*?src="([^"]+)".*?href="/name/(nm\d+).*?>([^<]+)<.*?href.*?>([^<]+)</a>' \
                 '</span>(.*?)</td>'
        patron = '<div\s*class="lister-item-image">\s*<a\s*href="\/name\/(nm\d+)">\s*' \
        ...
        for imdb_id, thumbnail, title, movie, datos in matches:
            ...
            new_item.infoLabels['year'] = scrapertools.find_single_match(datos, 'born\s*(?:on)?\s*\w+\s*\d{1,2},?\s*(\d{4})')
            if new_item.infoLabels['year']: 
                year = ' [COLOR grey](%s)[/COLOR]' % new_item.infoLabels['year']
            else:
                year = ''
            new_item.title = title.strip() + year + '   [COLOR %s](%s)[/COLOR]' % (color6, movie.strip())
        ...
        if 'calendar/' in item.url:
            estreno = ''
            patron = '(?:data-testid="release-date">\s*<hgroup>\s*<h3\s*class=' \
        ...
        for thumbnail, thumb2, imdb_id, title, datos, plot, plot2, estreno in matches:
            ...
            new_item.infoLabels['year'] = scrapertools.find_single_match(title, '\(*(\d{4})')
            ...
            if estreno: 
                ...
                if not new_item.infoLabels['year']: new_item.infoLabels['year'] = '-'
            
            itemlist.append(new_item)

    if 'calendar/' in item.url: tmdb.set_infoLabels_itemlist(itemlist, True)
    ...
    if next_page:
        if "title_type" in item.url:
            ...
            lines = int(scrapertools.find_single_match(item.url, 'start=(\d+)') or 0) + len(matches)
...
def filtro_imdb(item):
    ...
    try:
        ...
        data = httptools.downloadpage(urlparse.urljoin(host_imdb, "search/title"), cookies=False, **kwargs).data
...
def filtrado_imdb(item, values):
    ...
    item.url = urlparse.urljoin(host_imdb, 'search/title?countries=%s&num_votes=%s,&genres=%s&release_date=%s,%s&sort=%s&' \
...
def indices_imdb(item):
    ...
    if config.get_localized_string(70032) in item.title:
        generos_spa = {'Action': config.get_localized_string(70394), 'Adventure': config.get_localized_string(60267), 'Animation': config.
        
        get_localized_string(60268), 'Biography': config.get_localized_string(70403), 'Thriller': config.get_localized_string(70410),
        ...
        if matches:
            for valor, titulo in matches:
                ...
                itemlist.append(item.clone(title=title, action='listado_imdb', thumbnail=thumbnail,
                                           url=urlparse.urljoin(host_imdb, 'search/title/?genres=%s&%s' % (valor, item.url))))
        ...
        for i in range(year, 1899, -1):
            itemlist.append(item.clone(title=str(i), action='listado_imdb',
                                       url=urlparse.urljoin(host_imdb, 'search/title/?release_date=%s,%s&%s' % (i, i, item.url))))
...
def listado_fa(item):
    ...
    if item.page_fa:
        post = {'from': item.page_fa, 'paramGrid[numRows]': 5, 'paramGrid[position]': 'true'}
        post = urllib.urlencode(post)
        data = httptools.downloadpage(item.url, post=post, referer=item.url, **kwargs).data
        ...
        data = httptools.downloadpage(item.url, **kwargs).data
        item.page_fa = int(scrapertools.find_single_match(data, 'tmPageSize \s*=\s*(\d+)') or 30)
        item.total = int(scrapertools.find_single_match(data, 'tmResCount\s*=\s*(\d+)') or 30)
        if item.total <= item.page_fa:
            item.page_fa = ""
    ...
    if item.extra in ['top']:
        # Top
        patron = '<div\s*class="mc-title"\s*>\s*<a\s*href="([^"]+)"\s*title="([^"]+)"\s*>' \
    ...
    if not matches and item.extra in ['sagas']:
        # Sagas
        patron = '<div\s*class="mc-poster"\s*>\s*<a\s*href="([^"]+)">\s*<img[^>]*src="([^"]+)".*?' \
    ...
    if not matches and item.extra in ['cartelera', 'estrenos']:
        # Cartelera
        patron = '<div\s*class="mc-poster"\s*>\s*<a\s*href="([^"]+)"\s*title="([^"]+)"\s*>' \
    ...
    if not matches and item.extra in ['search']:
        # Búsquedas
        patron = '<div\s*class="ye-w">(\d{4})<\/div>.*?<div\s*class="mc-poster"\s*>\s*' \
    ...
    if not matches and item.extra in ['series_act']:
        ...
        patron = '<a\s*class="movie-poster-grid"\s*href="([^"]+)".*?<div\s*class=' \
    ...
    for url, _title, year, thumb, url2, rating, votos, estreno in matches:
        ...
        if not thumb:
            thumb = thumb2
        new_item.thumbnail = urlparse.urljoin(host_filmaffinity, thumb.replace("msmall", "large"))

        new_item.contentTitle = re.sub(r'(?i)\((?:mini)?serie de tv\)|\(tv\)|\(c\)', '', title)
        new_item.title = new_item.contentTitle + "  (%s) [COLOR %s]%s[/COLOR]" % (year, color6, rating)
        
        if re.search(r'(?i)(?:mini)?serie de tv|\(tv\)', title):
            if item.contentType == "movie" and item.extra != "sagas": continue
            new_item.contentType = "tvshow"
            new_item.extra = "tv"
            new_item.infoLabels["tvshowtitle"] = new_item.contentTitle
        elif item.contentType == "tvshow": continue

        new_item.infoLabels['year'] = year or '-' if new_item.contentType == "movie" else ''
        
        votaciones.append([rating, votos])
        
        if rating:
            new_item.infoLabels['rating'] = float(rating.replace(",", ".").replace("--", "0.0"))
    ...
    for i, it in enumerate(itemlist):
        try:
            ...
            it.infoLabels['rating'] = float(votaciones[i][0].replace(",", ".").replace("--", "0.0"))
        except:
            pass

    if item.page_fa or (matches and item.extra in ['search']):
        itemlist.append(Item(channel=item.channel, action=item.action, title=config.get_localized_string(70065), url=item.url,
                             extra=item.extra, contentType=item.contentType, page_fa=item.page_fa, total=item.total, unify=False))
...
def indices_fa(item):
    ...
    if "sagas" in item.extra:
        ...
        for url, title, thumbnail, info in matches:
            ...
            new_item.url = urlparse.urljoin(host_filmaffinity, url)
            new_item.thumbnail = urlparse.urljoin(host_filmaffinity, thumbnail.replace("msmall", "large"))
        ...
        if next_page:
            ...
            itemlist.append(Item(channel=item.channel, action=item.action, title=config.get_localized_string(70065), 
                                 url=next_page, extra=item.extra))
    
    elif config.get_localized_string(70032) in item.title:
        matches = scrapertools.find_multiple_matches(data, 'name="genres\[\]"\s*value="(\+[^"]+)">.*?name="genres\[\]"\s*disabled>\s*([^<]+)\s*<')
        ...
        if not 'topic_' in item.url:
            action = "indices_fa"
            for tema, letra, bloque in bloques:
                itemlist.append(item.clone(title='Temas en ' + letra, url='%s#%s' % (item.url, tema), text_color=color2, action=action, extra=item.extra))
        else:
            for tema, letra, bloque in bloques:
                if tema not in item.url: continue
                    
                patron = '<li>\s*<a\s*class="topic"\s*href="([^"]+)">\s*([^<]+)\s*<em>\((\d+)\)<\/em>'
                matches = scrapertools.find_multiple_matches(bloque, patron)
                extra = len(matches) + 1

                itemlist.append(item.clone(title='Temas en ' + letra, text_color=color2, extra=extra, folder=folder))
        ...
        for i in range(year, 1899, -1):
            ...
            new_item.url = urlparse.urljoin(host_filmaffinity, "%s/topgen.php?genres=&country=&fromyear=%s&toyear=%s&"
                                            "nodoc=1&orderby=avg&movietype=%s" % (langf, i, i, 
...
def temas_fa(item):
    ...
    for url, thumb, title, year in matches:
        title = title.strip()
        new_item = item.clone(action="detalles_fa", contentType="movie", extra="movie", text_color=color2, unify=unify_config)
        new_item.url = urlparse.urljoin(host_filmaffinity, "%s/film%s.html" % (langf, url))
        new_item.thumbnail = urlparse.urljoin(host_filmaffinity, thumb.replace("msmall", "large"))
...
def detalles_fa(item):
    ...
    orig_title = scrapertools.find_single_match(data, '(?i)<dt>T.tulo\s*original<\/dt>\s*<dd>\s*([^<]+)\s*<\/dd>').strip()
    if orig_title:
        item.infoLabels['originaltitle'] = re.sub(r"(?i)\(TV Series\)\s*|\(Miniserie de TV\)\s*|\(S\)|\(TV\)\s*", "", orig_title)
    ...
    generos = scrapertools.find_multiple_matches(data, '<span[^>]*itemprop="genre">\s*<a\s*href="([^"]+)">([^<]+)<\/a><\/span>')
    genres = []
    if generos:
        for url, g in generos:
            if 'Serie de TV' in g: continue
            genres.append(scrapertools.htmlclean(g.strip()))
        item.infoLabels['genre'] = ", ".join(genres)

    rating = scrapertools.find_single_match(data, 'itemprop="ratingValue".*?>([^<]+)<').strip().replace('-.-', '0.0').replace('-', '0')
    if rating:
    ...
    votos = scrapertools.find_single_match(data, 'itemprop="ratingCount".*?>([^<]+)<').strip().replace('-', '0')
    if votos == "0" and ob_tmdb.result:
        votos = ob_tmdb.result.get('vote_count', '')
    if votos:
        item.infoLabels['votes'] = votos

    fanart = scrapertools.find_single_match(data, ' <div\s*id="movie-main-image-container">[^>]*href="([^"]+)"')
    ...
    itemlist.append(item.clone(action="busqueda", title=config.get_localized_string(70069) % (title, item.contentTitle), 
                               unify=unify_config, plot_extend=plot_extend % 'Buscar'))
    ...
    if config.is_xbmc():
        item.contextual = True
    trailer_url = scrapertools.find_single_match(data, '(?i)<li\s*>\s*<a\s*href="([^"]+)"\s*>Tr.ilers[^<]*<em>\s*\[\d*\]')
    itemlist.append(item.clone(channel="trailertools", action="buscartrailer", title="Buscar Tráiler",
                               text_color=color5, filmaffinity=trailer_url))

    url_img = scrapertools.find_single_match(data, '(?i)<li\s*>\s*<a\s*href="([^"]+)"\s*>Im.genes[^<]*<em>\s*\[\d*\]')
    images = {}
...
def filtro_fa(item):
    ...
    data = httptools.downloadpage("https://m.filmaffinity.com/%s/topgen.php" % langf, **kwargs).data
    data = re.sub(r"\n|\r|\t|&nbsp;", "", data)
    data = re.sub(r"\s{2}", " ", data)
...
def filtrado_fa(item, values):
    ...
    item.url = urlparse.urljoin(host_filmaffinity, '%s/topgen.php?genre=%s&country=%s&fromyear=%s&toyear=%s&nodoc=1' \
...
def login_fa():
    ...
    try:
        ...
        data = httptools.downloadpage(urlparse.urljoin(host_filmaffinity, "%s" % langf), **kwargs).data
        
        if "modal-menu-user" in data and userid:
            return True, ""

        post = "postback=1&rp=&username=%s&password=%s&rememberme=on" % (user, password)
        data = httptools.downloadpage(urlparse.urljoin(host_filmaffinity, "%s/account.ajax.php?action=login" % langf), post=post, **kwargs).data
        ...
        if "Invalid username" in data:
            ...
            data = httptools.downloadpage(urlparse.urljoin(host_filmaffinity, "%s/tpl.ajax.php?action=getTemplate" % langf),
                                          post=post, **kwargs).data
...
def cuenta_fa(item):
    ...
    if not login:
        ...
        itemlist.append(item.clone(action="acciones_fa", title=config.get_localized_string(70332), text_color=color5, accion="votos",
                                   url=urlparse.urljoin(host_filmaffinity, "%s/user_ratings.php?id-user=%s" % (langf, userid))))
...
def acciones_fa(item):
    ...
    if item.accion == "votos" or item.accion == "lista":
        ...
        for bloque in bloques:
            ...
            for url, thumb, title, year, rating, votos in matches:
                ...
                if not new_item.thumbnail.startswith("http"):
                    new_item.thumbnail = urlparse.urljoin(host_filmaffinity, new_item.thumbnail)
        ...
        for url, title, content, imgs in matches:
            ...
            new_item.url = urlparse.urljoin(host_filmaffinity, "%s&orderby=%s" % (url, orderby))
        ...
        url = urlparse.urljoin(host_filmaffinity, "%s/edtmovielists.php?movie_id=%s" % (langf, movieid))
...
def callback_voto(item, values):
    ...
    data = jsontools.load(httptools.downloadpage(urlparse.urljoin(host_filmaffinity, "%s/ratingajax.php" % langf), post=post, **kwargs).data)
...
def newlist(item):
    ...
    if item.accion == "lista":
        ...
        data = httptools.downloadpage(urlparse.urljoin(host_filmaffinity, location), **kwargs).data
        
        itemlist.append(item.clone(action="", title=config.get_localized_string(70338)))
    
    else:
        url = urlparse.urljoin(host_filmaffinity, "%s/addlist.php?rp=%s" % (langf, item.url))
        data = httptools.downloadpage(url, **kwargs).data
        data = re.sub(r"\n|\r|\t|&nbsp;|\s{2}", "", data)
...
def imagenes(item):
    ...
    if item.extra == "menu":
        ...
        if "myanimelist" in item.images:
            data = httptools.downloadpage(item.url + "/pics", cookies=False, **kwargs).data
    ...
    if "Fanart" in item.title:
        ...
        try:
            ...
            patron = '<div\s*class="colorbox-image"[^>]*>\s*<a\s*href="([^"]+)"\s*' \
        ...
        try:
            data = jsontools.load(httptools.downloadpage(item.images["imdb"]["url"], cookies=False, **kwargs).data)
...
def fanartv(item):
    ...
    if id_search:
        if item.contentType == "movie":
            url = "https://webservice.fanart.tv/v3/movies/%s?api_key=cab16e262d72fea6a6843d679aa10300" \
                  % item.infoLabels['tmdb_id']
        else:
            url = "https://webservice.fanart.tv/v3/tv/%s?api_key=cab16e262d72fea6a6843d679aa10300" % id_search
        #kwargs['replace_headers'] = True
        data = jsontools.load(httptools.downloadpage(url, headers=headers, **kwargs).data)
...
def detalles_mal(item):
    ...
    data = httptools.downloadpage(item.url, headers=header_mal, cookies=False, **kwargs).data
    data = re.sub(r"\n|\r|\t|&nbsp;", "", data)
    data = re.sub(r"\s{2}", " ", data)
    ...
    try:
        ...
        data_music = httptools.downloadpage("https://www.freeanimemusic.org/song_search.php", post=post, **kwargs).data
...
def videos_mal(item):
    ...
    if not "No episode video" in data and not "https://www.hulu.com/" in data:
...
def indices_mal(item):
    ...
    if "Temporadas" in item.title:
        data = httptools.downloadpage("https://myanimelist.net/anime/season/archive", cookies=False, **kwargs).data
        data = re.sub(r"\n|\r|\t|&nbsp;", "", data)
        data = re.sub(r"\s{2}", " ", data)
        ...
        data = httptools.downloadpage(urlparse.urljoin(host_mal, "anime.php"), cookies=False, **kwargs).data
        data = re.sub(r"\n|\r|\t|&nbsp;", "", data)
        data = re.sub(r"\s{2}", " ", data)
...
def detail_staff(item):
    ...
    if item.extra == "character" and not "No biography written" in data:
        ...
        if not "questionmark" in item.thumbnail:
            data_img = httptools.downloadpage(item.url + "/pictures", cookies=False, **kwargs).data
        ...
        if not "No staff positions" in data:
            ...
            for url, thumb, title, rol in matches:
                ...
                i
                temlist.append(Item(channel=item.channel, action="detalles_mal", url=url, text_color=color2,
...
def callback_mal(item, values):
    ...
    item.url = urlparse.urljoin(host_mal, "anime.php?q=%s&type=%s&score=%s&status=%s" \
...
def musica_anime(item):
    ...
    data = httptools.downloadpage("https://www.freeanimemusic.org/song_search.php", post=item.post, **kwargs).data
...
def login_mal(from_list=False):
    ...
    try:
        ...
        response = httptools.downloadpage(urlparse.urljoin(host_mal, "logout.php"), post="csrf_token=%s" % token, **kwargs)
        
        post = "user_name=%s&password=%s&cookie=1&sublogin=Login&submit=1&csrf_token=%s" % (user, password, token)
        response = httptools.downloadpage(urlparse.urljoin(host_mal, "login.php?from=%2F"), post=post, 
                                          referer= urlparse.urljoin(host_mal, "login.php?from=%2F"), **kwargs)
...
def menu_mal(item):
    ...
    status = {'1': config.get_localized_string(70479), '2': config.get_localized_string(70480), 
              '3': config.get_localized_string(70384), '4': config.get_localized_string(70385),
              '6': config.get_localized_string(70481)}
...
def addlist_mal(item):
    ...
    #kwargs['replace_headers'] = True
    data = httptools.downloadpage(url, post=jsontools.dump(post), headers=headers_mal, **kwargs).data
    ...
    if not item.server:
        ...
        if item.server == "directo" and item.song:
            ...
            data_music = jsontools.load(
                httptools.downloadpage("https://www.musicaanime.org/scripts/resources/artists1.php", **kwargs).data)
            for child in data_music["data"]:
                if child["title"] == item.url.upper():
                    url = "https://www.musicaanime.org/aannmm11/%s/imagen%s.mp3" % (child["artist"], item.song.zfill(3))
                    break
            if url:
