...
def trakt(item):
    ...
    if not item.extra:
        ...
        itemlist.append(item.clone(title="[COLOR yellow]%s[/COLOR][B]" % config.get_localized_string(70417), action="",))
        item.extra = "show"
        ...
        itemlist.append(item.clone(title="[COLOR red]%s[/COLOR][B]" % config.get_localized_string(70048), extra="cuenta"))
    else:
        item.extra = "movie"
