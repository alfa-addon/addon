...
def mainlist(item):
    ...
    itemlist.append(Item(channel=item.channel, title="Pornstars" , action="section", url=host + "porno-models?sort_by=total_videos&from=01", extra="PornStar"))
    itemlist.append(Item(channel=item.channel, title="Categorias" , action="section", url=host + "porno-cats?sort_by=title", extra="Categorias"))
...
def play(item):
    ...
    if soup.find_all('a', href=re.compile("/(?:pornstars|models|model|pornosztarok)/[A-z0-9-]+(?:/|)")):
        ...
        for x, value in enumerate(pornstars):
            pornstars[x] = value.span.get_text(strip=True)
