...
#  https://pornwild.to   https://0dayporn.com
canonical = {
             'channel': 'yeapornpls', 
             'host': config.get_setting("current_host", 'yeapornpls', default=''), 
             'host_alt': ["https://pornwild.to"], 
             'host_black_list': [], 
...
def mainlist(item):
    ...
    itemlist.append(Item(channel=item.channel, title="Pornstars" , action="categorias", url=host + "/models/?sort_by=avg_videos_rating&from=1"))
...
def categorias(item):
    ...
    for scrapedurl,scrapedtitle,scrapedthumbnail,cantidad in matches:
        ...
        itemlist.append(Item(channel=item.channel, action="lista", title=title, url=url, fanart=thumbnail, thumbnail=thumbnail, plot="") )
