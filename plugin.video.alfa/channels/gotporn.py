...
canonical = {
             'channel': 'gotporn', 
             'host': config.get_setting("current_host", 'gotporn', default=''), 
             'host_alt': ["https://www.gotporn.com"], 
             'host_black_list': [], 
...
def mainlist(item):
    logger.info()
    itemlist = []
    itemlist.append(Item(channel=item.channel, title="Nuevos" , action="lista", url=host + "/?src=mn:mr&page=1"))
...
def lista(item):
    ...
    if "/channels/" in item.url:
        ...
        offset += 15
        itemlist.append(Item(channel=item.channel, action="lista", title="[COLOR blue]Página Siguiente >>[/COLOR]", url=next_page, post=post, offset=offset) )
    return itemlist
