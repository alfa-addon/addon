...
canonical = {
             'channel': 'theyarehuge', 
             'host': config.get_setting("current_host", 'theyarehuge', default=''), 
             'host_alt': ["https://www.theyarehuge.com/"], 
             'host_black_list': [], 
...
def mainlist(item):
    ...
    itemlist.append(Item(channel=item.channel, title="Mas popular" , action="lista", url=host + "popular.porn-video/"))
    itemlist.append(Item(channel=item.channel, title="Categorias" , action="categorias", url=host + "porn-video.tags/"))
...
def lista(item):
    ...
    matches = soup.find_all('a', class_='item drclass')
    for elem in matches:
        url = elem['href']
        # title = elem['title']
        # title = elem.img['alt']
