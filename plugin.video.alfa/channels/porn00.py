...
###################  Certificado SSL >> proxy  resuelve mal KTP
...
finds = {'find':  dict([('find', [{'tag': ['div'], 'class': ['video-list', 'list-videos']}]),
         ...
         'categories': dict([('find', [{'tag': ['div'], 'class': ['models_list', 'list-models', 'category_list','list-categories', 'video-list', 'list-sponsors']}]),
...
def section(item):
    ...
    findS = finds.copy()
    findS['url_replace'] = [['(\/(?:categories|category-name|channels|sites|models|pornstars|star-name)\/[^$]+$)', r'\1?sort_by=post_date&from=1']]
...
# def play(item):
    ...
    # if soup.find_all('a', href=re.compile("/(?:pornstars|models|star-name)/[A-z0-9-]+/")):
        # pornstars = soup.find_all('a', href=re.compile("/(?:pornstars|models|star-name)/[A-z0-9-]+/"))
