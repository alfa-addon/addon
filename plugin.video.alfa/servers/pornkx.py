# Conector pornkx By Alfa development Group
# --------------------------------------------------------
...
def get_video_url(page_url, premium=False, user="", password="", video_password=""):
    ...
    try:
        ...
        kaken= scrapertools.find_single_match(unpacked, 'kaken="([^"]+)"')
        post_url = '%s/api/?%s' %(host, kaken)
        datos = httptools.downloadpage(post_url, **kwargs).json
        
        m3u8_source = datos['sources'][0]['file']
        m3u8_source = httptools.downloadpage(m3u8_source, follow_redirects=False, only_headers=True, **kwargs).headers.get("location", "")
