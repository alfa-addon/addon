...
def test_video_exists(page_url):
    ...
    # if data.get('message', ''):
        # return False, "[bbwpage] El fichero no existe o ha sido borrado"
    return True, ""
...
def get_video_url(page_url, video_password):
    ...
    patron = "OLID =\s*'([^']+)'"
    texto = scrapertools.find_single_match(data, patron)
    encode = texto[::-1]
    decode = base64.b64decode(encode).decode('utf-8')
    url = urlparse.unquote(decode)
    # url = url.replace("\/", "/")
    video_urls.append(["[bbwpage]", url])
    return video_urls#[::-1]
