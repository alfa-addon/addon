...
def dialog_multiselect(heading, _list, autoclose=0, preselect=[], useDetails=False):
    return xbmcgui.Dialog().multiselect(
        heading, _list, autoclose=autoclose, preselect=preselect, useDetails=useDetails
    )
...
def dialog_browse(_type, heading, shares="files", default=""):
    if config.get_platform(True)["num_version"] < 18.0 and (
        shares == "local" or not shares
    ):
        shares = "files"
    dialog = xbmcgui.Dialog()
...
def dialog_qr_message(heading="", message="", qrdata=""):
    ...
    if not check_addon_installed("script.module.pyqrcode"):
        ...
        install_addon("script.module.pyqrcode")
        return False
    ...
    try:
        ...
        """
        media_folder = filetools.join(
            config.get_runtime_path(), "resources", "skins", "Default", "media"
        )
    ...
    if filetools.exists(img_path):
        ...
        try:
            ...
            logger.error(
                "Hubo un error al eliminar la imagen QR generada: %s" % img_path
            )
    else:
...
def itemlist_update(item, replace=False):
    if replace:  # reset the path history
        xbmc.executebuiltin(
            "Container.Update(" + sys.argv[0] + "?" + item.tourl() + ", replace)"
        )
    else:
        xbmc.executebuiltin(
            "Container.Update(" + sys.argv[0] + "?" + item.tourl() + ")"
        )
...
def render_items(itemlist, parent_item):
    ...
    use_unify = channel_param.get("force_unify", False) or config.get_setting(
        "unify", default=False
    )
    if use_unify:
    ...
    if (
        itemlist
        and not itemlist[0].module
        and itemlist[0].channel
        and itemlist[0].channel != "downloads"
    ):
        categories_channel = channeltools.get_channel_parameters(
            itemlist[0].channel.lower()
        ).get("categories", [])
    ...
    for item in itemlist:
        ...
        if item.unify or item.unify is False:
            use_unify = item.unify
        else:
            if channel_param.get("adult", False):
                use_unify = False
            else:
                use_unify = channel_param.get(
                    "force_unify", False
                ) or config.get_setting("unify", default=False)
        ...
        elif ("siguiente" in item.title.lower() and ">" in item.title) or (
            "pagina:" in item.title.lower()
        ):
        ...
        if (
            item.action in ["findvideos"]
            and not item.infoLabels["tmdb_id"]
            and domain_cs in CF_LIST
        ):
        ...
        if parent_item.channel != "special":
            context_commands = set_context_commands(
                item,
                item_url,
                parent_item,
                has_extendedinfo=has_extendedinfo,
                superfavourites=superfavourites,
                num_version_xbmc=num_version_xbmc,
    ...
    if config.get_setting("forceview"):  # ...forzamos segun el viewcontent
    ...
    elif parent_item.startpage is True:
        xbmcplugin.setContent(PLUGIN_HANDLE, "movies")
        ...
        not parent_item.module and parent_item.action != "mainlist"
    ):  # ... o segun el canal
        xbmcplugin.setContent(PLUGIN_HANDLE, "movies")
    ...
    if parent_item.list_type != "":
        ...
        if parent_item.category != "":
            ...
            breadcrumb = channeltools.get_channel_parameters(parent_item.channel).get(
                "title", ""
            )
        else:
    ...
    xbmcplugin.addSortMethod(
        handle=PLUGIN_HANDLE, sortMethod=xbmcplugin.SORT_METHOD_NONE
    )
    ...
    logger.info(
        "FINAL render_items %s elementos: %s" % (len(itemlist), (time.time() - start))
    )
...
def set_infolabels(listitem, item, player=False):
    ...
    infoLabels_dict = {
        ...
        "country": "country",
        "credits": "credits",
        "release_date": "dateadded",
        "dateadded": "dateadded",
        "dbid": "dbid",
        "director": "director",
        "duration": "duration",
        "episode": "episode",
        "episodio_sinopsis": "episodeguide",
        "episodio_air_date": "None",
        "episodio_imagen": "None",
        "episodio_titulo": "title",
        "episodio_vote_average": "rating",
        "episodio_vote_count": "votes",
        "fanart": "None",
        "genre": "genre",
        "homepage": "None",
        "imdb_id": "imdbnumber",
        ...
        "tag": "tag",
        "tagline": "tagline",
        "temporada_air_date": "None",
        "temporada_nombre": "None",
        "temporada_num_episodios": "None",
        "temporada_poster": "None",
        "title": "title",
        "top250": "top250",
    ...
    if item.infoLabels:
        try:
            if platform_version < 18.0:
                listitem.setUniqueIDs(
                    {
                        "tmdb": item.infoLabels.get("tmdb_id", 0),
        ...
        try:
            ...
            if platform_version >= 20.0:
                ...
                if infoLabels_kodi.get("castandrole", None) and isinstance(
                    infoLabels_kodi["castandrole"], list
                ):
                    cast = []
                    for actor, role in infoLabels_kodi["castandrole"]:
                        cast.append(xbmc.Actor(actor, role))
                    infotagvideo.setCast(cast)
                elif infoLabels_kodi.get("cast", None) and isinstance(
                    infoLabels_kodi["cast"], list
                ):
                ...
                if infoLabels_kodi.get("year", None) and isinstance(
                    infoLabels_kodi["year"], int
                ):
            ...
            if platform_version >= 20.0:
                listitem.setInfo(
                    "video", item.infoLabels
                )  # HACK: Solo en caso de excepción
            else:
                listitem.setInfo("video", item.infoLabels)
...
def set_context_commands(item, item_url, parent_item, categories_channel=[], **kwargs):
        ...
        if isinstance(command, dict):
            ...
            if "channel" in command:
                command["from_channel"] = (
                    command.get("from_channel", "") or item.channel
                )
            ...
            if (
                parent_item.module != "alfavorites"
                and "i_perfil" in command
                and "i_enlace" in command
            ):
                continue
    ...
    if "Container.Refresh" not in str(context_commands):
        context_commands.append(
            (
                "[B][COLOR grey]%s[/COLOR][/B]" % config.get_localized_string(70815),
                "Container.Refresh",
            )
        )
    ...
    if item.action and item.action not in [
        ...
        if item.infoLabels["plot"] and (
            kwargs.get("num_version_xbmc") < 17.0 or item.contentType == "season"
        ):
            context_commands.append(
                (config.get_localized_string(60348), "Action(Info)")
            )

        # ExtendedInfo: Si está instalado el addon y se cumplen una serie de condiciones
        if (
            kwargs.get("has_extendedinfo")
            and config.get_setting("extended_info", default=False) is True
        ):
            if (
                ...
                context_commands.append(
                    (
                        "ExtendedInfo",
                        "RunScript(script.extendedinfo,info=extendedepisodeinfo,%s)"
                        % param,
                    )
                )
                ...
                item.infoLabels["tmdb_id"]
                or item.infoLabels["tvdb_id"]
                or item.infoLabels["imdb_id"]
                or item.contentSerieName
            ):
            ...
            elif item.contentType == "movie" and (
                item.infoLabels["tmdb_id"]
                or item.infoLabels["imdb_id"]
        ...
        # Añadir a Favoritos
        if (
            kwargs.get("num_version_xbmc") < 17.0
            and (
                item.module not in ["favorites", "videolibrary", "help", ""]
                or item.action in ["update_videolibrary"]
            )
            and parent_item.module != "favorites"
        ):
            context_commands.append(
                (
                    config.get_localized_string(30155),
                    "RunPlugin(%s?%s&%s)"
                    % (
                        sys.argv[0],
                        item_url,
                        "module=favorites&action=addFavourite&from_channel="
                        + item.channel
                        + "&from_action="
                        + item.action,
                    ),
        ...
        if (
            item.module not in ["favorites", "videolibrary", "help", "search"]
            and parent_item.module != "favorites"
        ):
            context_commands.append(
                (
                    "[COLOR blue]%s[/COLOR]" % config.get_localized_string(70557),
                    "RunPlugin(%s?%s&%s)"
                    % (
                        sys.argv[0],
                        item_url,
        ...
        if (
            item.contentType in ["movie", "tvshow"]
            and item.channel != "search"
            and item.action not in ["play"]
        ):
            # Buscar en otros canales
            ...
            context_commands.append(
                (
                    config.get_localized_string(60350),
                    "Container.Update (%s?%s)"
                    % (
                        sys.argv[0],
                        item.clone(
            ...
            context_commands.append(
                (
                    "[COLOR yellow]%s[/COLOR]" % config.get_localized_string(70561),
                    "Container.Update (%s?%s&%s)"
                    % (
                        sys.argv[0],
                        item_url,
        ...
        if config.get_setting("start_page"):
            if item.action not in ["episodios", "seasons", "findvideos", "play"]:
                context_commands.insert(
                    0,
                    (
                        config.get_localized_string(60351),
                        "RunPlugin(%s?%s)"
                        % (
                            sys.argv[0],
                            Item(
                ...
                context_commands.append(
                    (
                        ...
                        % (
                            sys.argv[0],
                            item_url,
                            "action=add_serie_to_library&from_action=" + action,
                        ),
                    )
                )
                context_commands.append(
                    (
                        "Agregar temporada a videoteca",
                        "RunPlugin(%s?%s&%s)"
                        % (
                            sys.argv[0],
                            item_url,
                            "action=add_season_to_library&from_action=" + action,
                        ),
                    )
                )
            # Añadir Pelicula a videoteca
            elif (
                item.action in ["detail", "findvideos"]
                and item.contentType == "movie"
                and item.contentTitle
            ):
                context_commands.append(
                    (
                    ...
                    context_commands.append(
                        (
                            ...
                            % (
                                ...
                                % (
                                    item.action,
                                    item.contentSerieName or item.contentTitle,
                                    True,
                                ),
                            ),
                        )
        ...
        if item.module not in ["downloads", "search"]:
            ...
            if (
                "torrent" in str(categories_channel)
                and item.server
            ...
            if item.contentChannel and "list" not in item.contentChannel:
            ...
            if item.contentType == "movie" and item.contentTitle:
                context_commands.append(
                    (
                        config.get_localized_string(60354),
                        "RunPlugin(%s?%s&%s)"
                        % (
                            sys.argv[0],
                            item_url,
                            "module=downloads&action=save_download&from_channel="
                            + item.channel
                            + "&from_action="
                            + item.action,
                        ),
                ...
                if tc:
                    context_commands.append(
                        (
                            config.get_localized_string(60354) + en,
                            "RunPlugin(%s?%s&%s)"
                            % (
                                sys.argv[0],
                                item_url,
                                "module=downloads&action=save_download%s&from_channel="
                                % tc
                                + item.channel
                                + "&from_action="
                                + item.action,
                ...
                item.contentType in ["tvshow", "episode"]
                and item.infoLabels["tmdb_id"] == "None"
            ):
                # Descargar serie
                if item.contentType in ["tvshow", "season"] or (
                    ...
                    if tc:
                        context_commands.append(
                            (
                                config.get_localized_string(60355) + en,
                                "RunPlugin(%s?%s&%s)"
                                % (
                                    sys.argv[0],
                                    item_url,
                ...
                if (
                    item.contentType == "episode"
                    and item.server == "torrent"
                    and item.channel == "videolibrary"
                    and not item.channel_recovery
                ) or item.video_path:
                    context_commands.append(
                        (
                            "Descargar Epis NO Vistos",
                            "RunPlugin(%s?%s&%s)"
                            % (
                                sys.argv[0],
                                item_url,
                ...
                if item.contentType == "episode":
                    context_commands.append(
                        (
                            config.get_localized_string(60356),
                            "RunPlugin(%s?%s&%s)"
                            % (
                                sys.argv[0],
                                item_url,
                    ...
                    if tc:
                        context_commands.append(
                            (
                                config.get_localized_string(60356) + en,
                                "RunPlugin(%s?%s&%s)"
                                % (
                                    sys.argv[0],
                                    item_url,
                ...
                if (
                    item.contentType == "season"
                    or parent_item.action in ["episodios", "episodesxseason", "seasons"]
                    or (
                        item.contentType == "episode"
                        and item.server == "torrent"
                        and item.infoLabels["tmdb_id"] != "None"
                    ...
                    if tc:
                        context_commands.append(
                            (
                                config.get_localized_string(60357) + en,
                                "RunPlugin(%s?%s&%s)"
                                % (
                                    sys.argv[0],
                                    item_url,
        ...
        if parent_item.module not in ["setting", "news", "search"]:
            ...
            context_commands.append(
                (
                    config.get_localized_string(60358),
                    "Container.Update(%s?%s)"
        ...
        if item.action == "findvideos" or "buscar_trailer" in context:
            context_commands.append(
                (
                    config.get_localized_string(60359),
                    "RunPlugin(%s?%s)"
                    % (
                        sys.argv[0],
                        item.clone(
        ...
        if kwargs.get("superfavourites"):
            context_commands.append(
                (
                    config.get_localized_string(60361),
    ...
    context_commands.insert(
        0,
        (
            config.get_localized_string(60360),
            "Container.Update (%s?%s)"
            % (
                sys.argv[0],
...
def play_video(item, strm=False, force_direct=False, autoplay=False):
    ...
    opciones, video_urls, seleccion, salir = get_dialogo_opciones(
        item, default_action, strm, autoplay
    )
    if salir:
        return
    ...
    # truco malandro para el que viene paseando, copiando, pegando...
    ...
    if (
        force_direct
    ):  # cuando viene de una ventana y no directamente de la base del addon
...
def get_seleccion(default_action, opciones, seleccion, video_urls):
    ...
    if config.get_setting("resolve_stop", default=False) is False:
        fixpri = True
    # preguntar
    if default_action == 0:
        # "Elige una opción"
...
def show_channel_settings(**kwargs):
    ...
    return SettingsWindow("ChannelSettings.xml", config.get_runtime_path()).start(
        **kwargs
    )
...
def show_video_info(*args, **kwargs):
    ...
    return InfoWindow("InfoWindow.xml", config.get_runtime_path()).start(
        *args, **kwargs
    )
...
def alert_no_disponible_server(server):
    ...
    dialog_ok(
        config.get_localized_string(30055),
...
def handle_wait(time_to_wait, title, text):
    ...
    while secs < time_to_wait:
        ...
        remaining_display = (
            config.get_localized_string(70176)
            + secs_left
            + config.get_localized_string(70177)
        )
...
def get_dialogo_opciones(item, default_action, strm, autoplay):
    ...
    if puedes:
        for video_url in video_urls:
            ...
            opciones.append(
                "{} {}".format(config.get_localized_string(30151), video_url[0])
            )
    ...
    # Si no puedes ver el vídeo te informa
        ...
        if not autoplay:
            if item.server != "":
                if "<br/>" in motivo:
                    dialog_ok(
                        config.get_localized_string(60362),
                        motivo.split("<br/>")[0],
                        motivo.split("<br/>")[1],
                        item.url,
                    )
                ...
                dialog_ok(
                    config.get_localized_string(60362),
                    config.get_localized_string(60363),
    ...
    # "Buscar Trailer":
        ...
        xbmc.executebuiltin(
            ...
            % (
                sys.argv[0],
                item.clone(
                    module="trailertools", action="buscartrailer", contextual=True
                ).tourl(),
            )
        )
        salir = True
...
def get_video_seleccionado(item, seleccion, video_urls):
    ...
    if wait_time > 0:
        continuar = handle_wait(
            wait_time, item.server, config.get_localized_string(60365)
        )
        if not continuar:
            mediaurl = ""
...
def set_player(item, xlistitem, mediaurl, view, strm, autoplay):
    ...
    # Si es un fichero strm no hace falta el play
        ...
        logger.info("mediaurl=" + mediaurl)
        if (
            config.get_setting("player_mode", default=0) == 3
            or "megacrypter.com" in mediaurl
        ):
            from . import download_and_play

            download_and_play.download_and_play(
                mediaurl,
                "download_and_play.tmp",
                config.get_setting("downloadpath", default=""),
            )
            return
        ...
        if xbmc.getCondVisibility('System.HasAddon("%s")' % client["id"]):
            ...
            if show_tuple:
                torrent_options.append(
                    [config.get_localized_string(60366) % client["name"], client["url"]]
                )
            else:
                torrent_options.append(
                    config.get_localized_string(60366) % client["name"]
                )
...
def play_torrent(item, xlistitem, mediaurl):
    ...
    from servers.torrent import (
        torrent_dirs,
        update_control,
        shorten_rar_path,
        get_tclient_data,
        call_torrent_via_web,
    )
    ...
    if torrent_client > len(torrent_options):
        if len(torrent_options) > 0:
            if torrent_client > 2:
                torrent_client = torrent_client - 2
            else:
    ...
    if len(torrent_options) > 0 and torrent_client - 1 <= len(torrent_options):
        if torrent_client == 0 and not scrapertools.find_single_match(
            item.downloadFilename, r"^\:(\w+)\:"
        ):
            seleccion = dialog_select(
                config.get_localized_string(70193),
                [opcion[0] for opcion in torrent_options],
            )
            if seleccion < 0:
                return
        elif torrent_client == 0 and scrapertools.find_single_match(
            item.downloadFilename, r"^\:(\w+)\:"
        ):
            t_client_dnl = scrapertools.find_single_match(
                item.downloadFilename, r"^\:(\w+)\:"
            ).upper()
        ...
        if len(torrent_options) > 0 and not scrapertools.find_single_match(
            item.downloadFilename, r"^\:(\w+)\:"
        ):
            seleccion = dialog_select(
                config.get_localized_string(70193),
            ...
            dialog_ok(
                "Instale un gestor de torrents:",
                "[COLOR hotpink]Alfa le recomienda el uso de [B]TORREST[/B][/COLOR]",
            )
            return
        elif scrapertools.find_single_match(item.downloadFilename, r"^\:(\w+)\:"):
            t_client_dnl = scrapertools.find_single_match(
                item.downloadFilename, r"^\:(\w+)\:"
            ).upper()
            for x, t_client in enumerate(torrent_options):
    ...
    TORREST_advise = config.get_setting(
        "torrest_advise", server="torrent", default=False
    )
    ...
    BACKGROUND_DOWNLOAD = config.get_setting(
        "mct_background_download", server="torrent", default=""
    )
    ...
    )  # Obtenemos el path absoluto a partir de Downloads
    videolibrary_path = (
        config.get_videolibrary_path()
    )  # Obtenemos el path absoluto a partir de la Videoteca
    ...
    torr_client = scrapertools.find_single_match(
        torrent_options[seleccion][0], r":\s*(\w+)"
    ).lower()
    ...
    for i, alt_client in enumerate(torrent_options):
        if scrapertools.find_single_match(str(alt_client), r":\s*(\w+)").lower() in [
            "torrest",
            "quasar",
        ]:
            torr_client_alt += [
    ...
    if (
        not TORREST_advise
        and "torrest" not in str(torr_client_alt)
        and torrent_paths.get("ELEMENTUM", "") != "Memory"
    ):
    ...
    if (
        ...
        if dialog_yesno(
            torr_client,
            "Este plugin externo no soporta extraer on-line archivos RAR",
            "[COLOR yellow]¿Quiere que usemos esta vez el gestor [B]%s[/B]?[/COLOR]"
            % torr_client_alt[0][0].upper(),
            "Esta operación ocupará en disco [COLOR yellow][B]%s+[/B][/COLOR] veces el tamaño del vídeo"
            % size_rar,
        ):
    ...
    # Si hay RAR y es Elementum, pero con opción de Memoria, se muestras los Ajustes de Elementum y se pide al usuario que cambie a "Usar Archivos"
        ...
        and UNRAR
        and "RAR-" in item.torrent_info
        and "elementum" in torr_client
        and torrent_paths.get("ELEMENTUM", "") == "Memory"
    ):
        if dialog_yesno(
            torr_client,
            "Elementum con descarga en [COLOR yellow]Memoria[/COLOR] no soporta "
            + "extraer on-line archivos RAR (ocupación en disco [COLOR yellow][B]%s+[/B][/COLOR] veces)"
            % size_rar,
            ...
            if elementum_dl != "1":
                config.set_setting(
                    "elementum_dl", "1", server="torrent"
                )  # Salvamos el cambio para restaurarlo luego
        else:
            item.downloadProgress = 100
    ...
    if seleccion >= 0:
        ...
        try:
            xbmcplugin.setResolvedUrl(
                PLUGIN_HANDLE, False, xlistitem
            )  # Preparamos el entorno para evitar error Kod1 18
        ...
        if item.torrents_path and filetools.exists(item.torrents_path):
            ...
            or item.url.startswith("magnet:")
        ):
            if (
                videolibrary_path not in item.url
                and download_path not in item.url
                and torrent_paths[torr_client.upper() + "_torrents"] not in item.url
            ...
            if not url_local:
                if item.url_control:  # Se mira si es una descarga reiniciada en frio
                    ...
                    item.url = (
                        item.torrent_alt
                    )  # El .torrent alternativo puede estar en una url o en local
                if (
                    (
                        "\\" in item.url
                        or item.url.startswith("/")
                    ...
                    and videolibrary_path not in item.url
                    and download_path not in item.url
                    and torrent_paths[torr_client.upper() + "_torrents"] not in item.url
                    and not item.url.startswith("magnet:")
                ):
        ...
        if scrapertools.find_single_match(
            ...
            )  # Obtenemos el path absoluto a partir de Userdata
        if not filetools.exists(
            videolibrary_path
        ):  # Si no existe el path, pasamos al modo clásico
            videolibrary_path = False
        else:
            torrents_path = filetools.join(
                videolibrary_path, "temp_torrents_Alfa", "cliente_torrent_Alfa.torrent"
            )  # path descarga temporal
        if not videolibrary_path or not filetools.exists(
            filetools.join(videolibrary_path, "temp_torrents_Alfa")
        ...
        if not url_local and not url_stat:
            ...
            if (
                item.module != "downloads"
                and item.sub_action not in ["auto"]
                and item.torrents_path == "CF_BLOCKED"
            ):
            ...
            torrent_params = get_torrent_size(
                ...
                timeout=timeout,
                headers=headers,
                retry_404=0 if item.torrent_alt else 1,
                item=item,
            )
            size = torrent_params["size"]
            url = (
                torrent_params["url"]
                if not filetools.isfile(torrent_params["torrents_path"])
                else torrent_params["torrents_path"]
            )
            # torrent_f = torrent_params["torrent_f"]
            rar_files = torrent_params["files"]
            subtitles_list = torrent_params["subtitles_list"]
            if filetools.isfile(torrent_params["torrents_path"]) and filetools.exists(
            ...
            if subtitles_list:
                log(
                    "##### Subtítulos encontrados en el torrent: %s"
                    % str(subtitles_list)
                )
        ...
        if (
            (not url or "ERROR" in size) and not url_local and item.torrent_alt
        ):  # Si hay error, se busca un .torrent alternativo
            item.url = (
                item.torrent_alt
            )  # El .torrent alternativo puede estar en una url o en local
        ...
        if (
            not url_local
            and videolibrary_path
            and videolibrary_path not in item.url
            and download_path not in item.url
            and torrent_paths[torr_client.upper() + "_torrents"] not in item.url
            and "http" not in item.url
            and not item.url.startswith("magnet:")
        ):
            item.url = filetools.join(videolibrary_path, folder, item.url)
            url_local = filetools.exists(item.url)
        ...
        if (url_local and not url_stat and videolibrary_path) or (
            "ERROR" in size
            and not url_local
            and videolibrary_path
            and item.channel == "videolibrary"
        ):  # .torrent alternativo local
            if not filetools.exists(torrent_params["local_torr"]) and filetools.copy(
                item.url, torrents_path, silent=True
            ):  # se copia a la carpeta generíca para evitar problemas de encode
            ...
            if not item.subtitle:
                ...
                for subt in filetools.listdir(filetools.dirname(item.url)):
                    if subt.endswith(".srt"):
                        subtitles_list_vl += [
                            filetools.join(filetools.dirname(item.url), subt)
                        ]
                        subtitles_list += [
                            filetools.join(filetools.dirname(item.url), subt)
                        ]
            ...
            if url and url != item.url:
                ...
                item.url = url
            if (
                "torrentin" in torrent_options[seleccion][0]
            ):  # Si es Torrentin, hay que añadir un prefijo
                item.url = "file://" + item.url

        if not item.torrent_info:
            item.torrent_info = size
        mediaurl = item.url
    ...
    if (
        (
            "CF_BLOCKED" in size
            or "CF_BLOCKED" in torrent_params["torrents_path"]
            or "CF_BLOCKED" in item.torrents_path
            or "[B]BLOQUEO[/B]" in size
        )
        and item.downloadStatus in [4]
    ...
    if seleccion >= 0:
            ...
            if item.subtitle:
                ...
                if not filetools.exists(filetools.dirname(item.subtitle)):
                    item.subtitle = filetools.join(
                        videolibrary_path, folder, item.subtitle
                    )
        ...
        if torrent_paths.get(torr_client.upper(), ""):  # Es un cliente monitorizable?
                ...
                for entry in rar_files:
                    for file, path in list(entry.items()):
                        if file == "path":
                            ...
                            video_path = path
                item.downloadFilename = filetools.join(
                    ":%s: " % torr_client.upper(), video_path, video_name
                )

            # Si es un Magnet, componemos el path de descarga
            if item.url.startswith("magnet:"):
                t_hash = scrapertools.find_single_match(
                    item.url, r"xt=urn:btih:([^\&]+)\&"
                )
                video_name = torr_folder = scrapertools.find_single_match(
                    item.downloadFilename, r"(?:^\:\w+\:\s*)?[\\\|\/]?(.*?)$"
                )
                if not video_name:
                    video_name = urlparse.unquote_plus(
                        scrapertools.find_single_match(
                            item.url, r"(?:\&|&amp;)dn=([^\&]+)\&"
                        )
                    )
            ...
            if video_path:
                if not filetools.exists(
                    filetools.join(torrent_paths[torr_client.upper()], video_path)
                ) and filetools.exists(
                    filetools.join(torrent_paths[torr_client.upper()], short_video_path)
                ):
                    ...
                    if (
                        rar_control
                        and "downloading" not in rar_control["status"]
                        and rar_control["error"] <= 2
                    ):
                        item.downloadFilename = filetools.join(
                            ":%s: " % torr_client.upper(), short_video_path, video_name
                        )
                        if "path_control" in str(rar_control) and filetools.exists(
                            filetools.join(
                                DOWNLOAD_LIST_PATH, rar_control["path_control"]
                            )
                        ):
                            if item.path and item.path != rar_control["path_control"]:
                                filetools.remove(
                                    filetools.join(DOWNLOAD_LIST_PATH, item.path)
                                )
                            item.path = rar_control["path_control"]
                        update_control(item, function="play_torrent_rar_reintento")
                        try:
                            threading.Thread(
                                target=rar_control_mng,
                                args=(
                                    item,
                                    xlistitem,
                                    mediaurl,
                        ...
                        if torr_client in ["quasar", "elementum", "torrest"]:
                            torr_data, deamon_url, index = get_tclient_data(
                                video_path,
                                torr_client,
                                port=torrent_port,
                                web=torrent_web,
        ...
        if size or item.url.startswith("magnet:"):
            ...
            try:
                ...
                if PY3:
                    ...
                    from lib import alfaresolver
                threading.Thread(
                    target=alfaresolver.frequency_count, args=(item_freq,)
                ).start()
            except Exception:
        ...
        try:
            ...
            if (
                ...
                and torr_client in ["quasar"]
                and config.get_setting("assistant_binary", default=False)
                and not is_alfa_installed()
            ):
                dialog_notification(
                    "Alfa Assistant es requerido",
                    "%s lo requiere en esta versión de Android"
                    % torr_client.capitalize(),
                    time=10000,
                )
                logger.error(
                    "Alfa Assistant es requerido. %s lo requiere en esta versión de Android"
                    % torr_client.capitalize()
                )
            mediaurl = urlparse.quote_plus(item.url)
            ...
            if torr_client in ["quasar", "elementum"] and item.infoLabels["tmdb_id"]:
                if item.contentType == "episode" and "elementum" not in torr_client:
                    mediaurl += (
                        "&episode=%s&library=&season=%s&show=%s&tmdb=%s&type=episode"
                        % (
                    ...
                    mediaurl += (
                        "&library=&tmdb=%s&type=movie" % (item.infoLabels["tmdb_id"])
                    )
            ...
            save_path_videos = str(
                config.translatePath(torrent_paths[torr_client.upper()])
            )

            if (
                torr_client == "quasar" and "cliente_torrent_Alfa" not in item.url
            ):  # Quasar no copia el .torrent
                filetools.copy(
                    item.url,
                    filetools.join(
                        save_path_videos, "torrents", filetools.basename(item.url)
                    ),
                    silent=True,
                )

            if (
                torr_client in ["quasar", "elementum", "torrest"]
                and item.downloadFilename
                and (
                    item.downloadStatus not in [5]
                    or item.downloadProgress == -1
                    or (
                        item.url.startswith("magnet:")
                        and torr_client not in ["elementum"]
                    )
                )
            ) or (
                torr_client in ["quasar", "elementum", "torrest"]
                and ("RAR-" in size or "RAR-" in item.torrent_info)
                and BACKGROUND_DOWNLOAD
            ):
                if item.downloadProgress == -1:  # Si estaba pausado se resume
                    torr_folder = scrapertools.find_single_match(
                        item.downloadFilename, r"(?:^\:\w+\:\s*)?[\\\|\/]?(.*?)$"
                    )
                    ...
                    downloadProgress = -1
                    result, deamon_url, index = get_tclient_data(
                        torr_folder,
                        torr_client,
                        port=torrent_port,
                        web=torrent_web,
                ...
                if not result:  # Si es nuevo, o hay error en resume, se añade
                    result = call_torrent_via_web(
                        urlparse.quote_plus(item.url),
                        torr_client,
            ...
            if not result:  # Si falla todo, se usa el antiguo sistema
                if torr_client == "torrest":
                    ...
                    xbmc.executebuiltin(
                        "PlayMedia("
                        + torrent_options[seleccion][1]
                        % (play_type, play_type, mediaurl)
                        + ")"
                    )
                else:
                    xbmc.executebuiltin(
                        "PlayMedia(" + torrent_options[seleccion][1] % mediaurl + ")"
                    )
            update_control(item, function="play_torrent_externos_start")
            if item.downloadStatus not in [3, 4, 5]:
                itemlist_refresh()
            ...
            if downloadProgress != -1:
                try:
                    ...
                    ).start()  # Creamos un Thread independiente por .torrent
                    time.sleep(3)  # Dejamos terminar la inicialización...
                except Exception:  # Si hay problemas de threading, salimos
                    logger.error(traceback.format_exc())
            ...
            for entry in rar_files:
                for file, path in list(entry.items()):
                    if file == "__name":
                        rar_path = path
                        rar_path = filetools.join(
                            torrent_paths[torr_client.upper()], rar_path
                        )
                        for x in range(60):
            ...
            rar_path_folder = (
                rar_path if filetools.isdir(rar_path) else filetools.dirname(rar_path)
            )
            if subtitles_list and rar_path_folder:
                ...
                for subtitle in subtitles_list:
                    if filetools.exists(subtitle):
                        filetools.copy(
                            subtitle,
                            filetools.join(
                                rar_path_folder, filetools.basename(subtitle)
                            ),
                            silent=True,
                        )
                ...
                filetools.mkdir(rar_path_folder)
                dest_file = filetools.join(
                    rar_path_folder, filetools.basename(item.subtitle)
                )
            ...
            logger.error(traceback.format_exc())
            dialog_ok(
                "Error descargando .torrent",
                line1="Inténtelo de nuevo más tarde ... ",
                line2="[COLOR yellow][B]%s[/B][/COLOR]" % str(e),
            )


def rar_control_mng(
    item, xlistitem, mediaurl, rar_files, torr_client, password, size, rar_control={}
):
    ...
    try:
        ...
        if torrent_paths[torr_client.upper()] != "Memory":
            rar_file, save_path_videos, torr_folder, rar_control = wait_for_download(
                item,
                xlistitem,
                mediaurl,
                rar_files,
                ...
                size,
                rar_control,
            )  # Esperamos mientras se descarga el TORRENT
        else:
        ...
        if (
            "RAR-" in size and UNRAR and RAR_UNPACK and rar_file and save_path_videos
        ):  # Si se ha descargado RAR...
            dp = dialog_progress_bg("Alfa %s" % torr_client)
            video_file, rar, video_path, erase_file_path = extract_files(
                rar_file,
                save_path_videos,
                password,
                dp,
            ...
            while (
                is_playing()
                and rar
                and (not item.downloadFilename or item.downloadStatus == 5)
            ):
            ...
            if video_path:
                ...
                item.downloadFilename = filetools.join(
                    item.downloadFilename, video_file
                )
                item.downloadFilename = ":%s: %s" % (
                    torr_client.upper(),
        ...
        path = filetools.join(
            config.get_setting("downloadlistpath", default=""), item.path
        )
        if filetools.exists(path):
            ...
            path = ""
        if not save_path_videos and item_down.downloadStatus > 0:
            item_down.downloadStatus = 3  # Descarga en error
        ...
        if item_down.downloadProgress == -1:
            ...
            ]:  # Es un cliente monitorizable?
                if path:
                    item.downloadProgress = (
                        item_down.downloadProgress
                    )  # Si se ha borrado desde downloads, prevalece su status
                    item.downloadStatus = (
                        item_down.downloadStatus
                    )  # Si se ha borrado desde downloads, prevalece su status
                else:
                    item.downloadProgress = (
                        1  # lo dejamos preparado para el reinicio, o borrado auto
                    )
            else:
                item.downloadProgress = (
                    100  # ... si no, se da por terminada la monitorización
                )
        ...
        if not item.downloadFilename or item.downloadStatus == 5:
            ...
            if rar:
                if dialog_yesno(
                    "Alfa %s" % torr_client, "¿Borrar las descargas del RAR y Vídeo?"
                ):
        ...
        elementum_dl = config.get_setting(
            "elementum_dl", server="torrent", default=""
        )  # Si salvamos el cambio de Elementum
        if elementum_dl:
            config.set_setting(
                "elementum_dl", "", server="torrent"
            )  # lo reseteamos en Alfa
            xbmcaddon.Addon(id="plugin.video.%s" % torr_client).setSetting(
                "download_storage", elementum_dl
            )  # y lo reseteamos en Elementum
