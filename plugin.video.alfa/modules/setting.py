...
def mainlist(item):
    ...
    itemlist.append(
        Item(
            module=MODULE_NAME,
            title=config.get_localized_string(60535),
            action="settings",
            folder=False,
    ...
    itemlist.append(
        Item(
            module=MODULE_NAME,
            title=config.get_localized_string(60538),
            action="menu_servers",
            folder=True,
    ...
    if config.is_xbmc():
        itemlist.append(
            Item(
                module=MODULE_NAME,
                title=config.get_localized_string(70253),
    ...
    itemlist.append(
        Item(
            module=MODULE_NAME,
            title=config.get_localized_string(60544),
...
def menu_channels(item):
    ...
    itemlist.append(
        Item(
            module=MODULE_NAME,
            title=config.get_localized_string(60545),
            action="conf_tools",
            folder=False,
    ...
    itemlist.append(
        Item(
            module=MODULE_NAME,
            title=config.get_localized_string(60546) + ":",
            action="",
            folder=False,
            text_bold=True,
            thumbnail=get_thumb("setting_0.png"),
    ...
    for channel in channel_list:
        ...
        if channel_parameters["has_settings"]:
            itemlist.append(
                Item(
                    module=MODULE_NAME,
                    title=".    " + config.get_localized_string(60547) % channel.title,
    ...
    itemlist.append(
        Item(
            module=MODULE_NAME,
            title=config.get_localized_string(60548) + ":",
            action="",
            folder=False,
            text_bold=True,
            thumbnail=get_thumb("channels.png"),
    ...
    itemlist.append(
        Item(
            module=MODULE_NAME,
            title=".    " + config.get_localized_string(60549),
            action="conf_tools",
            folder=True,
...
def channel_config(item):
    return platformtools.show_channel_settings(
        channelpath=filetools.join(config.get_runtime_path(), "channels", item.config)
    )
...
def setting_torrent(item):
    ...
    BACKGROUND = config.get_setting(
        "mct_background_download", server="torrent", default=True
    )
    RAR = config.get_setting("mct_rar_unpack", server="torrent", default=True)
    MAGNET2TORRENT = config.get_setting(
        "magnet2torrent", server="torrent", default=False
    )
    CAPTURE_THRU_ROWSER_PATH = config.get_setting(
        "capture_thru_browser_path", server="torrent", default=""
    )
    ...
    if default > len(torrent_options) - 1:
        if len(torrent_options) - 1 > 0:
            if default > 2:
                ...
                default = len(torrent_options)
            if default > len(torrent_options):
                default = len(torrent_options)
    ...
    if item.item_org:
        ...
        if item_org.torrent_info:
            del item_org.torrent_info
...
def save_setting_torrent(item, dict_data_saved):
    if dict_data_saved and "list_torrent" in dict_data_saved:
        config.set_setting(
            "torrent_client", dict_data_saved["list_torrent"], server="torrent"
        )
    if dict_data_saved and "mct_background_download" in dict_data_saved:
        config.set_setting(
            "mct_background_download",
            dict_data_saved["mct_background_download"],
            server="torrent",
        )
    if dict_data_saved and "mct_rar_unpack" in dict_data_saved:
        config.set_setting(
            "mct_rar_unpack", dict_data_saved["mct_rar_unpack"], server="torrent"
        )
    if dict_data_saved and "magnet2torrent" in dict_data_saved:
        config.set_setting(
            "magnet2torrent", dict_data_saved["magnet2torrent"], server="torrent"
        )
    if dict_data_saved and "capture_thru_browser_path" in dict_data_saved:
        config.set_setting(
            "capture_thru_browser_path",
            dict_data_saved["capture_thru_browser_path"],
            server="torrent",
        )
    if dict_data_saved and "allow_seeding" in dict_data_saved:
        config.set_setting(
            "allow_seeding", dict_data_saved["allow_seeding"], server="torrent"
        )
...
def menu_servers(item):
    ...
    itemlist.append(
        Item(
            module=MODULE_NAME,
            title=config.get_localized_string(60550),
    ...
    itemlist.append(
        Item(
            module=MODULE_NAME,
            title=config.get_localized_string(60551),
    ...
    itemlist.append(
        Item(
            module=MODULE_NAME,
            title=config.get_localized_string(60552),
            action="",
            folder=False,
            text_bold=True,
            thumbnail=get_thumb("setting_0.png"),
        ...
        if server_parameters["has_settings"]:
            itemlist.append(
                Item(
                    module=MODULE_NAME,
                    title=".    "
                    + config.get_localized_string(60553) % server_parameters["name"],
    ...
    itemlist.append(
        Item(
            module=MODULE_NAME,
            title=config.get_localized_string(60554),
            action="",
            folder=False,
            text_bold=True,
            thumbnail=get_thumb("setting_0.png"),
    ...
    for server in sorted(server_list):
        ...
        if server_parameters["has_settings"] and [
            x
            for x in server_parameters["settings"]
...
def server_config(item):
    return platformtools.show_channel_settings(
        channelpath=filetools.join(config.get_runtime_path(), "servers", item.config)
    )
...
def servers_blacklist(item):
    ...
    if dict_values["filter_servers"] is None:
...
def servers_favorites(item):
    ...
    if dict_values["favorites_servers"] is None:
...
def submenu_tools(item):
    ...
    itemlist.append(
        Item(
            ...
            title="Comprobar actualizaciones urgentes (Actual: Alfa {})".format(
...
def conf_tools(item):
    ...
    if item.extra == "channels_onoff":
        if (
            config.get_platform(True)["num_version"] >= 17.0
        ):  # A partir de Kodi 16 se puede usar multiselect, y de 17 con preselect
            ...
            for channel in channel_list:
                ...
                if channel.channel not in excluded_channels:
                    ...
                    if status is None:
                        ...
                        logger.debug(
                            "%s | Status (XML): %s" % (channel.channel, status)
                        )
                        if not status:
        ...
        try:
            ...
            for channel in channel_list:
                ...
                if channel.channel and channel.channel not in excluded_channels:
                    ...
                    list_controls, dict_settings = (
                        channeltools.get_channel_controls_settings(channel.channel)
                    )
                    ...
                    if (
                        len(dict_settings) and datajson_size
                    ) == 0 or not channeljson_exists:
                        # Obtenemos controles del archivo ../channels/channel.json
                        needsfix = True
                        try:
                            # Se cargan los ajustes por defecto
                            list_controls, default_settings = (
                                channeltools.get_channel_controls_settings(
                                    channel.channel
                                )
                            )
                        ...
                        if needsfix or not channeljson_exists:
                            if default_settings is not None:
                                ...
                                if not filetools.write(
                                    file_settings,
                                    jsontools.dump(dict_file),
                                    silent=True,
                                ):
                                    logger.error(
                                        "ERROR al salvar el archivo: %s" % file_settings
                                    )
...
def channels_onoff(item):
    ...
    ret = platformtools.dialog_select(
        config.get_localized_string(60545), preselecciones
    )
    if ret == -1:
    ...
    # ------------------------
    ret = platformtools.dialog_multiselect(
        config.get_localized_string(60545), lista, preselect=preselect, useDetails=True
    )
    if ret is None:
        return False  # pedido cancel
...
def channel_status(item, dict_values):
    try:
        for k in dict_values:
            if k == "all_channels":
                logger.info(
                    "Todos los canales | Estado seleccionado: %s" % dict_values[k]
                )
                if dict_values[k] != 0:
                    excluded_channels = [
                        "url",
                    ...
                    for channel in item.channel_list:
                        if channel.channel not in excluded_channels:
                            ...
                            if new_status_all is not None:
                                config.set_setting(
                                    "enabled", new_status_all, channel.channel
                                )
                    break
                else:
                    continue
        ...
        platformtools.dialog_notification(
            config.get_localized_string(60579), config.get_localized_string(60580)
        )
...
def overwrite_tools(item):
    ...
    seleccion = platformtools.dialog_yesno(
        config.get_localized_string(60581),
...
def icon_set_selector(item=None):
    ...
    data = httptools.downloadpage(
        "https://github.com/alfa-addon/media/tree/master/themes"
    ).data
    ...
    for set_id in matches:
        logger.info(set_id)
        path_demo = (
            "https://raw.githubusercontent.com/alfa-addon/media/master/themes/%s/thumb_channels_movie.png"
            % set_id
        )
        path_info = (
            "https://raw.githubusercontent.com/alfa-addon/media/master/themes/%s/README.md"
            % set_id
        )
        opt = Item(
    ...
    ret = platformtools.dialog_select(
        "Selecciona un Set de iconos", options, useDetails=True
    )
    if ret != -1:
        if ret == 0:
...
def reset_trakt(item):
    ...
    data_path = filetools.join(
        config.get_data_path(), "settings_channels", "trakt_data.json"
    )
