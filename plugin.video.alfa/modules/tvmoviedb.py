...
perfil = [
    ["0xFFFFE6CC", "0xFFFFCE9C", "0xFF994D00", "0xFFFE2E2E", "lime", "0xFFFFD700"],
    ["0xFFA5F6AF", "0xFF5FDA6D", "0xFF11811E", "0xFFFE2E2E", "lime", "0xFFFFD700"],
    ["0xFF58D3F7", "0xFF2E9AFE", "0xFF2E64FE", "0xFFFE2E2E", "lime", "0xFFFFD700"],
]
if __perfil__ < 3:
    color1, color2, color3, color4, color5, color6 = perfil[__perfil__]
else:
    color1 = color2 = color3 = color4 = color5 = color6 = ""
plot_extend = "[COLOR grey](%s)[/COLOR]"
...
images_predef = (
    "https://raw.githubusercontent.com/master-1970/resources/master/images/genres/"
)
default_fan = filetools.join(config.get_runtime_path(), "fanart1_2.jpg")
unify_config = config.get_setting("unify", default=True)
unify = False
year = datetime.now().year
country = bdec(config.get_setting("proxy_zip", default="")).decode("utf-8")
country = scrapertools.find_single_match(country, "Country:\s*(\w+)") or langi

host_tmdb = "https://api.themoviedb.org/"
host_filmaffinity = "https://www.filmaffinity.com/"
host_imdb = "https://www.imdb.com/"
host_trakt = "https://api-v2launch.trakt.tv/"
host_mal = "https://myanimelist.net/"
host_theost = "https://theost.com/"
...
def mainlist(item):
    ...
    itemlist.append(
        item.clone(
            title=config.get_localized_string(70024), action="", text_color=color2
        )
    )
    itemlist.append(
        item.clone(
            title=config.get_localized_string(70022),
            action="filmaf",
            extra="movie",
            thumbnail="%s0/Movies.png" % images_predef,
        )
    ...
    itemlist.append(
        item.clone(
            title=config.get_localized_string(70023),
            action="filmaf",
            extra="tv",
            thumbnail=images_predef + "0/TV%20Series.png",
        )
    ...
    itemlist.append(
        item.clone(
            title=config.get_localized_string(70025), action="", text_color=color2
        )
    )
    itemlist.append(
        item.clone(
    ...
    itemlist.append(
        item.clone(
            title=config.get_localized_string(70415),
            action="trakt",
            text_color=color2,
            thumbnail="https://i.imgur.com/5sQjjuk.png",
        )
    )
    # itemlist.append(
    #    item.clone(title=config.get_localized_string(70026), action="mal", text_color=color2, thumbnail="https://i.imgur.com/RhsYWmd.png"))
    itemlist.append(
        item.clone(
            title=config.get_localized_string(70036),
            action="search_",
            url=urlparse.urljoin(host_imdb, "search/name/?start=1&name="),
            text_color=color4,
        )
    )
    ...
    itemlist.append(
        item.clone(
            title=config.get_localized_string(70027),
...
def search_(item):
    ...
    if texto:
        ...
        if "myanimelist" in item.url:
            ...
            item.url += (
                "&type=0&score=0&status=0&p=0&r=0&sm=0&sd=0&sy=0&em=0&ed=0&ey=0&c[0]=a"
...
def busqueda(item):
    ...
    new_item = Item(
        ...
        text=item.contentTitle.replace("+", " "),
...
def tmdb(item):
    ...
    itemlist.append(
        item.clone(
            title=config.get_localized_string(70028),
            action="listado_tmdb",
    ...
    itemlist.append(
        item.clone(
            title=config.get_localized_string(70029),
            action="listado_tmdb",
    ...
    if item.extra == "movie":
        itemlist.append(
            item.clone(
                title=config.get_localized_string(70030),
                action="listado_tmdb",
                search={
                    "url": item.extra + "/now_playing",
                    "language": langt,
                    "page": 1,
                },
            )
        ...
        itemlist.append(
            item.clone(
                title=config.get_localized_string(70031),
                action="listado_tmdb",
                search={
                    "url": item.extra + "/on_the_air",
                    "language": langt,
                    "page": 1,
                },
            )
    ...
    itemlist.append(
        item.clone(
            title=config.get_localized_string(70032),
            action="indices_tmdb",
    ...
    itemlist.append(
        item.clone(
            title=config.get_localized_string(70042),
            action="indices_tmdb",
            thumbnail="%s0/Year.png" % images_predef,
        )
    )
    ...
    if item.extra == "movie":
        itemlist.append(
            item.clone(
                title=config.get_localized_string(70033),
                action="listado_tmdb",
        ...
        itemlist.append(
            item.clone(
                title=config.get_localized_string(70034),
                action="listado_tmdb",
    ...
    if config.get_platform() != "plex":
        title = item.contentType.replace(
            "movie", config.get_localized_string(70283)
        ).replace("tvshow", "serie")
        itemlist.append(
            item.clone(
                title=config.get_localized_string(70035) % title,
                action="search_",
        ...
        if item.extra == "movie":
            itemlist.append(
                item.clone(
                    title=config.get_localized_string(70037),
                    action="search_",
                    search={"url": "search/person", "language": langt, "page": 1},
                    crew=True,
                )
    ...
    itemlist.append(
        item.clone(
            title=config.get_localized_string(70038), action="filtro", text_color=color4
        )
    )
    itemlist.append(
        item.clone(
            title=config.get_localized_string(70039), action="filtro", text_color=color4
        )
    )
...
def imdb(item):
    ...
    itemlist.append(
        item.clone(title=config.get_localized_string(70028), action="listado_imdb")
    )
    itemlist.append(
        item.clone(
            title=config.get_localized_string(70029),
        ...
        itemlist.append(
            item.clone(
                ...
                url=urlparse.urljoin(
                    host_imdb, "showtimes/location?ref_=inth_ov_sh_sm"
                ),
            )
        )
    itemlist.append(
    ...
    if item.extra == "movie":
        itemlist.append(
            item.clone(
                ...
                action="listado_imdb",
                url=urlparse.urljoin(
                    host_imdb, "calendar/?ref_=rlm&region=%s&type=MOVIE"
                )
                % country.upper(),
            )
        )
    ...
    itemlist.append(
        item.clone(
            ...
            action="listado_imdb",
            url=urlparse.urljoin(
                host_imdb, "search/name/?gender=male,female&ref_=nv_cel_m_3&start=1"
            ),
            text_color=color4,
        )
    )
    ...
    if config.get_platform() != "plex":
        ...
        itemlist.append(
            item.clone(
                ...
                action="search_",
                url=urlparse.urljoin(host_imdb, "search/title/?%s&title=" % item.url),
                text_color=color4,
            )
        )
    ...
    itemlist.append(
        item.clone(
            title=config.get_localized_string(70038),
            action="filtro_imdb",
            text_color=color4,
        )
    )
...
def filmaf(item):
    ...
    if item.extra == "movie":
        itemlist.append(
            item.clone(
                title=config.get_localized_string(70040),
                action="listado_fa",
                extra="top",
                url=urlparse.urljoin(
                    host_filmaffinity,
                    "%s/topgen.php?genres=&chv=0&orderby=avg&"
                    "movietype=movie&country=&" % langf,
                ),
            )
        )
        itemlist.append(
            item.clone(
                ...
                extra="cartelera",
                url=urlparse.urljoin(
                    host_filmaffinity, "%s/rdcat.php?id=new_th_%s" % (langf, langf)
                ),
            )
        )
        itemlist.append(
            item.clone(
                ...
                url=urlparse.urljoin(
                    host_filmaffinity,
                    "%s/topgen.php?chv=0&orderby=avg&"
                    "movietype=movie&country=&" % langf,
                ),
                thumbnail="%s0/Genres.png" % images_predef,
            )
        ...
        itemlist.append(
            item.clone(
                ...
                url=urlparse.urljoin(
                    host_filmaffinity,
                    "%s/topgen.php?genres=&country=&orderby=avg&"
                    "nodoc&movietype=serie" % langf,
                ),
            )
        )
        itemlist.append(
            item.clone(
                title="Series de actualidad",
                action="listado_fa",
                extra="series_act",
                url=urlparse.urljoin(
                    host_filmaffinity,
                    "%s/topgen.php?genres=-DO&chv=1&orderby=avg&"
                    "movietype=serie&country=&fromyear=%s&toyear=%s"
                    % (langf, year - 1, year),
                ),
            )
        )
    ...
    if item.extra == "movie":
        itemlist.append(
            item.clone(
                title=config.get_localized_string(70043),
                action="listado_fa",
                extra="estrenos",
                url=urlparse.urljoin(
                    host_filmaffinity, "%s/rdcat.php?id=upc_th_%s" % (langf, langf)
                ),
            )
        )
        itemlist.append(
            item.clone(
                title=config.get_localized_string(70044),
                action="indices_fa",
                extra="sagas",
                url=urlparse.urljoin(
                    host_filmaffinity, "%s/movie-groups-all.php" % langf
                ),
            )
        )
    itemlist.append(
        item.clone(
            title=config.get_localized_string(70045),
            action="indices_fa",
            extra="topics",
            url=urlparse.urljoin(host_filmaffinity, "%s/topics.php" % langf),
            text_color=color3,
        )
    )
    if config.get_platform() != "plex":
        itemlist.append(
            item.clone(
                title=config.get_localized_string(70046),
                action="search_",
                text_color=color4,
                extra="search",
                url=urlparse.urljoin(
                    host_filmaffinity,
                    "%s/search.php?stype=title&from=0&orderby=relevance&stext=" % langf,
                ),
            )
        )
        ...
        itemlist.append(
            item.clone(
                ...
                url=urlparse.urljoin(
                    host_filmaffinity,
                    "%s/search.php?stype=cast&from=0&orderby=relevance&stext=" % langf,
                ),
            )
        )
        itemlist.append(
            item.clone(
                title=config.get_localized_string(70047),
                action="search_",
                text_color=color4,
                extra="search",
                url=urlparse.urljoin(
                    host_filmaffinity,
                    "%s/search.php?stype=director&from=0&orderby=relevance&stext="
                    % langf,
                ),
            )
        )
    ...
    itemlist.append(
        item.clone(
            title=config.get_localized_string(70048),
            action="cuenta_fa",
            text_color=color3,
        )
    )
...
def trakt(item):
    ...
    if not item.extra:
        ...
        itemlist.append(
            item.clone(
                title="[COLOR yellow][B]%s[/B][/COLOR]"
                % config.get_localized_string(70416),
                action="",
            )
        )
        itemlist.append(
            item.clone(
                title=config.get_localized_string(70049),
                action="acciones_trakt",
                url="movies/popular%s" % page,
            )
        )
        itemlist.append(
            item.clone(
                title=config.get_localized_string(70050),
                action="acciones_trakt",
                url="movies/trending%s" % page,
            )
        )
        itemlist.append(
            item.clone(
                title=config.get_localized_string(70053),
                action="acciones_trakt",
                url="movies/watched/all%s" % page,
            )
        )
        itemlist.append(
            item.clone(
                title=config.get_localized_string(70051),
                action="acciones_trakt",
                url="movies/anticipated%s" % page,
            )
        )
        if token_auth:
            itemlist.append(
                item.clone(
                    title=config.get_localized_string(70052),
                    action="acciones_trakt",
                    url="recommendations/movies?limit=100&extended=full",
                    pagina=0,
                )
            )
        itemlist.append(
            item.clone(
                title="[COLOR yellow][B]%s[/B][/COLOR]"
                % config.get_localized_string(70417),
                action="",
            )
        )
        item.extra = "show"
        ...
        if token_auth:
            itemlist.append(
                item.clone(
                    title=config.get_localized_string(70052),
                    action="acciones_trakt",
                    url="recommendations/shows?limit=100&extended=full",
                    pagina=0,
                )
            )
        itemlist.append(
            item.clone(
                title="[COLOR red][B]%s[/B][/COLOR]"
                % config.get_localized_string(70048),
                extra="cuenta",
            )
        )
    else:
        ...
        if not token_auth:
            ...
            itemlist.append(
                item.clone(
                    title=config.get_localized_string(70054),
                    action="auth_trakt",
                    folder=folder,
                )
            )
            ...
            itemlist.append(
                item.clone(
                    title=config.get_localized_string(70055),
                    action="",
                    text_color=color2,
                )
            )
            itemlist.append(
                item.clone(
                    title=config.get_localized_string(60651),
                    action="acciones_trakt",
                    url="users/me/watchlist/movies%s" % page,
                    order="added",
                    how="desc",
                )
            )
            itemlist.append(
                item.clone(
                    title=config.get_localized_string(60652),
                    action="acciones_trakt",
                    url="users/me/watchlist/shows%s" % page,
                    extra="show",
                    order="added",
                    how="desc",
                )
            ...
            itemlist.append(
                item.clone(
                    title=config.get_localized_string(70056),
                    action="",
                    text_color=color2,
                )
            )
            itemlist.append(
                item.clone(
                    title=config.get_localized_string(60651),
                    action="acciones_trakt",
                    url="users/me/watched/movies%s" % page,
                    order="added",
                    how="desc",
                )
            )
            itemlist.append(
                item.clone(
                    title=config.get_localized_string(60652),
                    action="acciones_trakt",
                    url="users/me/watched/shows%s" % page,
                    extra="show",
                    order="added",
                    how="desc",
                )
            ...
            itemlist.append(
                item.clone(
                    title=config.get_localized_string(70068),
                    action="",
                    text_color=color2,
                )
            )
            itemlist.append(
                item.clone(
                    title=config.get_localized_string(60651),
                    action="acciones_trakt",
                    url="users/me/collection/movies%s" % page,
                    order="added",
                    how="desc",
                )
            )
            itemlist.append(
                item.clone(
                    title=config.get_localized_string(60652),
                    action="acciones_trakt",
                    url="users/me/collection/shows%s" % page,
                    extra="show",
                    order="added",
                    how="desc",
                )
            ...
            itemlist.append(
                item.clone(
                    title=config.get_localized_string(70057),
                    action="acciones_trakt",
                    url="users/me/lists",
                    text_color=color2,
                )
...
def mal(item):
    ...
    itemlist.append(
        item.clone(
            title=config.get_localized_string(70058),
            url=urlparse.urljoin(host_mal, "topanime.php?type=tv&limit=0"),
            action="top_mal",
            contentType="tvshow",
            extra="tv",
        )
    ...
    itemlist.append(
        item.clone(
            title=config.get_localized_string(70059),
            url=urlparse.urljoin(host_mal, "topanime.php?type=movie&limit=0"),
            action="top_mal",
            contentType="movie",
            extra="movie",
        )
    ...
    itemlist.append(
        item.clone(
            title=config.get_localized_string(70061),
            url=urlparse.urljoin(host_mal, "topanime.php?type=ova&limit=0"),
            action="top_mal",
            contentType="tvshow",
            extra="tv",
            tipo="ova",
    ...
    itemlist.append(
        item.clone(
            title=config.get_localized_string(70028),
            url=urlparse.urljoin(host_mal, "topanime.php?type=bypopularity&limit=0"),
            action="top_mal",
        )
    )
    itemlist.append(
        item.clone(
            title=config.get_localized_string(70060),
            url=urlparse.urljoin(host_mal, "topanime.php?type=upcoming&limit=0"),
            action="top_mal",
        )
    )
    itemlist.append(
        item.clone(
            title=config.get_localized_string(70062), url="", action="indices_mal"
        )
    )
    itemlist.append(
        item.clone(
            title=config.get_localized_string(70063), url="", action="indices_mal"
        )
    )
    if config.get_platform() != "plex":
        itemlist.append(
            item.clone(
                title=config.get_localized_string(70064),
                url=urlparse.urljoin(host_mal, "anime.php?q="),
                action="search_",
            )
        )
    itemlist.append(
    ...
    if "nm" in item.infoLabels["imdb_id"]:
        try:
            ...
            if item.contentType == "movie":
                ...
                item.search = {
                    "url": "person/%s/tv_credits" % id_cast,
                    "language": langt,
                }
            ob_tmdb = Tmdb(discover=item.search, tipo=item.extra, idioma_busqueda=langt)
        except Exception:
    ...
    if "collection" in item.search["url"]:
        try:
            ...
            for parte in ob_tmdb.result["parts"]:
                new_item = item.clone(action="detalles", unify=unify_config)
                new_item.infoLabels = ob_tmdb.get_infoLabels(
                    new_item.infoLabels, origen=parte
                )
        ...
        try:
            ...
            for i in range(0, len(ob_tmdb.results)):
                ...
                new_item.infoLabels = ob_tmdb.get_infoLabels(
                    new_item.infoLabels, origen=ob_tmdb.results[i]
                )
                # Si no hay sinopsis en idioma elegido, buscar en el alternativo
                if (
                    not new_item.infoLabels["plot"]
                    and "person" not in item.search["url"]
                ):
                    ob_tmdb2 = Tmdb(
                        id_Tmdb=new_item.infoLabels["tmdb_id"],
                        tipo=item.extra,
                        idioma_busqueda=langt_alt,
                    )
                ...
                if new_item.infoLabels["thumbnail"]:
                    ...
                    if not item.search.get("with_cast", "") and not item.search.get(
                        "with_crew", ""
                    ):
                        if item.contentType == "movie":
                            ...
                            new_item.search = {
                                "url": "discover/movie",
                                cast: new_item.infoLabels["tmdb_id"],
                                "sort_by": "primary_release_date.desc",
                                "language": langt,
                                "page": item.pagina,
                            }
                            ...
                            new_item.action = "listado_tmdb"
                            new_item.search = {
                                "url": "person/%s/tv_credits"
                                % new_item.infoLabels["tmdb_id"],
                                "language": langt,
                            }

                elif (
                    not new_item.infoLabels["thumbnail"]
                    and not new_item.infoLabels["profile_path"]
                ):
                    new_item.thumbnail = ""
                if new_item.infoLabels["fanart"]:
                    new_item.fanart = new_item.infoLabels["fanart"]
                ...
                if (
                    "person" not in item.search["url"]
                    or "tv_credits" in item.search["url"]
                ):
                    if new_item.infoLabels["year"]:
                        new_item.title = "%s  (%s) [COLOR %s]%s[/COLOR]" % (
                            new_item.contentTitle,
                    ...
                    if known_for:
                        ...
                        new_item.title = "%s  [COLOR %s](%s)[/COLOR] (%s)" % (
                            new_item.contentTitle,
                            color6,
                            known_for[random].get(
                                "title", known_for[random].get("name")
                            ),
                            type,
                        )
                        if known_for[random]["backdrop_path"]:
                            new_item.fanart = (
                                "https://image.tmdb.org/t/p/original"
                                + known_for[random]["backdrop_path"]
                            )
                    else:
        ...
        if "page" in item.search and ob_tmdb.total_pages > item.search["page"]:
            ...
            itemlist.append(
                Item(
                    ...
                    action=item.action,
                    title=config.get_localized_string(70065),
                    search=item.search,
                    extra=item.extra,
...
def detalles(item):
    ...
    if not item.infoLabels["tmdb_id"]:
        ...
        pics = scrapertools.find_single_match(
            data, 'showAllVidsAndPics.*?href=".*?(tt\d+)'
        )
        # Imágenes imdb
        if pics:
            images["imdb"] = {
                "url": urlparse.urljoin(host_imdb, "_json/title/%s/mediaviewer" % pics)
            }

        ob_tmdb = Tmdb(
            external_id=item.infoLabels["imdb_id"],
    ...
    ob_tmdb = Tmdb(
        id_Tmdb=item.infoLabels["tmdb_id"], tipo=item.extra, idioma_busqueda=langt
    )
    ...
    if data:
        plot = scrapertools.find_single_match(
            data, 'class="inline canwrap" itemprop="description">(.*?)</div>'
        )
    ...
    if item.infoLabels["tagline"]:
        itemlist.append(
            item.clone(
                title="--- %s ---" % item.infoLabels["tagline"],
                text_color="0xFFFF8C00",
                action="",
            )
        )
    ...
    itemlist.append(
        item.clone(
            action="busqueda",
            title=config.get_localized_string(70069) % (title, item.contentTitle),
            unify=unify_config,
        )
    )
    if (
        item.infoLabels["originaltitle"]
        and item.contentTitle != item.infoLabels["originaltitle"]
    ):
        itemlist.append(
            item.clone(
                action="busqueda",
                contentTitle=item.infoLabels["originaltitle"],
                title=config.get_localized_string(70070)
                % item.infoLabels["originaltitle"],
                unify=unify_config,
            )
        )
    ...
    if langt != "es" and langt != "en" and item.infoLabels["tmdb_id"]:
        tmdb_lang = Tmdb(
            id_Tmdb=item.infoLabels["tmdb_id"], tipo=item.extra, idioma_busqueda="es"
        )
        if (
            tmdb_lang.result.get("title")
            and tmdb_lang.result["title"] != item.contentTitle
            ...
            itemlist.append(
                item.clone(
                    action="busqueda",
                    title=config.get_localized_string(70066) % tmdb_lang,
    ...
    if item.contentType == "tvshow" and item.infoLabels["tmdb_id"]:
        itemlist.append(
            item.clone(
                action="info_seasons",
                text_color=color4,
                title=config.get_localized_string(70067)
    ...
    if item.infoLabels["tmdb_id"]:
        itemlist.append(
            item.clone(
                action="reparto",
                title=config.get_localized_string(70071),
                text_color=color4,
                infoLabels={
    ...
    itemlist.append(
        item.clone(
            ...
            title=config.get_localized_string(60359),
            text_color=color5,
        )
    )
    ...
    try:
        ...
        itemlist.append(
            item.clone(
                action="imagenes",
                title=config.get_localized_string(70316),
                text_color=color5,
                images=images,
                extra="menu",
            )
    ...
    try:
        if item.contentType == "movie" and item.infoLabels["year"] < 2014:
            url = urlparse.urljoin(
                host_theost,
                "search/custom/?key=%s&year=%s&country=0&genre=0"
                % (
                    item.infoLabels["originaltitle"].replace(" ", "+"),
                    item.infoLabels["year"],
                ),
            )

            while True:
                response = httptools.downloadpage(url, follow_redirects=False, **kwargs)
            ...
            url_album = scrapertools.find_single_match(
                data_music, 'album(?:s|) on request.*?href="([^"]+)"'
            )
            if url_album:
                url_album = urlparse.urljoin(host_theost, url_album)
                itemlist.append(
                    item.clone(
                        action="musica_movie",
                        title=config.get_localized_string(70317),
                        url=url_album,
                        text_color=color5,
                    )
                )
    ...
    if token_auth:
        itemlist.append(
            item.clone(title=config.get_localized_string(70318), action="menu_trakt")
        )
    ...
    try:
        if ob_tmdb.result.get("belongs_to_collection"):
            ...
            if saga["poster_path"]:
                new_item.thumbnail = (
                    "https://image.tmdb.org/t/p/original" + saga["poster_path"]
                )
            if saga["backdrop_path"]:
                new_item.fanart = (
                    "https://image.tmdb.org/t/p/original" + saga["backdrop_path"]
                )
            new_item.search = {"url": "collection/%s" % saga["id"], "language": langt}
            itemlist.append(
                new_item.clone(
                    title=config.get_localized_string(70327) % saga["name"],
                    action="listado_tmdb",
                    text_color=color5,
                )
            )
    ...
    if item.infoLabels["tmdb_id"]:
        ...
        title = title.replace("película", config.get_localized_string(70137)).replace(
            "serie", config.get_localized_string(30123)
        )
        itemlist.append(
            item.clone(
                title=config.get_localized_string(70328) % title,
                action="listado_tmdb",
                search={
                    "url": "%s/%s/similar" % (item.extra, item.infoLabels["tmdb_id"]),
                    "language": langt,
                    "page": 1,
                },
                infoLabels={"mediatype": item.contentType},
        ...
        itemlist.append(
            item.clone(
                title=config.get_localized_string(70315),
                action="listado_tmdb",
                infoLabels={"mediatype": item.contentType},
                search={
                    "url": "%s/%s/recommendations"
                    % (item.extra, item.infoLabels["tmdb_id"]),
                    "language": langt,
                    "page": 1,
                },
                text_color=color2,
...
def reparto(item):
    ...
    try:
        ...
        if cast:
            itemlist.append(
                item.clone(
                    title=config.get_localized_string(70314),
                    action="",
                    text_color=color2,
                )
            )
            ...
            for actor in cast:
                ...
                if item.contentType == "movie":
                    ...
                    new_item.search = {
                        "url": "person/%s/tv_credits" % actor["id"],
                        "language": langt,
                    }
    ...
    try:
        ...
        if crew:
            itemlist.append(
                item.clone(
                    title=config.get_localized_string(70319),
                    action="",
                    text_color=color2,
                )
            )
    ...
    for temp in range(item.infoLabels["number_of_seasons"], 0, -1):
        ...
        if temporada:
            ...
            if temporada["poster_path"]:
                new_item.infoLabels["poster_path"] = (
            ...
            for epi in range(1, len(temporada["episodes"])):
                ...
                if episodio:
                    new_item = item.clone(
                        action="", text_color=color1, mediatype="episode"
                    )
                    new_item.infoLabels["season"] = temp
                    ...
                    if episodio["episodio_air_date"]:
                        ...
                        new_item.infoLabels["year"] = episodio[
                            "episodio_air_date"
                        ].rsplit("/", 1)[1]
                    if episodio["episodio_vote_average"]:
                        new_item.infoLabels["rating"] = episodio[
                            "episodio_vote_average"
                        ]
                        new_item.infoLabels["votes"] = episodio["episodio_vote_count"]
                    new_item.title = "      %sx%s - %s" % (
                        temp,
                        epi,
                        new_item.infoLabels["title"],
                    )
...
def indices_tmdb(item):
    ...
    if config.get_localized_string(70032) in item.title:
        ...
        url = urlparse.urljoin(
            host_tmdb,
            "3/genre/%s/list?api_key=a1ab8b8669da03637a4b98fa39c39228&language=%s"
            % (item.extra, langt),
        )
        try:
            lista = jsontools.load(
                httptools.downloadpage(url, cookies=False, **kwargs).data
            )["genres"]

            for control in lista:
                lista_generos[str(control["id"])] = control["name"]
                if "es" in langt:
                    thumbnail[str(control["id"])] = "%s1/%s.jpg" % (
                        images_predef,
                        control["name"]
                        .lower()
                        .replace("ó", "o")
                        .replace("í", "i")
                        .replace(" ", "%20")
                        .replace("Aventuras", "Aventura")
                        .replace("ú", "u"),
                    )
                else:
                    thumbnail[str(control["id"])] = "%s2/%s.jpg" % (images_predef, control["name"])
        except Exception:
            pass
        ...
        for i in range(year, 1899, -1):
            ...
            itemlist.append(
                item.clone(title=str(i), action="listado_tmdb", search=search)
            )
...
def filtro(item):
    ...
    list_controls.append(
        {
            "id": "years",
            "label": config.get_localized_string(60232),
            "enabled": True,
            "color": "0xFFCC2EFA",
            "type": "list",
            "default": -1,
    ...
    if "Personalizado" in item.title:
        ...
        valores_guardados = config.get_setting(
            "filtro_defecto_" + item.extra, item.channel
        )
        ...
        url = urlparse.urljoin(
            host_tmdb,
            "3/genre/%s/list?api_key=f7f51775877e0bb6703520952b3c7840&language=%s"
            % (item.extra, langt),
        )
        try:
            lista = jsontools.load(
        ...
        list_controls.append(
            {
                "id": "orden",
                "label": config.get_localized_string(70455),
                "enabled": True,
                "color": "0xFF25AA48",
                "type": "list",
                "default": -1,
        ...
        orden = [
            config.get_localized_string(70456),
            config.get_localized_string(70457),
            config.get_localized_string(70458),
            config.get_localized_string(70459),
            config.get_localized_string(70460),
            config.get_localized_string(70461),
        ]
        if item.extra == "movie":
            orden.extend(
        ...
        list_controls.append(
            {
                "id": "save",
                "label": config.get_localized_string(70464),
                "enabled": True,
                "type": "bool",
                "default": False,
                "visible": True,
        ...
        list_controls.append(
            {
                "id": "keyword",
                "label": config.get_localized_string(70465),
                "enabled": True,
                "type": "text",
                "default": "",
                "visible": True,
    ...
    return platformtools.show_channel_settings(
        ...
        caption=config.get_localized_string(70320),
        item=item,
        callback="filtrado",
    )
...
def musica_movie(item):
    ...
    data = httptools.downloadpage(item.url, **kwargs).data

    patron = '<td>(\d+)\.*<\/td><td\s*class="left">([^<]+)<br>\s*<small>([^<]+)<\/small>[^<]*<\/td>\s*<td>(\d+:\d+)<'
    matches = scrapertools.find_multiple_matches(data, patron)
    patron_file = "AudioPlayer\.embed\('pl%s[^}]+soundFile:\s*'([^']+)'"

    for id_p, titulo, artist, duration in matches:
        title = "%s (%s) [%s]" % (titulo, artist, duration)
        url = scrapertools.find_single_match(data, patron_file % id_p)
...
def listado_imdb(item):
    ...
    headers = [["Accept-Language", langi]]
    # kwargs['replace_headers'] = True
    if host_imdb in item.url:
        data = httptools.downloadpage(item.url, headers=headers, **kwargs).data
    else:
        url = urlparse.urljoin(host_imdb, "search/title/?" + item.url)
    ...
    if "search/name" in item.url:
        patron = (
            '<td class="image">.*?src="([^"]+)".*?href="/name/(nm\d+).*?>([^<]+)<.*?href.*?>([^<]+)</a>'
            "</span>(.*?)</td>"
        )
        patron = (
            '<div\s*class="lister-item-image">\s*<a\s*href="\/name\/(nm\d+)">\s*'
            '<img[^>]*src="([^"]+)".*?<a\s*href.*?>\s*([^<]+)<\/a>.*?<span\s*class='
            '"ghost">[^<]*<\/span><a\s*href.*?>\s*([^<]+)<\/a>(.*?)<\/div>\s*<\/div>'
        )
        matches = scrapertools.find_multiple_matches(data, patron)

        for imdb_id, thumbnail, title, movie, datos in matches:
            new_item = item.clone(action="listado_tmdb", unify=unify)
            try:
                if "@" in thumbnail:
            ...
            new_item.infoLabels["year"] = scrapertools.find_single_match(
                datos, "born\s*(?:on)?\s*\w+\s*\d{1,2},?\s*(\d{4})"
            )
            if new_item.infoLabels["year"]:
                year = " [COLOR grey](%s)[/COLOR]" % new_item.infoLabels["year"]
            else:
                year = ""
            new_item.title = (
                title.strip()
                + year
                + "   [COLOR %s](%s)[/COLOR]" % (color6, movie.strip())
            )
        ...
        if "calendar/" in item.url:
            estreno = ""
            patron = (
                '(?:data-testid="release-date">\s*<hgroup>\s*<h3\s*class='
                '"ipc-title__text"\s*>([^<]*)<\/h3>\s*<\/hgroup>\s*<\/div>)?'
                '<(?:ul|li)\s*class="ipc-metadata-list[^>]*>.*?(?:<img[^>]*src="([^"]+)".*?)?'
                '<a\s*class="ipc-metadata-list-summary-item__t"[^>]*href="\/title\/([^"]+)\/[^>]*>([^<]+)<'
            )
            matches_ini = scrapertools.find_multiple_matches(data, patron)

            for _estreno, thumbnail, imdb_id, title in matches_ini:
                if _estreno:
                    estreno = _estreno
                matches.append((thumbnail, "", imdb_id, title, "", "", "", estreno))
        else:
            patron = (
                '(?:<div class="image">|<div class="lister-item-image).*?(?:loadlate="([^"]+)"|src="([^"]+)")'
                '.*?href=".*?/(tt\d+).*?>([^<]+)</a>(.*?)(?:<p class="(?:text-muted|)">([^<]+)|"description">([^<]+)<)()'
            )
            matches = scrapertools.find_multiple_matches(data, patron)

        for thumbnail, thumb2, imdb_id, title, datos, plot, plot2, estreno in matches:
            new_item = item.clone(action="detalles", unify=unify_config)
            new_item.title = re.sub("\(*\d{4}\)*", "", title).strip()
            ...
            if new_item.contentType == "tvshow":
                new_item.contentSerieName = new_item.contentTitle
            ...
            rating = scrapertools.find_single_match(
                datos, "(?:rating|Metascore).*?<strong>([^<]*)</strong>"
            )
            ...
            if estreno:
                new_item.plot_extend = plot_extend % estreno
                if not new_item.infoLabels["year"]:
                    new_item.infoLabels["year"] = "-"
    ...
    if next_page:
        if "title_type" in item.url:
            next_page = urlparse.urljoin(host_imdb, next_page)
        elif "search/name" in item.url:
            lines = int(
                scrapertools.find_single_match(item.url, "start=(\d+)") or 0
            ) + len(matches)
            next_page = re.sub(r"start=\d+", "start=%s" % lines, item.url)
        else:
            next_page = urlparse.urljoin(host_imdb, "search/title" + next_page)

        itemlist.append(
            item.clone(
                title=config.get_localized_string(70065), url=next_page, text_color=""
            )
        )
...
def filtro_imdb(item):
    ...
    # Se utilizan los valores por defecto/guardados
    valores_guardados = config.get_setting(
        "filtro_defecto_imdb_" + item.extra, item.channel
    )
    if valores_guardados:
        dict_values = valores_guardados
    ...
    list_controls.append(
        {
            "id": "title",
            "label": config.get_localized_string(60320),
            "enabled": True,
            "type": "text",
            "default": "",
            "visible": True,
    ...
    list_controls.append(
        {
            "id": "yearsdesde",
            "label": config.get_localized_string(70452),
            "enabled": True,
            "color": "0xFFCC2EFA",
            "type": "list",
            "default": -1,
    ...
    list_controls.append(
        {
            "id": "yearshasta",
            "label": config.get_localized_string(70453),
            "enabled": True,
            "color": "0xFF2ECCFA",
            "type": "list",
            "default": -1,
    ...
    try:
        generos_spa = {
            "Action": config.get_localized_string(70394),
            "Adventure": config.get_localized_string(60267),
            "Animation": config.get_localized_string(60268),
            "Biography": config.get_localized_string(70403),
            "Comedy": config.get_localized_string(60270),
            "Crime": config.get_localized_string(60271),
            "Documentary": config.get_localized_string(70396),
            "Family": config.get_localized_string(70399),
            "Fantasy": config.get_localized_string(60274),
            "Film-Noir": config.get_localized_string(70400),
            "Game-Show": config.get_localized_string(70401),
            "History": config.get_localized_string(70405),
            "Horror": config.get_localized_string(70013),
            "Music": config.get_localized_string(70404),
            "Mistery": config.get_localized_string(70402),
            "News": config.get_localized_string(60279),
            "Reality-TV": config.get_localized_string(70406),
            "Sci-Fi": config.get_localized_string(70397),
            "Sport": config.get_localized_string(70395),
            "Talk-Show": config.get_localized_string(70398),
            "War": config.get_localized_string(70407),
        }

        data = httptools.downloadpage(
            urlparse.urljoin(host_imdb, "search/title"), cookies=False, **kwargs
        ).data

        bloque = scrapertools.find_single_match(data, "<h3>Genres</h3>(.*?)</table>")
        matches = scrapertools.find_multiple_matches(
            bloque, ' value="([^"]+)"\s*>\s*<label.*?>([^<]+)<'
        )
    ...
    try:
        ...
        if matches:
            list_controls.append(
                {
                    "id": "pais",
                    "label": config.get_localized_string(70466),
                    "enabled": True,
                    "color": "0xFFFF8000",
                    "type": "list",
                    "default": -1,
    ...
    orden = [
        ...
        config.get_localized_string(70460),
        config.get_localized_string(70461),
        config.get_localized_string(70462),
        config.get_localized_string(70463),
    ]
...
def filtrado_imdb(item, values):
    ...
    if "save" in values and values["save"]:
        values_copy.pop("save")
        config.set_setting(
            "filtro_defecto_imdb_" + item.extra, values_copy, item.channel
        )
    ...
    item.url = urlparse.urljoin(
        host_imdb,
        "search/title?countries=%s&num_votes=%s,&genres=%s&release_date=%s,%s&sort=%s&"
        "title=%s&title_type="
        % (pais, str(votos), genero_ids, yeard, yearh, orden, values["title"]),
    )
    if item.contentType == "movie":
...
def indices_imdb(item):
    ...
    if config.get_localized_string(70032) in item.title:
        generos_spa = {
            ...
            "Animation": config.get_localized_string(60268),
            "Biography": config.get_localized_string(70403),
            "Thriller": config.get_localized_string(70410),
            "Comedy": config.get_localized_string(60270),
            "Crime": config.get_localized_string(60271),
            "Documentary": config.get_localized_string(70396),
            "Family": config.get_localized_string(70399),
            "Romance": config.get_localized_string(70409),
            "Fantasy": config.get_localized_string(60274),
            "Film-Noir": config.get_localized_string(70400),
            "Game-Show": config.get_localized_string(70401),
            "Drama": config.get_localized_string(70412),
            "Western": config.get_localized_string(70411),
            "History": config.get_localized_string(70405),
            "Horror": config.get_localized_string(70013),
            "Music": config.get_localized_string(70404),
            "Musical": config.get_localized_string(70408),
            "Mystery": config.get_localized_string(70402),
            "News": config.get_localized_string(60279),
            "Reality-TV": config.get_localized_string(70406),
            "Sci-Fi": config.get_localized_string(70397),
            "Sport": config.get_localized_string(70395),
            ...
            for valor, titulo in matches:
                ...
                itemlist.append(
                    item.clone(
                        ...
                        thumbnail=thumbnail,
                        url=urlparse.urljoin(
                            host_imdb, "search/title/?genres=%s&%s" % (valor, item.url)
                        ),
                    )
                )
        ...
        for i in range(year, 1899, -1):
            itemlist.append(
                item.clone(
                    ...
                    url=urlparse.urljoin(
                        host_imdb,
                        "search/title/?release_date=%s,%s&%s" % (i, i, item.url),
                    ),
                )
            )
...
def listado_fa(item):
    ...
    if item.page_fa:
        post = {
            "from": item.page_fa,
            "paramGrid[numRows]": 5,
            "paramGrid[position]": "true",
        }
        post = urlparse.urlencode(post)
        data = httptools.downloadpage(
            item.url, post=post, referer=item.url, **kwargs
        ).data
        ...
        data = httptools.downloadpage(item.url, **kwargs).data
        item.page_fa = int(
            scrapertools.find_single_match(data, "tmPageSize \s*=\s*(\d+)") or 30
        )
        item.total = int(
            scrapertools.find_single_match(data, "tmResCount\s*=\s*(\d+)") or 30
        )
        if item.total <= item.page_fa:
            item.page_fa = ""
    ...
    if item.extra in ["top"]:
        # Top
        patron = (
            '<div\s*class="mc-title"\s*>\s*<a\s*href="([^"]+)"\s*title="([^"]+)"\s*>'
            '[^>]*>\s*\((\d{4})\)\s*<img\s*[^>]*src="([^"]+)[^>]*">\s*<\/div>\s*'
            '<div\s*class="mc-director"\s*>.*?<a\s*href="([^"]+)".*?'
            '<div\s*class="avg-rating"\s*>([^<]+)<\/div>\s*<div\s*class="rat-count">([^<]+)<()'
        )
        matches = scrapertools.find_multiple_matches(data, patron)

    if not matches and item.extra in ["sagas"]:
        # Sagas
        patron = (
            '<div\s*class="mc-poster"\s*>\s*<a\s*href="([^"]+)">\s*<img[^>]*src="([^"]+)".*?'
            '<div\s*class="mc-title">.*?title="([^"]+)">[^<]*<\/a>\s*\(*(\d{4})\)*\s*<.*?'
            '<div\s*class="mc-director">.*?<a\s*href="([^"]+)".*?<div\s*class="avg-rating">'
            "\s*(.*?)\s*<\/div>\s*<\/td>\s*<td>([^<]*)<\/td>"
        )
        matches_ini = scrapertools.find_multiple_matches(data, patron)

        if matches_ini:
            for url, thumb, title, year, url2, rating, votos in matches_ini:
                matches.append((url, title, year, thumb, url2, rating, votos, ""))

    if not matches and item.extra in ["cartelera", "estrenos"]:
        # Cartelera
        patron = (
            '<div\s*class="mc-poster"\s*>\s*<a\s*href="([^"]+)"\s*title="([^"]+)"\s*>'
            '\s*<img[^>]*src="([^"]+)".*?<div\s*class="mc-data">\s*<div>(\d{4})\s*<.*?'
            '<div\s*class="director">.*?<a\s*href="([^"]+)".*?<div\s*class="avg-rating">'
            '([^<]*)<\/div>(?:\s*<div\s*class="rat-count">.*?(\d+)\s*<)?(?:.*?'
            '<span\s*class="date">.*?(\d+\s*de[^<]+)<\/span>)?'
        )
        matches_ini = scrapertools.find_multiple_matches(data, patron)

        if matches_ini:
            for url, title, thumb, year, url2, rating, votos, estreno in matches_ini:
                matches.append((url, title, year, thumb, url2, rating, votos, estreno))

    if not matches and item.extra in ["search"]:
        # Búsquedas
        patron = (
            '<div\s*class="ye-w">(\d{4})<\/div>.*?<div\s*class="mc-poster"\s*>\s*'
            '<a\s*title="([^"]+)"\s*href="([^"]+)"\s*>\s*<img[^<]*src="([^"]+)".*?'
            '<div\s*class="avgrat-box"\s*>([^<]+)<\/div>\s*(?:<div\s*class="ratcount-box"\s*>'
            '([^<]+)<)?.*?(?:<div\s*class="mc-director"\s*>.*?<a\s*href="([^"]+)")?'
        )
        matches_ini = scrapertools.find_multiple_matches(data, patron)

        if matches_ini:
            for year, title, url, thumb, rating, votos, url2 in matches_ini:
                matches.append((url, title, year, thumb, url2, rating, votos, ""))
            lines = int(
                scrapertools.find_single_match(item.url, "start=(\d+)") or 0
            ) + len(matches)
            item.url = re.sub(r"from=\d+", "from=%s" % lines, item.url)

    if not matches and item.extra in ["series_act"]:
        # Series de actualidad
        patron = (
            '<a\s*class="movie-poster-grid"\s*href="([^"]+)".*?<div\s*class='
            '"mc-oposter"\s*title="([^"]+)"\s*>\s*<img\s*[^>]*src="([^"]+)".*?'
            'class="avgrat-box">([^<]*)<'
        )
        matches_ini = scrapertools.find_multiple_matches(data, patron)

        if matches_ini:
            for url, title, thumb, rating in matches_ini:
                matches.append((url, title, "", thumb, "", rating, "", ""))

    for url, _title, year, thumb, url2, rating, votos, estreno in matches:
        title = _title.strip().replace("&amp;", "&")
        thumb2 = thumb

        new_item = item.clone(action="detalles_fa", extra="movie", unify=unify_config)

        if not url:
            url = url2
        new_item.url = urlparse.urljoin(host_filmaffinity, url)

        if not thumb:
            thumb = thumb2
        new_item.thumbnail = urlparse.urljoin(
            host_filmaffinity, thumb.replace("msmall", "large")
        )

        new_item.contentTitle = re.sub(
            r"(?i)\((?:mini)?serie de tv\)|\(tv\)|\(c\)", "", title
        )
        new_item.title = new_item.contentTitle + "  (%s) [COLOR %s]%s[/COLOR]" % (
            year,
            color6,
            rating,
        )

        if re.search(r"(?i)(?:mini)?serie de tv|\(tv\)", title):
            if item.contentType == "movie" and item.extra != "sagas":
                continue
        ...
        new_item.infoLabels["year"] = (
            year or "-" if new_item.contentType == "movie" else ""
        )

        votaciones.append([rating, votos])

        if rating:
            new_item.infoLabels["rating"] = float(
                rating.replace(",", ".").replace("--", "0.0")
            )
            new_item.infoLabels["votes"] = votos.replace("-", "0")
    ...
    for i, it in enumerate(itemlist):
        try:
            ...
            it.infoLabels["rating"] = float(
                votaciones[i][0].replace(",", ".").replace("--", "0.0")
            )
        except Exception:
            pass

    if item.page_fa or (matches and item.extra in ["search"]):
        itemlist.append(
            Item(
                channel=item.channel,
                action=item.action,
...
def indices_fa(item):
    ...
    if "sagas" in item.extra:
        patron = (
            '<li class="fa-shadow">.*?href="([^"]+)".*?group-name">([^<]+)<.*?src="([^"]+)".*?'
            '"count-movies">([^<]+)<'
        )
        matches = scrapertools.find_multiple_matches(data, patron)
        ...
        for url, title, thumbnail, info in matches:
            ...
            new_item.url = urlparse.urljoin(host_filmaffinity, url)
            new_item.thumbnail = urlparse.urljoin(
                host_filmaffinity, thumbnail.replace("msmall", "large")
            )
        ...
        if next_page:
            next_page = urlparse.urljoin(host_filmaffinity, next_page)
            itemlist.append(
                Item(
                    channel=item.channel,
                    action=item.action,
        ...
        matches = scrapertools.find_multiple_matches(
            data,
            'name="genres\[\]"\s*value="(\+[^"]+)">.*?name="genres\[\]"\s*disabled>\s*([^<]+)\s*<',
        )

        for _valor, _titulo in matches:
            valor = _valor.replace("+", "%2B")
            titulo = _titulo.strip()
            if valor == "TV_SE":
                continue
            new_item = item.clone(title=titulo, action="listado_fa", extra="top")
            new_item.url = item.url.replace("|", "%7C") + "&genres=%s" % valor
            ...
            if langf != "en":
                try:
                    new_item.thumbnail = "%s/%s" % (
                        images_predef,
                        generos[len(itemlist)],
                    )
                except Exception:
    ...
    elif "temas" in item.title or "Temas" in item.title:
        patron = '<ul>\s*<div\s*id="(topic_[\w|\W])"\s*class="title">([\w|\W])<\/div>(.*?)<\/ul>'
        bloques = scrapertools.find_multiple_matches(data, patron)
        folder = True
        if config.is_xbmc():
            folder = False

        if "topic_" not in item.url:
            action = "indices_fa"
            for tema, letra, bloque in bloques:
                itemlist.append(
                    item.clone(
                        title="Temas en " + letra,
                        url="%s#%s" % (item.url, tema),
                        text_color=color2,
                        action=action,
                        extra=item.extra,
                    )
            ...
            for tema, letra, bloque in bloques:
                if tema not in item.url:
                    continue

                patron = '<li>\s*<a\s*class="topic"\s*href="([^"]+)">\s*([^<]+)\s*<em>\((\d+)\)<\/em>'
                ...
                for url, titulo, numero in matches:
                    ...
                    new_item.url = url.replace("&nodoc", "").replace("&notvse", "")
        ...
        for i in range(year, 1899, -1):
            ...
            new_item.url = urlparse.urljoin(
                host_filmaffinity,
                "%s/topgen.php?genres=&country=&fromyear=%s&toyear=%s&"
                "nodoc=1&orderby=avg&movietype=%s"
                % (
                    langf,
                    i,
                    i,
                    item.contentType if item.contentType == "movie" else "serie",
                ),
            )
            if item.contentType == "movie":
...
def temas_fa(item):
    ...
    patron = (
        '<div class="mc-poster">\s*<a href=".*?(\d+)\.html".*?src="([^"]+)".*?'
        "href.*?>([^<]+)</a>\s*\((\d+)\)"
    )
    matches = scrapertools.find_multiple_matches(data, patron)
    ...
    for url, thumb, title, year in matches:
        ...
        new_item.url = urlparse.urljoin(
            host_filmaffinity, "%s/film%s.html" % (langf, url)
        )
        new_item.thumbnail = urlparse.urljoin(
            host_filmaffinity, thumb.replace("msmall", "large")
        )
...
def detalles_fa(item):
    ...
    orig_title = scrapertools.find_single_match(
        data, "(?i)<dt>T.tulo\s*original<\/dt>\s*<dd>\s*([^<]+)\s*<\/dd>"
    ).strip()
    if orig_title:
        item.infoLabels["originaltitle"] = re.sub(
            r"(?i)\(TV Series\)\s*|\(Miniserie de TV\)\s*|\(S\)|\(TV\)\s*",
            "",
            orig_title,
        )
    ...
    if item.contentType == "movie":
        ob_tmdb = Tmdb(
            texto_buscado=item_tmdb.contentTitle,
            year=item_tmdb.infoLabels["year"],
            tipo=item_tmdb.extra,
            idioma_busqueda=langt,
        )
        if not ob_tmdb.result:
            ob_tmdb = Tmdb(
                texto_buscado=item_tmdb.infoLabels["originaltitle"],
                year=item_tmdb.infoLabels["year"],
                tipo=item_tmdb.extra,
                idioma_busqueda=langt,
            )
        ...
        if not ob_tmdb.result:
            ob_tmdb = Tmdb(
                texto_buscado=item_tmdb.infoLabels["tvshowtitle"],
                tipo=item_tmdb.extra,
                idioma_busqueda=langt,
            )
    ...
    if ob_tmdb.result:
        ob_tmdb = Tmdb(
            id_Tmdb=ob_tmdb.get_id(), tipo=item_tmdb.extra, idioma_busqueda=langt
        )
        item.infoLabels = ob_tmdb.get_infoLabels(item.infoLabels)
    ...
    plot = scrapertools.decodeHtmlentities(plot).replace(" (FILMAFFINITY)", "").strip()
    if plot and (item.infoLabels["plot"] and item.infoLabels["plot"] != plot):
        item.infoLabels["plot"] += " (TMDB)\n" + plot + " (FILMAFFINITY)"
    elif plot and not item.infoLabels["plot"]:
        item.infoLabels["plot"] = plot
    ...
    generos = scrapertools.find_multiple_matches(
        data, '<span[^>]*itemprop="genre">\s*<a\s*href="([^"]+)">([^<]+)<\/a><\/span>'
    )
    genres = []
    if generos:
        for url, g in generos:
            if "Serie de TV" in g:
                continue
    ...
    rating = (
        scrapertools.find_single_match(data, 'itemprop="ratingValue".*?>([^<]+)<')
        .strip()
        .replace("-.-", "0.0")
        .replace("-", "0")
    )
    ...
    votos = (
        scrapertools.find_single_match(data, 'itemprop="ratingCount".*?>([^<]+)<')
        .strip()
        .replace("-", "0")
    )
    ...
    fanart = scrapertools.find_single_match(
        data, ' <div\s*id="movie-main-image-container">[^>]*href="([^"]+)"'
    )
    item.fanart = fanart or item.infoLabels["fanart"]
    if item.infoLabels["thumbnail"]:
        item.thumbnail = item.infoLabels["thumbnail"]
    ...
    itemlist.append(
        item.clone(
            ...
            title=config.get_localized_string(70069) % (title, item.contentTitle),
            unify=unify_config,
            plot_extend=plot_extend % "Buscar",
        )
    )
    ...
    trailer_url = scrapertools.find_single_match(
        data, '(?i)<li\s*>\s*<a\s*href="([^"]+)"\s*>Tr.ilers[^<]*<em>\s*\[\d*\]'
    )
    itemlist.append(
        item.clone(
            channel="trailertools",
    ...
    url_img = scrapertools.find_single_match(
        data, '(?i)<li\s*>\s*<a\s*href="([^"]+)"\s*>Im.genes[^<]*<em>\s*\[\d*\]'
    )
    images = {}
    ...
    if token_auth and ob_tmdb.result:
        itemlist.append(
            item.clone(title=config.get_localized_string(70323), action="menu_trakt")
        )
    ...
    if mivoto:
        ...
        new_item = item.clone(
            action="votar_fa",
            title=config.get_localized_string(70324) % mivoto,
            itk=itk,
            voto=int(mivoto),
            folder=folder,
        )
        ...
        if itk:
            new_item = item.clone(
                action="votar_fa",
                title=config.get_localized_string(70325) % title,
                itk=itk,
                accion="votar",
                folder=folder,
            )
    ...
    if ob_tmdb.result:
        ...
        itemlist.append(
            item.clone(
                title=config.get_localized_string(70328) % title.capitalize(),
                action="listado_tmdb",
                search={
                    "url": "%s/%s/similar" % (item.extra, item.infoLabels["tmdb_id"]),
                    "language": langt,
...
def filtro_fa(item):
    ...
    # Se utilizan los valores por defecto/guardados
    valores_guardados = config.get_setting(
        "filtro_defecto_filmaf_" + item.extra, item.channel
    )
    if valores_guardados:
        dict_values = valores_guardados
    ...
    data = httptools.downloadpage(
        "https://m.filmaffinity.com/%s/topgen.php" % langf, **kwargs
    ).data
    data = re.sub(r"\n|\r|\t|&nbsp;", "", data)
    data = re.sub(r"\s{2}", " ", data)
    ...
    if item.contentType == "movie":
        try:
            bloque = scrapertools.find_single_match(
                data, 'name="genre">.*?</option>(.*?)</select>'
            )
            matches = scrapertools.find_multiple_matches(
                bloque, '<option value="([^"]+)">([^<]+)</option>'
            )
            if matches:
                list_controls.append(
                    {
                        "id": "genero",
                        "label": config.get_localized_string(70467),
                        "enabled": True,
                        "type": "list",
                        "default": -1,
                        "visible": True,
    ...
    try:
        bloque = scrapertools.find_single_match(
            data, 'name="country">.*?</option>(.*?)</select>'
        )
        matches = scrapertools.find_multiple_matches(
            bloque, '<option value="([^"]+)"\s*>([^<]+)</option>'
        )
...
def filtrado_fa(item, values):
    ...
    if "save" in values and values["save"]:
        values_copy.pop("save")
        config.set_setting(
            "filtro_defecto_filmaf_" + item.extra, values_copy, item.channel
        )
    ...
    item.url = urlparse.urljoin(
        host_filmaffinity,
        "%s/topgen.php?genre=%s&country=%s&fromyear=%s&toyear=%s&nodoc=1"
...
def login_fa():
    ...
    try:
        ...
        data = httptools.downloadpage(
            urlparse.urljoin(host_filmaffinity, "%s" % langf), **kwargs
        ).data
        ...
        if "Invalid username" in data:
            ...
            post = "name=user-menu&url=%s%s/main.php" % (host_filmaffinity, langf)
...
def cuenta_fa(item):
    ...
    if not login:
        ...
        itemlist.append(
            item.clone(
                action="acciones_fa",
                title=config.get_localized_string(70332),
                text_color=color5,
                accion="votos",
                url=urlparse.urljoin(
                    host_filmaffinity,
                    "%s/user_ratings.php?id-user=%s" % (langf, userid),
                ),
            )
        )
        ...
        itemlist.append(
            item.clone(
                ...
                text_color=color5,
                accion="listas",
                url=urlparse.urljoin(host_filmaffinity, "%s/mylists.php" % langf),
            )
        )
...
def acciones_fa(item):
    ...
    if item.accion == "votos" or item.accion == "lista":
        ...
        for bloque in bloques:
            ...
            for url, thumb, title, year, rating, votos in matches:
                ...
                if not new_item.thumbnail.startswith("http"):
                    new_item.thumbnail = urlparse.urljoin(
                        host_filmaffinity, new_item.thumbnail
                    )
        ...
        data = re.sub(r"\s{2}", " ", data)
        patron = (
            'list-group-item rip.*?href="([^"]+)".*?<strong>([^<]+)</strong>.*?<em>([^<]+)</em>'
        ...
        for url, title, content, imgs in matches:
            ...
            new_item.url = urlparse.urljoin(
                host_filmaffinity, "%s&orderby=%s" % (url, orderby)
            )
            new_item.title = title + "  [COLOR %s](%s)[/COLOR]" % (color6, content)
            if imgs:
                imagenes = scrapertools.find_multiple_matches(
                    imgs, 'data-src="([^"]+)"'
                )
                from random import randint
        ...
        url = urlparse.urljoin(
            host_filmaffinity, "%s/edtmovielists.php?movie_id=%s" % (langf, movieid)
        )
        ...
        for listid, chequeo, title in matches:
            ...
            if "checked" in chequeo:
                new_item.title = "[COLOR %s]%s[/COLOR] %s" % (
                    "green",
                    "\u0474".encode("utf-8"),
                    title,
                )
        ...
        new_item = item.clone(
            action="newlist",
            title=config.get_localized_string(70333),
            text_color=color6,
        )
        new_item.infoLabels["duration"] = ""
        ...
        url = urlparse.urljoin(host_filmaffinity, "%s/movieslist.ajax.php" % langf)
        movieid = item.url.rsplit("=", 1)[1]
        post = "action=%s&listId=%s&movieId=%s&itk=%s" % (
            item.accion,
            item.listid,
            movieid,
            item.itk,
        )
        data = jsontools.load(httptools.downloadpage(url, post=post, **kwargs).data)
...
def votar_fa(item):
    ...
    list_controls.append(
        {
            "id": "voto",
            "label": config.get_localized_string(70468),
            "enabled": True,
            "type": "list",
            "default": 0,
            "visible": True,
    ...
    return platformtools.show_channel_settings(
        list_controls=list_controls,
        dict_values=dict_values,
        caption=config.get_localized_string(70336) % item.contentTitle,
        item=item,
...
def newlist(item):
    ...
    if item.accion == "lista":
        location = httptools.downloadpage(item.url, **kwargs).headers["location"]
        data = httptools.downloadpage(
            urlparse.urljoin(host_filmaffinity, location), **kwargs
        ).data
        ...
        url = urlparse.urljoin(
            host_filmaffinity, "%s/addlist.php?rp=%s" % (langf, item.url)
        )
        data = httptools.downloadpage(url, **kwargs).data
        data = re.sub(r"\n|\r|\t|&nbsp;|\s{2}", "", data)

        patron = (
            'data-list-id="[^"]+" href="([^"]+)"><[^>]+><div class="col-xs-10">'
            '([^<]+)</div><div[^>]+><div type="button" class="btn btn-primary">'
        )
        matches = scrapertools.find_multiple_matches(data, patron)

        for scrapedurl, title in matches:
            scrapedurl = urlparse.urljoin(host_filmaffinity, scrapedurl)
...
def imagenes(item):
    ...
    if item.extra == "menu":
        ...
        if "filmaffinity" in item.images:
            itemlist.append(
                item.clone(title="Filmaffinity", text_color=color2, extra="")
            )

        if "myanimelist" in item.images:
            data = httptools.downloadpage(
                item.url + "/pics", cookies=False, **kwargs
            ).data
            images = scrapertools.find_multiple_matches(
                data, '<div class="picSurround"><a href="([^"]+)" title="([^"]+)"'
            )
            if images:
                for thumb, title in images:
                    item.images["myanimelist"].append([thumb, title])
                itemlist.append(
                    item.clone(title="MyAnimeList", text_color=color2, extra="")
                )
    ...
    if "Fanart" in item.title:
        ...
        if not resultado:
            itemlist.append(
                item.clone(title=config.get_localized_string(70338), action="")
            )
            return itemlist
        ...
        try:
            ...
            patron = (
                '<div\s*class="colorbox-image"[^>]*>\s*<a\s*href="([^"]+)"\s*'
                'title=".*?(?:<div\s*><strong>\s*Desc:\s*<\/strong>([^<]+)<[^"]+)?"'
            )
            matches_int = scrapertools.find_multiple_matches(data, patron)

            for url, _title in matches_int:
            ...
            itemlist.append(
                item.clone(title=config.get_localized_string(70339), action="")
            )
            return itemlist
        ...
        try:
            data = jsontools.load(
                httptools.downloadpage(
                    item.images["imdb"]["url"], cookies=False, **kwargs
                ).data
            )
                ...
                if item.folder:
                    for tipo, child in value.iteritems():
                        for i, imagen in enumerate(child):
                            thumb = (
                                "https://image.tmdb.org/t/p/w500" + imagen["file_path"]
                            )
                            fanart = (
                                "https://image.tmdb.org/t/p/original"
                                + imagen["file_path"]
...
def fanartv(item):
    ...
    if item.contentType == "tvshow" and id_search:
        search = {
            "url": "tv/%s/external_ids" % item.infoLabels["tmdb_id"],
            "language": langt,
        }
    ...
    if id_search:
        if item.contentType == "movie":
            url = (
                "https://webservice.fanart.tv/v3/movies/%s?api_key=cab16e262d72fea6a6843d679aa10300"
                % item.infoLabels["tmdb_id"]
            )
        else:
            url = (
                "https://webservice.fanart.tv/v3/tv/%s?api_key=cab16e262d72fea6a6843d679aa10300"
                % id_search
            )
        # kwargs['replace_headers'] = True
        data = jsontools.load(
            httptools.downloadpage(url, headers=headers, **kwargs).data
        )
...
def menu_trakt(item):
    ...
    try:
        ...
        if '"tmdb":%s' % item.infoLabels["tmdb_id"] in result:
            itemlist.append(
                item.clone(
                    title=config.get_localized_string(70341) % title,
                    action="acciones_trakt",
            ...
            itemlist.append(
                item.clone(
                    title=config.get_localized_string(70342) % title,
                    action="acciones_trakt",
                    url="sync/history",
                    post=post,
                )
    ...
    try:
        ...
        if '"tmdb":%s' % item.infoLabels["tmdb_id"] in result:
            itemlist.append(
                item.clone(
                    title=config.get_localized_string(70343) % title,
                    action="acciones_trakt",
            ...
            itemlist.append(
                item.clone(
                    title=config.get_localized_string(70344) % title,
                    action="acciones_trakt",
                    url="sync/watchlist",
                    post=post,
                )
    ...
    try:
        ...
        if '"tmdb":%s' % item.infoLabels["tmdb_id"] in result:
            itemlist.append(
                item.clone(
                    title=config.get_localized_string(70345) % title,
                    action="acciones_trakt",
            ...
            itemlist.append(
                item.clone(
                    title=config.get_localized_string(70346) % title,
                    action="acciones_trakt",
                    url="sync/collection",
                    post=post,
                )
...
def acciones_trakt(item):
    ...
    itemlist = []
    item.text_color = color1
    rating_extend = " Trakt:[COLOR %s][%.1f][/COLOR]"
    ...
    url = urlparse.urljoin(host_trakt, "%s" % item.url)
    # kwargs['replace_headers'] = True
    data = httptools.downloadpage(url, post=post, headers=headers, **kwargs)
    ...
    if data and "sync" in item.url:
        if not item.post:
            ...
            if "not_found" in data:
                return platformtools.dialog_notification(
                    "Trakt", config.get_localized_string(70347)
                )
            else:
                return platformtools.dialog_notification(
                    "Trakt", config.get_localized_string(70348)
                )
        ...
        try:
            ...
            for i, new_item in enumerate(itemlist[:-1]):
                if new_item.infoLabels["title"]:
                    new_item.title = (
                        new_item.infoLabels["title"]
                        + "  (%s)" % new_item.infoLabels["year"]
                    )
                if ratings[i]:
                    new_item.plot_extend = rating_extend % (color6, ratings[i])
                    new_item.title += (
                        "  [COLOR %s]Trakt:%.2f[/COLOR]/[COLOR %s]Tmdb:%.2f[/COLOR]"
        ...
        if data and "page=1" in item.url and item.order:
            valores = {
                "rank": config.get_localized_string(70003),
                "added": config.get_localized_string(70469),
                "title": config.get_localized_string(60320),
                "released": config.get_localized_string(70470),
                "runtime": config.get_localized_string(70471),
                "popularity": config.get_localized_string(70472),
                "percentage": config.get_localized_string(70473),
                "votes": config.get_localized_string(70474),
                "asc": config.get_localized_string(70475),
                "desc": config.get_localized_string(70476),
            }
        ...
        if "page" in item.url and len(itemlist) == 20:
            ...
            itemlist.append(
                item.clone(
                    title=config.get_localized_string(70065), text_color="", url=url
                )
            )
    else:
        data = jsontools.load(data)
        ...
        for entry in data:
            new_item = item.clone()
            new_item.title = entry["name"] + "  [COLOR %s](%s)[/COLOR]" % (
                color6,
                entry["item_count"],
            )
            new_item.infoLabels["plot"] = entry.get("description")
            new_item.url = (
                "users/me/lists/%s/items/?page=1&limit=20&extended=full"
                % entry["ids"]["trakt"]
            )
...
def order_list(item):
    ...
    dict_values = {
        "orderby": valores1.index(item.order),
        "orderhow": valores2.index(item.how),
    }
...
def top_mal(item):
    ...
    itemlist = []
    item.text_color = color1
    data = httptools.downloadpage(item.url, cookies=False, **kwargs).data
    data = re.sub(r"\n|\r|\t|&nbsp;", "", data)
    data = re.sub(r"\s{2}", " ", data)

    patron = (
        '<td class="title al va-t word-break">.*?href="([^"]+)".*?src="(.*?).jpg.*?'
    ...
    for url, thumb, title, info, year, rating in matches:
        ...
        if not item.extra:
            new_item.title += "   %s" % info.replace("Movie (1 eps)", "Movie").replace(
                "TV ", ""
            )
    ...
    next_page = scrapertools.find_single_match(
        data, 'limit=(\d+)" class="link-blue-box next">'
    )
    if next_page:
...
def detalles_mal(item):
    ...
    header_mal = {
        "Cookie": "%s search_sort_anime=score; search_view=tile; is_logged_in=1"
        % cookie_session
    }
    data = httptools.downloadpage(
        item.url, headers=header_mal, cookies=False, **kwargs
    ).data
    data = re.sub(r"\n|\r|\t|&nbsp;", "", data)
    data = re.sub(r"\s{2}", " ", data)
    ...
    if not item.extra:
        extra = scrapertools.find_single_match(
            data, "Type:</span>.*?>([^<]+)</a>"
        ).lower()
        item.tipo = extra
    ...
    if item.infoLabels["rating"] != "0.0":
        ...
        rating = scrapertools.find_single_match(
            data, '<span itemprop="ratingValue">(\d.\d+)</span>'
        )
    ...
    eng_title = scrapertools.find_single_match(
        data, "English:</span> ([^<]+)</div>"
    ).strip()
    item_tmdb = item.clone()
    ...
    plot = scrapertools.find_single_match(
        data, '<span itemprop="description">(.*?)</span>'
    )
    ...
    if not item.infoLabels["duration"]:
        try:
            horas, min1, min2 = scrapertools.find_single_match(
                data, "Duration:</span>\s*(?:(\d+) hr\. (\d+) min|(\d+) min)"
            )
            if horas:
                horas = int(horas) * 360
            else:
    ...
    votos = scrapertools.find_single_match(
        data, '<span itemprop="ratingCount">([^<]+)<'
    )
    ...
    itemlist.append(
        item.clone(
            action="busqueda",
            title=config.get_localized_string(70350) % title_mal,
    ...
    if item.infoLabels["title"] and title_mal != item.infoLabels["title"]:
        itemlist.append(
            item.clone(
                action="busqueda",
                contentTitle=item.infoLabels["title"],
    ...
    itemlist.append(
        item.clone(
            action="videos_mal",
            title=config.get_localized_string(70353),
            text_color=color5,
            url=item.url + "/video",
        )
    )
    ...
    if (
        "No characters or voice actors" not in data
        and "No staff for this anime" not in data
    ):
        itemlist.append(
            item.clone(
                action="staff_mal",
                title=config.get_localized_string(70354),
                text_color=color2,
    ...
    itemlist.append(
        item.clone(
            channel="trailertools",
            action="buscartrailer",
            title=config.get_localized_string(30162),
            text_color=color5,
        )
    )
    ...
    try:
        ...
        post = "busqueda=%s&button=Search" % urlparse.quote(title_search)
        data_music = httptools.downloadpage(
            "https://www.freeanimemusic.org/song_search.php", post=post, **kwargs
        ).data
        if "NO MATCHES IN YOUR SEARCH" not in data_music:
            itemlist.append(
                item.clone(
                    action="musica_anime",
                    title=config.get_localized_string(70317),
    ...
    score = scrapertools.find_single_match(
        data, 'id="myinfo_score".*?selected" value="(\d+)"'
    )
    if score != "0":
    ...
    if item.login and "Add to My List</span>" in data and config.is_xbmc():
        itemlist.append(
            item.clone(
                title=config.get_localized_string(70321) % score,
                action="menu_mal",
                contentTitle=title_mal,
            )
        )
        ...
        status = {
            "1": config.get_localized_string(70479),
            "2": config.get_localized_string(70480),
            "3": config.get_localized_string(70384),
            "4": config.get_localized_string(70385),
            "6": config.get_localized_string(70481),
        }
        estado = scrapertools.find_single_match(
            data, 'myinfo_updateInfo".*?option selected="selected" value="(\d+)"'
        )
        try:
            estado = status[estado]
            itemlist.append(
                item.clone(
                    title=config.get_localized_string(70322) % (color6, estado, score),
                    action="menu_mal",
                    contentTitle=title_mal,
                )
            )
    ...
    alt_version = scrapertools.find_single_match(
        data, "Alternative version:</td>(.*?)</td>"
    )
    if alt_version:
        matches = scrapertools.find_multiple_matches(
            alt_version, 'href="([^"]+)">(.*?)</a>'
    ...
    if ob_tmdb.result:
        ...
        itemlist.append(
            item.clone(
                title=config.get_localized_string(70358),
                action="listado_tmdb",
                infoLabels={"mediatype": item.contentType},
                search={
                    "url": "%s/%s/recommendations"
    ...
    itemlist.append(
        item.clone(title=config.get_localized_string(70359), action="reco_mal")
    )
    anidb_link = scrapertools.find_single_match(
        data,
        '<a href="(https://anidb.info/perl-bin/animedb.pl\?show=anime&amp;aid=\d+)',
    )
    if anidb_link:
...
def videos_mal(item):
    ...
    if "No episode video" not in data and "https://www.hulu.com/" not in data:
        patron = (
            '<a class="video-list di-ib po-r" href="([^"]+)".*?data-src="([^"]+)".*?'
        ...
        for url, thumb, epi, info, title in matches:
            ...
            new_item = item.clone(
                url=url, thumbnail=thumb, action="play", text_color=color1
            )
        ...
        next_page = scrapertools.find_single_match(
            data, '<a href="([^"]+)" class="link-blue-box">More'
        )
        if next_page:
            itemlist.append(
                item.clone(
                    title=config.get_localized_string(70361),
                    url=next_page,
                    text_color="",
                )
            )
    if itemlist:
        itemlist.insert(
            0,
            item.clone(
                title=config.get_localized_string(70362), action="", text_color=color3
            ),
        )
    ...
    if matches:
        itemlist.append(
            item.clone(
                title=config.get_localized_string(70363), action="", text_color=color3
            )
        )
...
def reco_mal(item):
    ...
    data = httptools.downloadpage(item.url + "/userrecs", cookies=False, **kwargs).data
    data = re.sub(r"\n|\r|\t|&nbsp;", "", data)
    data = re.sub(r"\s{2}", " ", data)
    # bloque = scrapertools.find_single_match(
    #     data, '<div class="anime-slide-block" id="anime_recommendation"(.*?)</ul></div>'
    # )
    patron = (
        '<div class="picSurround"><a href="([^"]+)".*?data-src="([^"]+)".*?<strong>(.*?)</strong>.*?'
...
def indices_mal(item):
    ...
    if "Temporadas" in item.title:
        data = httptools.downloadpage(
            "https://myanimelist.net/anime/season/archive", cookies=False, **kwargs
        ).data
        data = re.sub(r"\n|\r|\t|&nbsp;", "", data)
        data = re.sub(r"\s{2}", " ", data)
        matches = scrapertools.find_multiple_matches(
            data, '<td>\s*<a href="([^"]+)">\s*(.*?)\s*</a>'
        )
        ...
        for url, title in matches:
            ...
            title = (
                title.replace("Winter", config.get_localized_string(70364))
                .replace("Spring", config.get_localized_string(70365))
                .replace("Summer", config.get_localized_string(70366))
                .replace("Fall", config.get_localized_string(70367))
            )
            itemlist.append(
                Item(
                    channel=item.channel,
        ...
        data = httptools.downloadpage(
            urlparse.urljoin(host_mal, "anime.php"), cookies=False, **kwargs
        ).data
        data = re.sub(r"\n|\r|\t|&nbsp;", "", data)
        data = re.sub(r"\s{2}", " ", data)
        bloque = scrapertools.find_single_match(data, "Genres</div>(.*?)View More</a>")
        matches = scrapertools.find_multiple_matches(
            bloque, '<a href="([^"]+)" class="genre-name-link">(.*?)</a>'
        )
        ...
        for url, title in matches:
            ...
            if genero in ["Hentai", "Yaoi", "Yuri"] and not adult_mal:
                continue
            url = urlparse.urljoin(host_mal, url)
...
def season_mal(item):
    ...
    if item.info:
        patron = (
            '<div class="anime-header">([^<]+)</div>(.*?)</div>\s*</div></div></div>'
        )
        bloques = scrapertools.find_multiple_matches(data, patron)

        for head_title, bloque in bloques:
            head_title = head_title.replace(
                "(New)", config.get_localized_string(70369)
            ).replace("(Continuing)", config.get_localized_string(70369))
            patron = (
                '<a href="([^"]+)" class="link-title">(.*?)</a>.*?<span>(\? ep|\d+ ep).*?'
            ...
            for (
                ...
                if (
                    "Hentai" in generos or "Yaoi" in generos or "Yuri" in generos
                ) and adult_mal:
                    continue
        ...
        next_page = scrapertools.find_single_match(
            data, '<a class="link current" href.*?href="([^"]+)"'
        )
        if next_page:
            itemlist.append(
                Item(
...
def staff_mal(item):
    ...
    data = re.sub(r"\s{2}", " ", data)
    patron = (
        '<a href="(/character[^"]+)".*?data-src="([^"]+)".*?href=.*?>([^<]+)<.*?<small>([^<]+)</small>'
        "(.*?)</table>"
    )
    matches = scrapertools.find_multiple_matches(data, patron)
    ...
    if matches:
        itemlist.append(
            item.clone(
                title=config.get_localized_string(70370), action="", text_color=color3
            )
        )
...
def detail_staff(item):
    ...
    if item.extra == "character" and "No biography written" not in data:
        bio = scrapertools.find_single_match(
            data,
            'itemprop="name">.*?<div class="normal_header".*?</div>(.*?)<div class="normal_header"',
        )
        bio = bio.replace("<br />", "\n")
        bio = scrapertools.htmlclean(bio)
        if "questionmark" not in item.thumbnail:
            data_img = httptools.downloadpage(
                item.url + "/pictures", cookies=False, **kwargs
            ).data
            matches = scrapertools.find_multiple_matches(
                data_img, 'rel="gallery-character"><img src="([^"]+)"'
            )
        ...
        matches = scrapertools.find_multiple_matches(
            data, '<a href="(/anime[^"]+)"><img src="([^"]+)".*?href.*?>(.*?)</a>'
        )
        if matches:
            itemlist.append(
                Item(
        ...
        infoLabels = {"plot": bio}
        if "No voice acting roles" not in data:
            itemlist.append(
                Item(
                    channel=item.channel,
                    title="Da voz a/en:",
            ...
            bloque = scrapertools.find_single_match(
                data, "Voice Acting Roles</div>(.*?)</table>"
            )
            patron = (
                '<a href="(/anime[^"]+)"><img data-src="([^"]+)".*?href.*?>(.*?)</a>.*?href="(/character[^"]+)".*?'
                '>(.*?)</a>.*?data-src="([^"]+)"'
            )
            matches = scrapertools.find_multiple_matches(bloque, patron)
        ...
        if "No staff positions" not in data:
            itemlist.append(
                Item(
                    channel=item.channel,
                    title="Staff en animes:",
                    action="",
                    text_color=color3,
                    thumbnail=item.thumbnail,
                    infoLabels=infoLabels,
            ...
            bloque = scrapertools.find_single_match(
                data, "Anime Staff Positions</div>(.*?)</table>"
            )
...
def busqueda_mal(item):
    ...
    patron = (
        '<a class="hoverinfo_trigger" href="([^"]+)".*?(?:data-src|src)="([^"]+)".*?'
    ...
    if "&show=" not in item.url:
        next_page = item.url + "&show=50"
    else:
        pagina = int(item.url.rsplit("=", 1)[1])
        next_page = item.url.replace(
            "&show=%s" % str(pagina), "&show=%s" % str(pagina + 50)
        )

    check_page = next_page.replace(urlparse.urljoin(host_mal, "anime.php"), "")
...
def info_anidb(item, itemlist, url):
    ...
    generos = scrapertools.find_multiple_matches(
        data, '<div class="tag".*?<span class="tagname">(.*?)</span>'
    )
    ...
    try:
        ...
        pass
    infoLabels["votes"] = scrapertools.find_single_match(
        data, 'itemprop="ratingCount">(.*?)</span>'
    )
    thumbnail = scrapertools.find_single_match(
        data, '<div class="image">.*?src="([^"]+)"'
    )
    if infoLabels:
    ...
    if re.search(r'(?:subtitle|audio) | language: spanish"', data):
        epi_total = scrapertools.find_single_match(
            data, 'itemprop="numberOfEpisodes">([^<]+)</span>'
        )
        patron = (
            '<td class="name group">.*?title="([^"]+)">(.*?)</a>.*?>([^<]+)</a>.*?'
        ...
        for fansub, abrev, estado, epis, lang, source in matches:
            if "spanish" not in lang:
                continue
            title = "    " + fansub
            if abrev != title:
                title += "  [%s]" % abrev
            estado = (
                estado.replace("complete", config.get_localized_string(70378))
                .replace("finished", config.get_localized_string(70379))
                .replace("stalled", config.get_localized_string(70380))
                .replace("dropped", config.get_localized_string(70381))
            )
            title += " [COLOR %s](%s)[/COLOR] %s/%s  [%s]" % (
                color6,
                estado,
                epis,
                epi_total,
...
def filtro_mal(item):
    ...
    list_controls.append(
        {
            "id": "tipo",
            "label": config.get_localized_string(70482),
            "enabled": True,
            "type": "list",
            "default": -1,
            "visible": True,
    ...
    list_controls[1]["lvalues"] = [
        config.get_localized_string(70483),
        config.get_localized_string(70484),
        config.get_localized_string(60244),
        config.get_localized_string(70136),
        config.get_localized_string(70450),
    ]
    valores["tipo"] = ["4", "2", "3", "1", "0"]
    list_controls.append(
        {
            "id": "valoracion",
            "label": config.get_localized_string(70473),
            "enabled": True,
            "type": "list",
            "default": -1,
            "visible": True,
    ...
    list_controls[2]["lvalues"] = [
        config.get_localized_string(70486),
        config.get_localized_string(70487),
        config.get_localized_string(70488),
        config.get_localized_string(70489),
        config.get_localized_string(70490),
        config.get_localized_string(70491),
        config.get_localized_string(70492),
        config.get_localized_string(70493),
        config.get_localized_string(70494),
        config.get_localized_string(70495),
        config.get_localized_string(70450),
    ]
    ...
    list_controls.append(
        {
            "id": "estado",
            "label": config.get_localized_string(70485),
            "enabled": True,
            "type": "list",
            "default": -1,
            "visible": True,
    ...
    list_controls[3]["lvalues"] = [
        "Por estrenar",
        config.get_localized_string(60264),
        config.get_localized_string(70379),
        config.get_localized_string(70450),
    ]
...
def callback_mal(item, values):
    ...
    for v in values:
        if "genre" in v:
            if values[v]:
                genero_ids.append(
                    "genre[%s]=%s" % (len(genero_ids), v.replace("genre", ""))
                )
    ...
    item.url = urlparse.urljoin(
        host_mal,
        "anime.php?q=%s&type=%s&score=%s&status=%s"
...
def musica_anime(item):
    ...
    data = httptools.downloadpage(
        "https://www.freeanimemusic.org/song_search.php", post=item.post, **kwargs
    ).data
    patron = (
        "<span class='Estilo6'>(\d+).*?<span class='Estilo22'>([^<]+)<.*?<span class='Estilo22'>([^<]+)<"
        ".*?href='https://www.freeanimemusic.org/anime/([^/]+)/index.php\?var=(\d+)"
    )
    matches = scrapertools.find_multiple_matches(data, patron)
    animes = {}
    action = ""
        ...
        if not animes.get(anime):
            ...
            animes[anime].append(
                Item(
                    channel=item.channel,
                    action=action,
                    title="[COLOR %s][%s][/COLOR]" % (color6, anime.capitalize()),
                    url="",
                    number="0",
                    thumbnail=item.thumbnail,
                    fanart=item.fanart,
...
def login_mal(from_list=False):
    ...
    try:
        ...
        post = (
            "user_name=%s&password=%s&cookie=1&sublogin=Login&submit=1&csrf_token=%s"
            % (user, password, token)
        )
        response = httptools.downloadpage(
...
def cuenta_mal(item):
    ...
    if not login:
        itemlist.append(
            item.clone(action="configuracion", title=message, text_color=color4)
        )
    else:
        itemlist.append(
            item.clone(
                action="items_mal",
                title=config.get_localized_string(70382),
                text_color=color5,
                accion="lista_mal",
                url=urlparse.urljoin(
                    host_mal, "animelist/%s?status=1" % user, login=True
                ),
            )
        )
        itemlist.append(
            item.clone(
                action="items_mal",
                title=config.get_localized_string(70383),
                text_color=color5,
                accion="lista_mal",
                url=urlparse.urljoin(
                    host_mal, "animelist/%s?status=2" % user, login=True
                ),
            )
        )
        itemlist.append(
            item.clone(
                action="items_mal",
                title=config.get_localized_string(70384),
                text_color=color5,
                accion="lista_mal",
                url=urlparse.urljoin(
                    host_mal, "animelist/%s?status=3" % user, login=True
                ),
            )
        )
        itemlist.append(
            item.clone(
                action="items_mal",
                title=config.get_localized_string(70385),
                text_color=color5,
                accion="lista_mal",
                url=urlparse.urljoin(
                    host_mal, "animelist/%s?status=4" % user, login=True
                ),
            )
        )
        itemlist.append(
            item.clone(
                action="items_mal",
                title=config.get_localized_string(70386),
                text_color=color5,
                accion="lista_mal",
                url=urlparse.urljoin(
                    host_mal, "animelist/%s?status=6" % user, login=True
                ),
            )
        )
...
def items_mal(item):
    ...
    for d in data_items:
        ...
        title += " %s [COLOR %s][%s/%s][/COLOR] (%s)" % (
            ...
            d["anime_num_episodes"],
            d["anime_media_type_string"],
        )
        title = title.replace("\\", "")
        ...
        url = urlparse.urljoin(host_mal, d["anime_url"].replace("\\", ""))
        if d["score"] != 0:
    ...
    if itemlist:
        itemlist.insert(
            ...
            Item(
                channel=item.channel,
                action="",
                title=config.get_localized_string(70387),
            ),
        )
...
def menu_mal(item):
    ...
    try:
        ...
        button, estado = scrapertools.find_single_match(
            data, 'myinfo_updateInfo"(.*?)>.*?option selected="selected" value="(\d+)"'
        )
    ...
    itemlist.append(
        item.clone(title="Anime: %s%s" % (item.contentTitle, title_estado), action="")
    )
    ...
    for key, value in status.items():
        if value not in title_estado:
            itemlist.append(
                item.clone(
                    title=config.get_localized_string(70391) % value,
                    action="addlist_mal",
                    text_color=color5,
                    value=key,
                    estado=value,
    ...
    for i in range(10, 0, -1):
        if i != int(score):
            itemlist.append(
                item.clone(
                    title=config.get_localized_string(70392) % (color6, i),
                    action="addlist_mal",
                    value=estado,
                    estado=status[estado],
                    score=i,
...
def addlist_mal(item):
    ...
    anime_id = scrapertools.find_single_match(
        data, 'id="myinfo_anime_id" value="([^"]+)"'
    )
    if item.value == "2":
        vistos = scrapertools.find_single_match(
            data, 'id="myinfo_watchedeps".*?<span id="curEps">(\d+)'
        )
    else:
        vistos = scrapertools.find_single_match(
            data, 'id="myinfo_watchedeps".*?value="(\d+)"'
        )
    if not item.score:
        item.score = scrapertools.find_single_match(
            data, 'id="myinfo_score".*?selected" value="(\d+)"'
        )
    ...
    post = {
        ...
        "num_watched_episodes": int(vistos),
        "csrf_token": token,
    }
    headers_mal = {
    ...
    url = urlparse.urljoin(host_mal, "ownlist/anime/add.json")
    if item.lista:
        url = urlparse.urljoin(host_mal, "ownlist/anime/edit.json")
    # kwargs['replace_headers'] = True
    data = httptools.downloadpage(
        url, post=jsontools.dump(post), headers=headers_mal, **kwargs
    ).data
...
def play(item):
    ...
    if not item.server:
        ...
        if "Sorry, this video is not available to be embedded" in data:
            id_video = scrapertools.find_single_match(
                data, '<div class="video-embed.*?-(\d+)\&amp;aff'
            )
            crunchy = (
                "https://www.crunchyroll.com/affiliate_iframeplayer?aff=af-12299-plwa&media_id=%s&video_format=106&video_quality=60&auto_play=0"
                % id_video
            )
        else:
        ...
        if item.server == "directo" and item.song:
            ...
            for child in data_music["data"]:
                if child["title"] == item.url.upper():
                    url = "https://www.musicaanime.org/aannmm11/%s/imagen%s.mp3" % (
                        child["artist"],
                        item.song.zfill(3),
                    )
                    break
...
def get_cookie_value():
    ...
    malsess = scrapertools.find_single_match(
        cookiedata, "myanimelist.*?MALHLOGSESSID\s+([A-z0-9\+\=]+)"
    )
    cookievalue = "MALHLOGSESSID=" + malsess
    mal_id = scrapertools.find_single_match(
        cookiedata, "myanimelist.*?MALSESSIONID\s+([A-z0-9\+\=\-]+)"
    )
    if mal_id:
