...
STATUS_CODES = type(
    "StatusCode",
    (),
    {"stoped": 0, "canceled": 1, "completed": 2, "error": 3, "auto": 4, "control": 5},
)
HOST = "Local"
...
def mainlist(item):
    ...
    if remote_download_dict and not isinstance(remote_download_dict, dict):
        try:
            ...
            logger.error(
                '"downloads_remote_download" ERROR: %s: %s'
                % (str(e), str(remote_download_dict))
            )
    ...
    for file in sorted(filetools.listdir(DOWNLOAD_LIST_PATH)):
        ...
        try:
            ...
            if (
                not i.action
                or "downloadStatus" not in i
                or "downloadCompleted" not in i
                or "downloadProgress" not in i
                or "downloadQueued" not in i
                ...
                logger.error(
                    "Deleting corrupted .json file: %s" % filetools.basename(file)
                )
                continue
        except Exception:
            continue
        ...
        if item.contentType != "tvshow":
            ...
            if i.contentType == "episode":
                if i.from_title and not i.contentSerieName:
                    i.contentSerieName = i.from_title
                # Comprobamos que la serie no este ya en el itemlist
                if not [
                    x
                    for x in itemlist
                    if (
                        x.infoLabels.get("tmdb_id") is not None
                    ...
                    i_bis = Item.clone(i)
                    title = TITLE_TVSHOW % (
                        STATUS_COLORS[i.downloadStatus],
                        i.downloadProgress,
                    ...
                    if i_bis.infoLabels["episode"]:
                        del i_bis.infoLabels["episode"]
                    if i_bis.infoLabels["title"]:
                        del i_bis.infoLabels["title"]
                    ...
                    s = [
                        ...
                        if (
                            x.infoLabels.get("tmdb_id") is not None
                            and x.infoLabels.get("tmdb_id")
                            == i.infoLabels.get("tmdb_id")
                        )
                        or x.contentSerieName.lower() == i.contentSerieName.lower()
                    ][0]
                    ...
                    s.title = TITLE_TVSHOW % (
                        STATUS_COLORS[s.downloadStatus],
                        downloadProgress,
            ...
            # Peliculas
                ...
                i.title = TITLE_FILE % (
                    ...
                    "%s %s"
                    % (
                        unify.set_color(i.contentTitle, "movie"),
                        unify.set_color(i.server.capitalize(), "server")
                        if i.server
                        else "",
                    ),
                    unify.set_color(year, "year"),
        ...
        # Listado dentro de una serie
            ...
            if i.contentType == "episode" and (
                i.infoLabels.get("tmdb_id") is not None
    ...
    if 2 in estados or 4 in estados or 5 in estados:
        itemlist.insert(
            ...
            Item(
                ...
                action="clean_ready",
                title=config.get_localized_string(70218),
                contentType=item.contentType,
                contentChannel=item.contentChannel,
                thumbnail=get_thumb("folder.png"),
    ...
    if 3 in estados:
        itemlist.insert(
            ...
            Item(
                ...
                title=config.get_localized_string(70219),
                contentType=item.contentType,
                contentChannel=item.contentChannel,
    ...
    if len(itemlist) and resetear:
        itemlist.insert(
            ...
            Item(
                channel=item.channel,
                action="restart_all",
                title=config.get_localized_string(70227),
                contentType=item.contentType,
                contentChannel=item.contentChannel,
                thumbnail=get_thumb("update.png"),
                contentSerieName=item.contentSerieName,
    ...
    if (
        ...
        or 4 in estados
        or 5 in estados
        or (2 in estados and item.downloadProgress != 100)
    ):
        itemlist.insert(
            0,
            Item(
                channel=item.channel,
                action="download_all",
                title=config.get_localized_string(70220),
                contentType=item.contentType,
                contentChannel=item.contentChannel,
    ...
    if len(itemlist):
        itemlist.insert(
            ...
            Item(
                channel=item.channel,
                action="clean_all",
                title=config.get_localized_string(70221),
                contentType=item.contentType,
                contentChannel=item.contentChannel,
                thumbnail=get_thumb("folder.png"),
                contentSerieName=item.contentSerieName,
    ...
    if (
        not item.contentType == "tvshow"
        and config.get_setting("downloads_browser") is True
    ):
        itemlist.insert(
            0,
            Item(
...
def get_progress(item):
    ...
    if folder:
        torr_data, deamon_url, index = torrent.get_tclient_data(
            folder,
            torr_client.lower(),
...
def browser(item):
    ...
    context = [
        ...
        {"title": "Copiar a...", "action": "copy_video", "channel": item.channel},
        {"title": "Mover a...", "action": "move_video", "channel": item.channel},
    ]
    ...
    if config.get_setting("downloads_show_torrent_paths", default=True):
        torrent_paths_list = config.get_setting(
            "downloads_torrent_paths_list", default=[]
        )
        if torrent_paths_list and not isinstance(torrent_paths_list, list):
            try:
                ...
                logger.error(
                    '"downloads_torrent_paths_list" ERROR: %s: %s'
                    % (str(e), str(torrent_paths_list))
                )
                torrent_paths_list = []
        if HOST != "Local":
            torrent_paths_list = [
                [
                    "%s" % torrent_paths["TORR_client"].lower(),
                    "%s" % torrent_paths[torrent_paths["TORR_client"].upper()],
                ]
            ]
    contentPlot = (
        "[COLOR limegreen]Idiomas: [/COLOR]%s\n[COLOR limegreen]Calidad: [/COLOR]%s\n\n"
    )
            ...
            if file.endswith(".json"):
                ...
                if download_item.downloadFilename.startswith(":"):
                    ...
                    if filetools.dirname(path) and filetools.isdir(
                        filetools.join(
                            torrent_paths[torr_client], filetools.dirname(path)
                        )
                    ):
                        url = filetools.join(
                            torrent_paths[torr_client], filetools.dirname(path)
                    ...
                    url = filetools.join(
                        download_item.downloadAt, download_item.downloadFilename
                    )
                    if (
                        download_item.infoLabels["mediatype"] == "movie"
                        or item.infoLabels["tmdb_id"] == null
                ...
                title = TITLE_VIDEO % (
                    ...
                    unify.set_color(download_item.server.capitalize(), "server")
                    if download_item.server
                    else "",
                    unify.set_color(year, "year"),
    ...
    for file in filetools.listdir(item.url):
        ...
        if filetools.isdir(url):
            for torr_client, path in torrent_paths_list:
                ...
                if file in [
                    "Extracted",
                    "cached_torrents_Alfa",
                    "Mis_Torrents",
                    "Torrents",
                ]:
                    contentType = "library"
            if file.startswith(".") or file == "MCT-torrents":
                continue
            itemlist.append(
                item.clone(
                    channel=item.channel,
    ...
    if filetools.exists(url):
        ...
        if file_stat:
            ...
            if scrapertools.find_single_match(file_stat, file_stat_pattern):
                file_stat_result = "%s %s %s %s" % scrapertools.find_single_match(
                    file_stat, file_stat_pattern
                )
...
def copy_video(item, move=False):
    ...
    if path_out and filetools.exists(path_out):
        ...
        def copy_background(infile, outfile, move):
            if move:
                if filetools.isdir(infile):
                    ...
                    filetools.move(
                        infile, filetools.join(outfile, filetools.basename(infile))
                    )
                msg = "Mover terminado: "
            else:
                if filetools.isdir(infile):
                    ...
                    filetools.copy(
                        infile,
                        filetools.join(outfile, filetools.basename(infile)),
                        silent=True,
                    )
                msg = "Copia terminada: "
        ...
        if filetools.basename(item.url) == "Extracted":
            path_out = filetools.join(
                path_out, filetools.basename(filetools.dirname(item.url))
            )
        ...
        platformtools.dialog_notification(msg, msg1)
        threading.Thread(
            target=copy_background, args=(item.url, path_out, move)
        ).start()
        time.sleep(1)
    else:
    ...
    for fichero in sorted(filetools.listdir(DOWNLOAD_LIST_PATH)):
        if fichero.endswith(".json"):
            ...
            if not item.contentType == "tvshow" or (
                (
                    item.infoLabels.get("tmdb_id") is not None
                    and item.infoLabels.get("tmdb_id")
                    == download_item.infoLabels.get("tmdb_id")
                )
                or item.contentSerieName.lower()
                == download_item.contentSerieName.lower()
            ):
                if download_item.server == "torrent":
                    if item.remote_download:
                        download_item.remote_download = item.remote_download
    ...
    for fichero in sorted(filetools.listdir(DOWNLOAD_LIST_PATH)):
        if fichero.endswith(".json"):
            ...
            if not item.contentType == "tvshow" or (
                ...
                if download_item.downloadStatus in [
                    STATUS_CODES.completed,
                    STATUS_CODES.auto,
        ...
        if fichero.endswith(".json"):
            ...
            if not item.contentType == "tvshow" or (
                ...
                if download_item.downloadStatus == STATUS_CODES.error:
                    logger.info(
                        "contentAction: %s | contentChannel: %s | downloadProgress: %s | downloadQueued: %s | server: %s | url: %s"
                        % (
                            download_item.contentAction,
                            download_item.contentChannel,
                            download_item.downloadProgress,
                            download_item.downloadQueued,
                            download_item.server,
                            download_item.url,
                        )
                    )
                    ...
                    if download_item.server == "torrent":
                        if item.remote_download:
                            download_item.remote_download = item.remote_download
                        delete_torrent_session(
                            download_item, delete_RAR=False, action="reset"
                        )
                    ...
                    if (
                        download_item.contentAction == "play"
                        and not download_item.downloadServer
                        and not download_item.torr_folder
                    ):
                    ...
                    update_control(
                        fichero,
                        {
                            ...
                            "downloadCompleted": 0,
                            "downloadProgress": 0,
                            "downloadQueued": download_item.downloadQueued,
    ...
    for fichero in sorted(filetools.listdir(DOWNLOAD_LIST_PATH)):
        if fichero.endswith(".json"):
            ...
            if not item.contentType == "tvshow" or (
                ...
                if (
                    download_item.server == "torrent"
                    and download_item.downloadProgress > 0
                    and download_item.downloadProgress < 99
                ):
                    if item.remote_download:
                        download_item.remote_download = item.remote_download
                    delete_torrent_session(
                        download_item, delete_RAR=False, action="pause"
                    )
...
def download_all(item):
    ...
    for fichero in filelist:
        if fichero.endswith(".json") and filetools.exists(
            filetools.join(DOWNLOAD_LIST_PATH, fichero)
        ):
            download_item = Item(
                path=filetools.join(DOWNLOAD_LIST_PATH, fichero)
            ).fromjson(filetools.read(filetools.join(DOWNLOAD_LIST_PATH, fichero)))
            ...
            if not item.contentType == "tvshow" or (
                ...
                if download_item.downloadStatus in [
                    ...
                    download_item.downloadStatus
                    in [STATUS_CODES.completed, STATUS_CODES.auto, STATUS_CODES.control]
                    and (
                    ...
                    if (
                        ...
                        download_item.downloadQueued = (
                            -1 if not download_item.remote_download else 1
                        )
                        res = filetools.write(
                            filetools.join(DOWNLOAD_LIST_PATH, fichero),
                        ...
                        not second_pass
                        and download_item.downloadQueued != 0
                        and download_item.downloadProgress <= 0
                    ):
                        if download_item.downloadStatus == 0:
                        ...
                        if (
                            download_item.server == "torrent"
                            or download_item.sub_action
                            or download_item.url_tvshow
                        ):
                            torrent_params["lookup"] = False
                            try:
                                threading.Thread(
                                    target=start_download, args=(download_item,)
                                ).start()  # Creamos un Thread independiente
                        ...
                        if res == STATUS_CODES.canceled:
                            for file in filelist:
                                download_item = Item(
                                    path=filetools.join(DOWNLOAD_LIST_PATH, file)
                                ).fromjson(
                                    filetools.read(
...
def download_auto(item, start_up=False):
    ...
    if move_to_remote and not isinstance(move_to_remote, list):
        try:
            ...
            logger.error(
                '"downloads_move_to_remote" ERROR: %s: %s'
                % (str(e), str(move_to_remote))
            )
            move_to_remote = []
    filelist = sorted(filetools.listdir(DOWNLOAD_LIST_PATH))
    ...
    for fichero in filelist:
        ...
        if fichero.endswith(".json") and filetools.exists(
            ...
            if download_item.contentType == "movie":
                PATH = filetools.join(
                    config.get_videolibrary_path(), config.get_setting("folder_movies")
                )
            else:
                PATH = filetools.join(
                    config.get_videolibrary_path(), config.get_setting("folder_tvshows")
                )
                ...
                if (
                    ...
                    if res and move_to_remote:
                        for serie, address in move_to_remote:
                            if (
                                serie.lower() in download_item.contentSerieName.lower()
                            ):  # Si está en la lista es que es remoto
                                download_item.downloadQueued = 1
                                if download_item.nfo:
                                    download_item.nfo = download_item.nfo.replace(
                                        PATH, ""
                                    )
                                if download_item.strm_path:
                                    download_item.strm_path = (
                                        download_item.strm_path.replace(PATH, "")
                                    )
                                res = filetools.write(
                                    filetools.join(address, fichero),
                                    download_item.tojson(),
                                    silent=True,
                                )
                                if res:
                                    res = filetools.remove(
                                        filetools.join(DOWNLOAD_LIST_PATH, fichero),
                                        silent=True,
                                    )
                                    break
...
def menu(item):
    ...
    if item.server == "torrent" and item.path.endswith(".json"):
        ...
        if remote_download:
            item.remote_download = remote_download
    # Opciones disponibles para el menu
    op = [
        config.get_localized_string(70225),
        config.get_localized_string(70226),
        config.get_localized_string(70227),
        "Pausar descarga",
        "Modificar servidor: %s" % (servidor.capitalize()),
        config.get_localized_string(70221),
        config.get_localized_string(70146),
        "[COLOR gold]Ver[/COLOR]",
    ]
    ...
    if item.downloadStatus in [0]:  # Sin descargar
        ...
        if not item.server:
            opciones.append(op[4])  # Elegir Servidor
    ...
    if item.downloadStatus in [
        2,
        4,
        5,
    ]:  # descarga completada o archivo de control o auto
        ...
        if item.server == "torrent":
            ...
            update_control(
                item.path,
                {
                    ...
                    "downloadQueued": item.downloadQueued,
                },
                function="menu_op[0]",
            )
            if item.remote_download:
    ...
    if (
        opciones[seleccion] == op[2]
        or opciones[seleccion] == op[3]
        or opciones[seleccion] == op[5]
    ):
        ...
        if item.server == "torrent":
            ...
            if filetools.isfile(
                filetools.join(item.downloadAt or DOWNLOAD_PATH, item.downloadFilename)
            ):
                filetools.remove(
                    filetools.join(
                        item.downloadAt or DOWNLOAD_PATH, item.downloadFilename
                    ),
                    silent=True,
                )
                if (
    ...
    if opciones[seleccion] == op[5]:
        ...
        logger.info(
            "Archivo de control ELIMINADO (%s): %s" % (opciones[seleccion], item.path)
        )
    ...
    if opciones[seleccion] == op[7]:
        ...
        item.url = filetools.dirname(
            filetools.join(item.downloadAt or folder_view, file_view)
        )
        return browser(item)
...
def delete_torrent_session(item, delete_RAR=True, action="delete"):
    if not delete_RAR and "RAR-" not in item.torrent_info:
        delete_RAR = True
    logger.info(
        "action: %s - progress: %s - delete_RAR: %s"
        % (action, item.downloadProgress, str(delete_RAR))
    )
    ...
    if folder_new:
        ...
        if folder_new:
                ...
                if filetools.exists(rar_control):
                    try:
                        ...
                        if rar_control_json.get("pid", 0):
                            try:
                                logger.info(
                                    "os.kill: UnRAR pid %s terminando"
                                    % rar_control_json["pid"],
                                    force=True,
                                )
                                os.kill(rar_control_json["pid"], 9)
                            except Exception as e:
                                logger.debug(
                                    "os.kill: UnRAR pid %s NO terminado, error %s"
                                    % (rar_control_json["pid"], str(e))
                                )
                                if config.get_setting(
                                    "assistant_binary", default=False
                                ):
                                    from lib.alfa_assistant import (
                                        execute_binary_from_alfa_assistant,
                                    )

                                    retCode = execute_binary_from_alfa_assistant(
                                        "killBinary",
                                        "unrar",
                                        p=rar_control_json["pid"],
                                        kwargs={},
                                    )
                                    if retCode in [0, 9, -9, 137, 255]:
                                        logger.info(
                                            "Assistant killBinary: UnRAR pid %s terminado"
                                            % rar_control_json["pid"],
                                            force=True,
                                        )
                                    else:
                                        logger.error(
                                            "Assistant killBinary: UnRAR pid %s NO terminado, retCode %s"
            ...
            if (
                action in ["reset"]
                and "RAR-" in item.torrent_info
                and item.downloadProgress == 99
            ):
                delete_RAR = True
                file_rar = ""
                res = True
                ...
                if (
                    file_rar
                    and rar_control_json
                    and rar_control_json.get("error", 0) < 4
                ):
                    folder_new_mod = folder_new.rstrip("/").rstrip("\\") + "xyz123"
                    res = filetools.rename(
                        folder_new, filetools.basename(folder_new_mod), strict=True
                    )
                    if not res:
                        delete_RAR = True
                        platformtools.dialog_notification(
                            "No es posible cancelar unRAR", "Espere unos minutos..."
                        )
                    else:
                        platformtools.dialog_notification(
                            "Cancelando unRAR", "Puede tardar unos minutos..."
                        )
                if (
                    filetools.join(torrent_paths[torr_client.upper()], folder)
                    != folder_new
                ):
                    res = filetools.rename(
                        folder_new_mod, folder + "xyz123", silent=True, strict=True
                    )
                    if not res:
                        delete_RAR = True
                if not delete_RAR:
                    folder_new = filetools.join(
                        torrent_paths[torr_client.upper()], folder
                    )
                    ...
                    filetools.rmdirtree(
                        filetools.join(folder_new_mod, "Extracted"), silent=True
                    )

            elif (
                action in ["reset", "delete"]
                and "RAR-" in item.torrent_info
                and item.downloadProgress != 99
            ):
                delete_RAR = True
            ...
            if (
                action in ["delete"]
                and not filetools.exists(folder_new)
                and filetools.join(torrent_paths[torr_client.upper()], folder)
                != folder_new
    ...
    if torr_client in ["QUASAR", "ELEMENTUM", "TORREST"]:
        if not delete_RAR or action == "pause":
            folder_new = ""
        torr_data, deamon_url, index = torrent.get_tclient_data(
            folder,
            torr_client.lower(),
    ...
    update_control(
        ...
        function="delete_torrent_session_aft",
    )

    config.set_setting(
        "downloads_RESTART_DOWNLOADS", True
    )  # Forzamos restart downloads
...
def move_to_library(item, forced=False):
    ...
    if config.get_setting("downloads_library_add") is True or forced is True:
        download_path = item.downloadFilename
        item_library_path = filetools.join(
            config.get_videolibrary_path(), *filetools.split(item.downloadFilename)
        )
        absolute_path = download_path
        if item.server == "torrent":
            torrent_client = scrapertools.find_single_match(
                download_path, ":(.+?):"
            ).upper()
            torrent_dirs(item)
        ...
        if config.get_setting("downloads_library_move") is True:
        ...
        if filetools.isfile(absolute_path):
            if (
                ...
                and item.infoLabels["tmdb_id"] != null
            ):
                library_item = Item(
                    title=config.get_localized_string(70228) % item.downloadFilename,
                    channel="downloads",
                    action="findvideos",
                ...
                library_item = Item(
                    title=config.get_localized_string(70228) % item.server.title(),
                    channel="downloads",
                    action="findvideos",
                    infoLabels=item.infoLabels,
                    url=final_path,
...
def get_match_list(
    data, match_list, order_list=None, only_ascii=False, ignorecase=False
):
    """
    ...
    for key in match_list:
        if order_list and key not in order_list:
            logger.error(
                "key '%s' from match_list '%s' not in order_list: '%s'"
                % (key, match_list, order_list)
            )
    ...
    if only_ascii:
        data = "".join(
            (
                c
                for c in unicodedata.normalize("NFD", data)
    ...
    if matches:
        if order_list:
            return type(
                ...
                (),
                {
                    "key": match_dict[matches[-1]],
                    "index": order_list.index(match_dict[matches[-1]]),
                },
            )
        else:
            return type(
                "Mtch_list", (), {"key": match_dict[matches[-1]], "index": None}
            )
    else:
        if order_list:
...
def sort_method(item):
    ...
    order_list_idiomas = lang_orders[
        int(config.get_setting("downloads_sort_servers_language"))
    ]
    match_list_idimas = {
        "ES": ["CAST", "ESP", "Castellano", "Español", "Audio Español"],
        "LAT": ["LAT", "Latino"],
    ...
    order_list_calidad = quality_orders[
        int(config.get_setting("downloads_sort_servers_quality"))
    ]
    match_list_calidad = {
        "BLURAY": ["BR", "BLURAY"],
...
def sort_torrents(play_items, emergency_urls=False, channel="", torrent_info=[]):
    ...
    try:
            ...
            if len(play_items) > 1:
                ...
                for x, play_item in enumerate(play_items):
                    ...
                    if play_item.startswith("magnet") and not quality:
                        ...
                        torrent_params = get_torrent_size(
                            torrent_params["url"], torrent_params=torrent_params
                        )
                        size = torrent_params["size"]
                    elif channel not in blocked_channels or (
                ...
                if play_items_torrent:
                    ...
                    if size_order:
                        ...
                        if (
                            size_order == 1
                            and len(play_items_torrent) > 2
                            and "4k" in play_items_torrent[0][2].lower()
                        ):  # Tomamos calidad 2
                            play_items_torrent[0][1] = (
                                0.0  # Ponemos el de más calidad al final de la lista
                            )
                            play_items_torrent = sorted(
                                play_items_torrent,
                                reverse=True,
                        ...
                        play_items_torrent = sorted(
                            play_items_torrent, key=lambda it: (float(it[1]))
                        )  # clasificamos
            ...
            if play_items[0].server == "torrent" and play_items[1].server == "torrent":
                for x, play_item in enumerate(play_items):
                    ...
                    if play_item.server == "torrent":
                        logger.info(
                            "torrent_info: %s, %s"
                            % (play_item.quality, play_item.torrent_info)
                        )
                        if play_item.torrent_info and scrapertools.find_single_match(
                            ...
                            if play_item.contentChannel in blocked_channels:
                                play_item.infoLabels["quality"] = play_item.infoLabels[
                                    "quality"
                                ].replace("720", " 720")
                                if "Digg" in play_item.torrent_info:
                                    play_item.infoLabels["quality"] = (
                                        "b%s" % play_item.infoLabels["quality"]
                                    )
                                if (
                                    "CF_BLOCKED" in play_item.torrent_info
                                    or "BLOQUEO" in play_item.torrent_info
                                ):
                                    play_item.infoLabels["quality"] = (
                                        "a%s" % play_item.infoLabels["quality"]
                                    )
                        elif play_item.url.startswith("magnet"):
                            logger.error("Size ERROR total: %s" % play_item.url)
                            return play_items
                        else:
                            if play_item.url.startswith(
                                "/"
                            ) or play_item.url.startswith("\\"):
                                play_item.url = filetools.join(
                                    config.get_videolibrary_path(),
                                    config.get_setting("folder_movies")
                                    if play_item.contentType == "movie"
                            ...
                            torrent_params["url"] = (
                                play_item.torrents_path
                                if "CF_BLOCKED" not in play_item.torrents_path
                                else play_item.url
                            )
                            if play_item.contentChannel not in blocked_channels or (
                                not torrent_params["lookup"]
                                and len(play_items) == x + 1
                            ):
                                torrent_params = get_torrent_size(
                                    torrent_params["url"],
                            ...
                            if filetools.isfile(torrent_params["torrents_path"]):
                                play_item.torrents_path = torrent_params[
                                    "torrents_path"
                                ]
                                if play_item.contentChannel not in blocked_channels:
                                    play_item.url = torrent_params[
                                        "torrents_path"
                                    ]
                        ...
                        try:
                            if play_item.contentChannel not in blocked_channels:
                                if "Seeds: 0" in play_item.torrent_info:
                                    magnitude = 0.0
                ...
                if play_item.contentChannel not in blocked_channels:
                    if size_order:
                        ...
                        if (
                            size_order == 1
                            and len(play_items_torrent) > 2
                            and "4k" in play_items_torrent[0].quality.lower()
                        ):  # Tomamos calidad 2
                            play_items_torrent[
                                0
                            ].size_torr = (
                        ...
                        play_items_torrent = sorted(
                            play_items_torrent, key=lambda it: (float(it.size_torr))
                        )  # clasificamos
                else:
                    if size_order:
                        play_items_torrent = sorted(
                            play_items_torrent, reverse=True, key=lambda it: it.quality
                        )  # clasificamos
                        if (
                            size_order == 1
                            and len(play_items_torrent) > 2
                            ...
                            play_items_torrent = sorted(
                                play_items_torrent,
                                reverse=True,
                                key=lambda it: it.quality,
                            )  # RE-clasificamos
                    else:
                        play_items_torrent = sorted(
                            play_items_torrent, key=lambda it: it.quality
                        )  # clasificamos
        ...
        for play_item in play_items:
            logger.info(
                "Qualitiy, Size FINAL: %s, %s"
                % (play_item.quality, play_item.size_torr)
            )
...
def download_from_url(url, item):
    ...
    item.downloadQueued = 0
    config.set_setting(
        "downloads_DOWNLOADER_in_use", True
    )  # Marcamos Downloader en uso
    ...
    config.set_setting(
        "downloads_DOWNLOADER_in_use", False
    )  # Marcamos Downloader como disponible
    return {
...
def download_from_server(item, silent=False):
    ...
    if (
        channeltools.has_attr(item.contentChannel, "play")
        and not item.play_menu
        and not item.url.startswith("magnet:")
    ):
        if not silent:
            progreso.update(
                50,
    ...
    logger.info(
        ...
        % (
            ...
            item.downloadQueued,
            item.server,
            item.torrents_path or item.url,
        )
    )
    ...
    if item.server == "torrent":
        ...
        if (
            config.get_videolibrary_path() not in item.url
            and torrent_paths["TORR_client"]
        ...
        if "cliente_torrent_Alfa.torrent" in item.url or (
            "ERROR" in item.torrent_info and CF_BLOCKED not in item.torrent_info
        ):
            try:
                if item.torrent_alt or item.emergency_urls:
                    ...
                    if (
                        config.get_videolibrary_path() not in item.url
                        and torrent_paths["TORR_client"]
                        and torrent_paths[torrent_paths["TORR_client"].upper()]
                        not in item.url
                        and not item.url.startswith("http")
                        and not item.url.startswith("magnet:")
                    ):
        ...
        if not PATH:
            ...
            if item.url_save_rec:
                result["url_save_rec"] = item.url_save_rec
            if item.post or item.post is None or item.post_back:
                result["post"] = item.post
            if item.post or item.post is None or item.post_back:
                result["post_back"] = item.post_back
            if item.referer or item.referer is None or item.referer_back:
                result["referer"] = item.referer
            if item.referer or item.referer is None or item.referer_back:
                result["referer_back"] = item.referer_back
            if item.headers or item.headers is None or item.headers_back:
                result["headers"] = item.headers
            if item.headers or item.headers is None or item.headers_back:
                result["headers_back"] = item.headers_back
    ...
    if (
        not item.server
        or not item.url
        or not item.contentAction == "play"
        or item.server in unsupported_servers
    ):
...
def download_from_best_server(item, silent=False):
    ...
    if not silent:
        progreso = platformtools.dialog_progress(
            config.get_localized_string(30101), config.get_localized_string(70179)
        )

    if not silent:
        progreso.update(
            50,
            config.get_localized_string(70184)
            + "\n"
    ...
    play_items = [
        x for x in play_items if x.action == "play" and "trailer" not in x.title.lower()
    ]

    if not silent:
        progreso.update(
            100,
            config.get_localized_string(70183)
            + "\n"
    ...
    for play_item in play_items:
        ...
        if result["downloadStatus"] in [
            ...
            if "downloadServer" not in result:
                result["downloadServer"] = {
                    "url": play_item.url,
                    "server": play_item.server,
                }
            break
...
def select_server(item):
    ...
    for x, i in enumerate(play_items):
        if not i.server and channeltools.has_attr(i.channel, "play"):
            play_items[x] = channeltools.get_channel_attr(i.channel, "play")(i)

    seleccion = platformtools.dialog_select(
        config.get_localized_string(70192), ["Auto"] + [s.title for s in play_items]
    )
    if seleccion > 0:
        update_control(
            item.path,
            {
                "downloadServer": {
                    "url": play_items[seleccion - 1].url,
                    "server": play_items[seleccion - 1].server,
                }
            },
            function="select_server_1",
        )
    elif seleccion == 0:
...
def start_download(item):
    ...
    if (
        (item.contentAction == "play" or item.from_action == "play")
        and item.server == "torrent"
        and not item.url.startswith("magnet")
        and (
            item.contentChannel not in blocked_channels or not torrent_params["lookup"]
        )
    ):
        from lib.generictools import get_torrent_size
        ...
        if (
            "ERROR" in torrent_params["size"]
            and "CF_BLOCKED" not in torrent_params["torrents_path"]
        ):
            if item.url_save_rec:
                ...
                item.url = item.url_control
            if item.url_save_rec and item.url_save_rec[1]:
                item.headers = item.url_save_rec[1]
            item.contentAction = "findvideos"
        elif (
            "ERROR_CF_BLOCKED" in torrent_params["torrents_path"]
            or "ERROR_CF_BLOCKED" in torrent_params["size"]
        ):
    ...
    if item.contentAction == "play" or item.from_action == "play":
        ...
        ret = download_from_server(
            item.clone(
                server=item.downloadServer.get("server"),
                url=item.downloadServer.get("url"),
                contentAction="play",
            )
        )
...
def get_episodes(item):
    logger.info(
        "contentAction: %s | sub_action: %s | contentChannel: %s | contentType: %s | contentSeason: %s | contentEpisodeNumber: %s"
        % (
            item.contentAction,
            item.sub_action,
            item.contentChannel,
    ...
    sub_action = [
        ...
        "unseen",
        "auto",
    ]  # Acciones especiales desde Findvideos
    SERIES = filetools.join(
        config.get_videolibrary_path(), config.get_setting("folder_tvshows")
    )
    head = ""
    ...
    sesxepi = []
    event = False
    channel_settings = filetools.join(
        config.get_data_path(), "settings_channels", item.contentChannel + "_data.json"
    )
    channel_json = (
        jsontools.load(filetools.read(channel_settings))
        ...
        if item.infoLabels["imdb_id"]:
            if filetools.exists(
                filetools.join(
                    SERIES,
                    "%s [%s]"
                    % (item.contentSerieName.lower(), item.infoLabels["imdb_id"]),
                    "tvshow.nfo",
                )
            ):
                item.nfo = filetools.join(
                    SERIES,
                    "%s [%s]"
                    % (item.contentSerieName.lower(), item.infoLabels["imdb_id"]),
                    "tvshow.nfo",
            ...
            if filetools.exists(
                filetools.join(
                    SERIES,
                    "%s [tmdb_%s]"
                    % (item.contentSerieName.lower(), item.infoLabels["tmdb_id"]),
                    "tvshow.nfo",
                )
            ):
                item.nfo = filetools.join(
    ...
    if item.nfo:
        if filetools.exists(item.nfo):
            head, nfo_json = videolibrarytools.read_nfo(
                item.nfo
            )  # ... tratamos de recuperar la info de la Serie
    ...
    for serie, address in move_to_remote:
        if (
            serie.lower() in item.contentSerieName.lower()
        ):  # Si está en la lista es que es remoto
            remote = True
            break

    # El item que pretendemos descargar YA es un episodio
    if item.contentType == "episode" and (
        not item.sub_action or item.sub_action not in sub_action
    ):
        episodes = [item.clone()]
        ...
        if item.sub_action in sub_action:  # Si viene de Play...
            if item.sub_action in ["unseen", "auto"] and config.is_xbmc() and item.nfo:
                ...
                xbmc_videolibrary.update(
                    folder="_scan_series"
                )  # Se escanea el directorio de series para catalogar en Kodi
                ...
                xbmc_videolibrary.mark_content_as_watched_on_alfa(
                    item.nfo
                )  # Sincronizamos los Vistos de Kodi con Alfa ...
                head, nfo_json = videolibrarytools.read_nfo(
                    item.nfo
                )  # ... refrescamos la info de la Serie
                format_tmdb_id(nfo_json)
            ...
            if item.sub_action in ["auto"] and not config.get_setting(
                "auto_download_new_all", item.contentChannel, default=False
            ):
                if not nfo_json:
                    return []
            ...
            if item.sub_action in ["unseen"] and item.from_action != "episodios":
                ...
                if item.headers:
                    del item.headers
            if not item.from_action:
                item.from_action = "episodios"
            if not item.contentAction:
                item.contentAction = item.from_action
            ...
            if item.from_num_season_colapse:
                del item.from_num_season_colapse
            if item.password:
                del item.password
            if item.torrent_info:
                del item.torrent_info
            if item.torrent_alt:
                del item.torrent_alt

        if (item.strm_path or item.nfo or nfo_json) and item.sub_action in [
            "auto"
        ]:  # Si viene de Videoteca, usamos los .jsons
            if item.strm_path:
                serie_path = filetools.dirname(item.strm_path)
            if not serie_path and item.nfo:
                serie_path = filetools.dirname(item.nfo)
            if not serie_path and nfo_json:
                serie_path = filetools.join(SERIES, item.path.lower())
            ...
            for file in serie_listdir:
                ...
                episode = Item().fromjson(
                    filetools.read(filetools.join(serie_path, file))
                )
                if remote:  # Quitamos las referencias locales
                    if episode.emergency_urls:
                        del episode.emergency_urls
                    if episode.torrent_info:
                        del episode.torrent_info
                    if episode.server:
                        del episode.server
                if (
                    episode.emergency_urls
                    and isinstance(episode.emergency_urls[0][0], str)
                    and (
                        (
                            not episode.emergency_urls[0][0].startswith("http")
                            and (
                                episode.emergency_urls[0][0].endswith(".torrent")
                    ...
                    episode.action = "play"
                episode.strm_path = filetools.join(
                    serie_path,
                    "%sx%s.strm"
                    % (
            ...
            if item.action == "play":
                ...
                if item.quality != "HDTV":
                    item.quality = "HDTV-720p"

            episodes = getattr(channel, item.contentAction)(
                item
            )  # Si no viene de Videoteca, descargamos desde la web
    ...
    for episode in episodes:
        if (
            episode.action in ["add_serie_to_library", "actualizar_titulos"]
            or not episode.action
        ):
            continue

        # Descargar en ...
        if item.downloadAt:
            episode.downloadAt = item.downloadAt

        # Si se ha llamado con la opción de NO Vistos, se comprueba contra el .NFO de la serie
        if (
            item.sub_action in ["unseen", "auto"]
            and nfo_json
            and nfo_json.library_playcounts
            and episode.contentSeason
            and episode.contentEpisodeNumber
        ...
        if item.sub_action in ["tvshow", "unseen"] and episode.strm_path:
            episode.strm_path = re.sub(
                r"x\d{2,}.strm",
                "x%s.strm" % str(episode.contentEpisodeNumber).zfill(2),
                episode.strm_path,
            )
        ...
        if episode.emergency_urls and not remote:
            for x, emergency_urls in enumerate(episode.emergency_urls[0]):
                if (
                    episode.emergency_urls[0][x].startswith("magnet")
                    and len(episode.emergency_urls) > 3
                ):
                    continue
                if not episode_local:
                    episode.emergency_urls[0][x] = re.sub(
                        r"x\d{2,}\s*\[",
                        "x%s [" % str(episode.contentEpisodeNumber).zfill(2),
                        emergency_urls,
                    )
                if not episode.emergency_urls[0][x] or (
                    not episode.emergency_urls[0][x].startswith("http")
                    and not filetools.exists(
                        filetools.join(SERIES, episode.emergency_urls[0][x])
                    )
                ):
            ...
            if not episode.emergency_urls[0]:
                ...
                if (
                    len(episode.emergency_urls) > 3
                    and episode.emergency_urls[0]
                    and episode.emergency_urls[3]
                    ...
                    if len(episode.emergency_urls[0]) > 1:
                        ...
                        epis_filter = filtertools.get_links(
                            epis_filter, episode, list_language, list_quality
                        )
                        if epis_filter:
                            ...
                            for x, epi_filter in enumerate(epis_filter):
                                if (
                                    not epi_filter.contentSeason
                                    or not epi_filter.contentEpisodeNumber
                                ):
                                    continue
                                episode.emergency_urls[0].append(epi_filter.url)
                                episode.emergency_urls[2].append(epi_filter.url)
                                episode.emergency_urls[3].append(
                                    emergency_urls_3[epi_filter.order]
                                )
                ...
                if episode_sort:
                    ...
                    emergency_urls_alt = sort_torrents(
                        episode.emergency_urls[0],
                        emergency_urls=True,
                    ...
                    if "ERROR_CF_BLOCKED" in torrent_params["size"]:
                        ...
                        if emergency_urls_alt[0][2]:
                            episode.quality = emergency_urls_alt[0][2]

                elif (
                    len(episode.emergency_urls) > 2
                    and episode.emergency_urls[0]
                    and episode.emergency_urls[2]
                ):
                    try:
        ...
        if episode.contentType == "episode":
            ...
            if (
                not isinstance(episode.contentSeason, int)
                or not episode.contentEpisodeNumber
            ):
            ...
            if not isinstance(episode.contentSeason, int):
                episode.contentSeason = 1
            if not episode.contentEpisodeNumber:
                episode.contentEpisodeNumber = 1
            episode.downloadFilename = filetools.join(
                item.downloadFilename,
                filetools.validate_path(
                    "%dx%0.2d - %s"
                    % (
    ...
    if item.add_videolibrary:
        ...
        if channel and itemlist:
            itemlist = filtertools.get_links(
                itemlist,
                item,
                channel.list_language,
                channel.list_quality_tvshow,
                replace_label=config.BTDIGG_LABEL,
            )
...
def write_json(item):
    ...
    filetools.write(
        filetools.join(DOWNLOAD_LIST_PATH, item.path), item_control.tojson()
    )
    time.sleep(0.1)
...
def save_download(item, silent=False):
    ...
    if (
        ...
        and item.contentAction == "videolibrary"
        and item.from_channel
        and item.from_action
    ):
        item.contentAction = item.from_action
    ...
    if item.from_action and item.from_channel:
        if "list" in item.contentChannel and item.channel != "downloads":
        ...
        logger.debug(
            "Activar descargas experimentales: "
            + str(config.get_setting("downloads_alternate_system"))
        )
        if config.get_setting("downloads_alternate_system") is True:
            ...
            if item.server != "torrent" and item.sub_action != "auto":
                    ...
                    if len(channels_list) > 1:
                        ...
                        if isinstance(channels_list[0], list):
                            ...
                            for c, url in channels_list:
                                channel_title = channeltools.get_channel_parameters(c)[
                                    "title"
                                ]
                                channels.append(
                                    "Descargar desde {}".format(channel_title)
                                )
                            seleccion = platformtools.dialog_select(
                                "Descargar desde el canal...", channels
                            )
                            if seleccion != -1:
                            ...
                            seleccion = platformtools.dialog_select(
                                "{}...".format(config.get_localized_string(70763)),
                                channels,
                            )
                            if seleccion != -1:
                ...
                if item.action in ["play", "findvideos"] and not item.channel == "list":
                    ...
                    error_generico = "{}[CR]{}{}".format(
                        config.get_localized_string(60014),
                        config.get_localized_string(50004),
                        envtal.get_environment()["log_path"],
                    )
                    ...
                    if item.action == "findvideos":
                        ...
                        result = channeltools.get_channel_attr(
                            item.channel, "findvideos", item
                        )
                        if isinstance(result, list):
                            if len(result) > 1:
                                ...
                                logger.error(
                                    "ERROR: result no devolvió enlaces válidos"
                                )
                                logger.error(result)
                                from platformcode import envtal

                                platformtools.dialog_ok(
                                    config.get_localized_string(60208), error_generico
                                )
                                return False
                            item.action = "play"
                        else:
                    ...
                    if item.action == "play" or item.downloadServer:
                        ...
                        if item.video_urls:
                            ...
                            video_urls, puedes, motivo = (
                                servertools.resolve_video_urls_for_playing(
                                    item.server, item.url, item.password, True
                                )
                            )
                        ...
                        if puedes:
                            ...
                            if len(video_urls) > 1:
                                ...
                                for it in video_urls:
                                    opciones.append(
                                        "{} {}".format(
                                            config.get_localized_string(30153), it[0]
                                        )
                                    )
                                seleccion = platformtools.dialog_select(
                                    config.get_localized_string(30163), opciones
                            ...
                            if "motivo" not in locals():
                                motivo = error_generico
                            platformtools.dialog_ok(
                                config.get_localized_string(60208), motivo
                            )
                            return False
...
def save_download_video(item):
    logger.info(
        "contentAction: %s | contentChannel: %s | contentTitle: %s"
    ...
    item.downloadFilename = filetools.validate_path(
        "%s [%s]" % (item.contentTitle.strip(), item.contentChannel)
    )

    write_json(item)

    if not platformtools.dialog_yesno(
        config.get_localized_string(30101), config.get_localized_string(70189)
    ):
        platformtools.dialog_ok(
            config.get_localized_string(30101),
            item.contentTitle,
        ...
        update_control(
            item.path,
            {
                "downloadCompleted": item.downloadCompleted,
                "downloadQueued": item.downloadQueued,
            },
            function="save_download_video",
        )
        start_download(item)
...
def save_download_movie(item, silent=False):
    ...
    if not silent:
        progreso = platformtools.dialog_progress(
            config.get_localized_string(30101), config.get_localized_string(70191)
        )
    ...
    if item.emergency_urls:  # Se intenta usar el .torrent local
        ...
        for x, emergency_urls in enumerate(item.emergency_urls[0]):
            if len(item.emergency_urls[0][x]) == 0 or not filetools.exists(
                filetools.join(PATH, item.emergency_urls[0][x])
            ):
    ...
    result = ""
    if (
        not item.infoLabels["tmdb_id"]
        and item.infoLabels["tmdb_id"] != null
        and not channeltools.is_adult(item.channel)
    ):
    ...
    if not silent and not platformtools.dialog_yesno(
        config.get_localized_string(30101), config.get_localized_string(70189)
    ):
        platformtools.dialog_ok(
            config.get_localized_string(30101),
...
def save_download_tvshow(item, silent=False):
    ...
    logger.info(
        "contentAction: %s | contentChannel: %s | contentType: %s | contentSerieName: %s | sub_action: %s"
        % (
            item.contentAction,
            item.contentChannel,
            item.contentType,
    ...
    if (
        ...
        if len(item.emergency_urls) > 3:
            ...
            item.emergency_urls[3] += [
                "#%s#[%s]" % (item.infoLabels["quality"], item.torrent_info)
            ]

    if not silent:
        progreso.update(
            0,
            config.get_localized_string(70186)
            + "\n"
    ...
    for x, i in enumerate(episodes):
        ...
        write_json(i)
        epi_saved += [
            "%sx%s" % (str(i.contentSeason), str(i.contentEpisodeNumber).zfill(2))
        ]
    if not silent:
        progreso.close()
    logger.info(
        "Serie: %s | Total Saved: %s | Episodes: %s"
        % (item.contentSerieName, str(len(episodes)), str(sorted(epi_saved)))
    )
    ...
    if not silent and not platformtools.dialog_yesno(
        ...
        for i in episodes:
            ...
            update_control(
                i.path,
                {
                    "downloadCompleted": i.downloadCompleted,
                    "downloadQueued": i.downloadQueued,
                },
                function="save_download_tvshow",
            )
        for i in episodes:
            time.sleep(0.5)
            if filetools.exists(filetools.join(DOWNLOAD_LIST_PATH, i.path)):
                ...
                logger.info(
                    "Episodio BORRADO: Serie: %s | Episode: %sx%s | Archivo: %s"
                    % (
                        i.contentSerieName,
                        str(i.contentSeason),
                        str(i.contentEpisodeNumber).zfill(2),
                        i.path,
                    )
                )
