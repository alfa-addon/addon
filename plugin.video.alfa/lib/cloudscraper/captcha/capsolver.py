...
class captchaSolver(Captcha):
    ...
    def checkErrorStatus(response, request_type):
        if response.status_code in [500, 502]:
            raise CaptchaServiceUnavailable('CapSolver: Server Side Error {}'.format(response.status_code))
        ...
        if rPayload.get('errorDescription', False) and 'Current system busy' not in rPayload['errorDescription']:
            raise CaptchaAPIError(
                "CapSolver: {} -> {}".format(request_type, rPayload.get('errorDescription'))"
            )
    ...
    def getCaptchaAnswer(self, captchaType, url, siteKey, captchaParams):
        ...
        try:
            ...
            raise CaptchaTimeout(
                "CapSolver: Captcha solve (task ID: {} took to long.".format(jobID)
            )
