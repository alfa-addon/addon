...
class Handler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def send_pls(self, files):
        ...
        for x, f in enumerate(files):
            playlist += (
                ...
                + str(x + 1)
                + "=http://"
                + self.server._client.ip
                + ":"
                + str(self.server._client.port)
                + "/"
                + urlparse.quote(f.name)
                + "\n"
            )
    ...
    def do_HEAD(self):
        ...
        if PY3:
            filename = urlparse.unquote(url)[1:]
        else:
            filename = urlparse.unquote(url)[1:].decode("utf-8")

        if (
            not self.server._client.file
            or urlparse.unquote(url)[1:] != self.server._client.file.name
        ):
