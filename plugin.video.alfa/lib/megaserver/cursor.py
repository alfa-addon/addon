    ...
    def mega_request(self, offset, retry=False):
        if not self._file.url or retry:
            if self._file.folder_id:
                file = self._file._client.api_req({"a": "g", "g": 1, "n": self._file.file_id},
                                                  "&n=" + self._file.folder_id)
        ...
        try:
            ...
            # La url del archivo expira transcurrido un tiempo, si da error 403, reintenta volviendo a
            # solicitar la url mediante la API
